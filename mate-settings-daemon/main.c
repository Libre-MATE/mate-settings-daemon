...
#define MATE_SESSION_PRIVATE_DBUS_INTERFACE \
...
static GOptionEntry entries[] = {
    ...
    {"replace", 0, 0, G_OPTION_ARG_NONE, &replace,
     N_("Replace the current daemon"), NULL},
    {"no-daemon", 0, G_OPTION_FLAG_HIDDEN, G_OPTION_ARG_NONE, &no_daemon,
     N_("Don't become a daemon"), NULL},
    {"timed-exit", 0, 0, G_OPTION_ARG_NONE, &do_timed_exit,
     N_("Exit after a time (for debugging)"), NULL},
...
static gboolean acquire_name_on_proxy(DBusGProxy *bus_proxy) {
  ...
  res = dbus_g_proxy_call(bus_proxy, "RequestName", &error, G_TYPE_STRING,
                          MSD_DBUS_NAME, G_TYPE_UINT, flags, G_TYPE_INVALID,
...
static DBusHandlerResult bus_message_handler(DBusConnection *connection,
                                             DBusMessage *message,
                                             void *user_data) {
...
static DBusGConnection *get_session_bus(void) {
  ...
  dbus_connection_add_filter(
      connection, (DBusHandleMessageFunction)bus_message_handler, NULL, NULL);
...
static void on_session_over(DBusGProxy *proxy, MateSettingsManager *manager) {
  /* not used, see on_session_end instead */
}

static void on_session_query_end(DBusGProxy *proxy, guint flags,
                                 MateSettingsManager *manager) {
  GError *error = NULL;
  gboolean ret = FALSE;

  /* send response */
  ret = dbus_g_proxy_call(proxy, "EndSessionResponse", &error, G_TYPE_BOOLEAN,
                          TRUE /* ok */, G_TYPE_STRING, NULL /* reason */,
...
static void on_session_end(DBusGProxy *proxy, guint flags,
                           MateSettingsManager *manager) {
  GError *error = NULL;
  gboolean ret = FALSE;
...
static gboolean on_term_signal_pipe_closed(GIOChannel *source,
...
static void watch_for_term_signal(MateSettingsManager *manager) {
  GIOChannel *channel;
...
static void set_session_over_handler(DBusGConnection *bus,
  ...
  session_proxy = dbus_g_proxy_new_for_name(bus, MATE_SESSION_DBUS_NAME,
                                            MATE_SESSION_DBUS_OBJECT,
                                            MATE_SESSION_DBUS_INTERFACE);

  dbus_g_object_register_marshaller(g_cclosure_marshal_VOID__VOID, G_TYPE_NONE,
                                    G_TYPE_INVALID);

  dbus_g_proxy_add_signal(session_proxy, "SessionOver", G_TYPE_INVALID);

  dbus_g_proxy_connect_signal(session_proxy, "SessionOver",
                              G_CALLBACK(on_session_over), manager, NULL);

  /* Register with mate-session */
  startup_id = g_getenv("DESKTOP_AUTOSTART_ID");
  if (startup_id != NULL && *startup_id != '\0') {
    res = dbus_g_proxy_call(
        session_proxy, "RegisterClient", &error, G_TYPE_STRING,
        "mate-settings-daemon", G_TYPE_STRING, startup_id, G_TYPE_INVALID,
        DBUS_TYPE_G_OBJECT_PATH, &client_id, G_TYPE_INVALID);
    if (!res) {
      g_warning("failed to register client '%s': %s", startup_id,
                error->message);
      g_error_free(error);
    } else {
      ...
      if (private_proxy == NULL) {
        ...
        /* get QueryEndSession */
        dbus_g_proxy_add_signal(private_proxy, "QueryEndSession", G_TYPE_UINT,
                                G_TYPE_INVALID);
        dbus_g_proxy_connect_signal(private_proxy, "QueryEndSession",
                                    G_CALLBACK(on_session_query_end), manager,
                                    NULL);

        /* get EndSession */
        dbus_g_proxy_add_signal(private_proxy, "EndSession", G_TYPE_UINT,
                                G_TYPE_INVALID);
...
static void msd_log_default_handler(const gchar *log_domain,
...
int main(int argc, char *argv[]) {
   ...
   * mate-settings-daemon has registered itself. Otherwise, wait for an Awake
   * etc. */
  if (g_getenv("DBUS_STARTER_BUS_TYPE") == NULL) {
    error = NULL;
