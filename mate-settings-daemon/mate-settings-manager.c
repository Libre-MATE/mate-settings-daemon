...
enum { PLUGIN_ACTIVATED, PLUGIN_DEACTIVATED, LAST_SIGNAL };
...
G_DEFINE_TYPE_WITH_PRIVATE(MateSettingsManager, mate_settings_manager,
                           G_TYPE_OBJECT)
...
static void maybe_activate_plugin(MateSettingsPluginInfo *info,
  ...
  if (mate_settings_plugin_info_get_enabled(info)) {
    ...
    if (manager->priv->load_init_flag == PLUGIN_LOAD_ALL ||
        (manager->priv->load_init_flag == PLUGIN_LOAD_INIT &&
         plugin_priority <= manager->priv->init_load_priority) ||
        (manager->priv->load_init_flag == PLUGIN_LOAD_DEFER &&
         plugin_priority > manager->priv->init_load_priority)) {
      gboolean res;
      res = mate_settings_plugin_info_activate(info);
      if (res) {
        g_debug("Plugin %s: active",
                mate_settings_plugin_info_get_location(info));
      } else {
        g_debug("Plugin %s: activation failed",
                mate_settings_plugin_info_get_location(info));
      }
    } else {
      g_debug("Plugin %s: loading deferred or previously loaded",
              mate_settings_plugin_info_get_location(info));
    }
  } else {
    g_debug("Plugin %s: inactive",
            mate_settings_plugin_info_get_location(info));
  }
}

static gint compare_location(MateSettingsPluginInfo *a,
...
static int compare_priority(MateSettingsPluginInfo *a,
                            MateSettingsPluginInfo *b) {
  return mate_settings_plugin_info_get_priority(a) -
         mate_settings_plugin_info_get_priority(b);
}

static void on_plugin_activated(MateSettingsPluginInfo *info,
                                MateSettingsManager *manager) {
  const char *name;
...
static void on_plugin_deactivated(MateSettingsPluginInfo *info,
                                  MateSettingsManager *manager) {
  const char *name;
  name = mate_settings_plugin_info_get_location(info);
...
static gboolean is_item_in_schema(char **items, const char *item) {
  while (*items) {
...
static gboolean is_schema(const char *schema) {
  ...
  g_settings_schema_source_list_schemas(source, TRUE, &non_relocatable,
                                        &relocatable);
...
static void _load_file(MateSettingsManager *manager, const char *filename) {
  ...
  l = g_slist_find_custom(manager->priv->plugins, info,
                          (GCompareFunc)compare_location);
  if (l != NULL) {
    goto out;
  }

  schema = g_strdup_printf("%s.plugins.%s", DEFAULT_SETTINGS_PREFIX,
                           mate_settings_plugin_info_get_location(info));

  /* Ignore unknown schemas or else we'll assert */
  if (is_schema(schema)) {
    manager->priv->plugins =
        g_slist_prepend(manager->priv->plugins, g_object_ref(info));

    g_signal_connect(info, "activated", G_CALLBACK(on_plugin_activated),
                     manager);
    g_signal_connect(info, "deactivated", G_CALLBACK(on_plugin_deactivated),
                     manager);
...
static void _load_dir(MateSettingsManager *manager, const char *path) {
  GError *error;
  GDir *d;
  const char *name;
...
static void _load_all(MateSettingsManager *manager) {
  mate_settings_profile_start(NULL);
  ...
  manager->priv->plugins =
      g_slist_sort(manager->priv->plugins, (GCompareFunc)compare_priority);
  g_slist_foreach(manager->priv->plugins, (GFunc)maybe_activate_plugin,
                  manager);
  mate_settings_profile_end(NULL);
}

static void _unload_plugin(MateSettingsPluginInfo *info, gpointer user_data) {
  if (mate_settings_plugin_info_get_enabled(info)) {
...
static void _unload_all(MateSettingsManager *manager) {
...
static gboolean register_manager(MateSettingsManager *manager) {
  GError *error = NULL;
  ...
  dbus_g_connection_register_g_object(manager->priv->connection,
                                      MSD_MANAGER_DBUS_PATH, G_OBJECT(manager));
...
gboolean mate_settings_manager_start(MateSettingsManager *manager,
  ...
  if (!g_module_supported()) {
    ...
    g_set_error(error, MATE_SETTINGS_MANAGER_ERROR,
                MATE_SETTINGS_MANAGER_ERROR_GENERAL, "Plugins not supported");
...
static void mate_settings_manager_dispose(GObject *object) {
  MateSettingsManager *manager;
...
static void mate_settings_manager_class_init(MateSettingsManagerClass *klass) {
  ...
  signals[PLUGIN_ACTIVATED] = g_signal_new(
      "plugin-activated", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(MateSettingsManagerClass, plugin_activated), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);
  signals[PLUGIN_DEACTIVATED] = g_signal_new(
      "plugin-deactivated", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(MateSettingsManagerClass, plugin_deactivated), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);

  dbus_g_object_type_install_info(MATE_TYPE_SETTINGS_MANAGER,
                                  &dbus_glib_mate_settings_manager_object_info);
}

static void mate_settings_manager_init(MateSettingsManager *manager) {
  char *schema;
  ...
  if (is_schema(schema)) {
    ...
    manager->priv->init_load_priority =
        g_settings_get_int(settings, "init-load-priority");
...
MateSettingsManager *mate_settings_manager_new(void) {
  if (manager_object != NULL) {
    ...
    manager_object = g_object_new(MATE_TYPE_SETTINGS_MANAGER, NULL);
