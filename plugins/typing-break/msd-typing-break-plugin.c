...
MATE_SETTINGS_PLUGIN_REGISTER_WITH_PRIVATE(MsdTypingBreakPlugin,
                                           msd_typing_break_plugin)

static void msd_typing_break_plugin_init(MsdTypingBreakPlugin *plugin) {
...
static void msd_typing_break_plugin_finalize(GObject *object) {
...
static void impl_activate(MateSettingsPlugin *plugin) {
  ...
  error = NULL;
  res = msd_typing_break_manager_start(
      MSD_TYPING_BREAK_PLUGIN(plugin)->priv->manager, &error);
  if (!res) {
...
static void msd_typing_break_plugin_class_init(
    MsdTypingBreakPluginClass *klass) {
...
static void msd_typing_break_plugin_class_finalize(
    MsdTypingBreakPluginClass *klass) {}
