...
static gboolean typing_break_timeout(MsdTypingBreakManager *manager) {
...
static void child_watch(GPid pid, int status, MsdTypingBreakManager *manager) {
...
static void setup_typing_break(MsdTypingBreakManager *manager,
                               gboolean enabled) {
  mate_settings_profile_start(NULL);

  if (!enabled) {
    if (manager->typing_monitor_pid != 0) {
      manager->typing_monitor_idle_id =
          g_timeout_add_seconds(3, (GSourceFunc)typing_break_timeout, manager);
    }
    return;
  }
  ...
  if (manager->typing_monitor_pid == 0) {
    ...
    res =
        g_spawn_async("/", argv, NULL,
                      ...
                      NULL, NULL, &manager->typing_monitor_pid, &error);
    if (!res) {
    ...
    manager->child_watch_id = g_child_watch_add(
        manager->typing_monitor_pid, (GChildWatchFunc)child_watch, manager);
  }

  mate_settings_profile_end(NULL);
}

static void typing_break_enabled_callback(GSettings *settings, gchar *key,
                                          MsdTypingBreakManager *manager) {
  setup_typing_break(manager, g_settings_get_boolean(settings, key));
}

static gboolean really_setup_typing_break(MsdTypingBreakManager *manager) {
...
gboolean msd_typing_break_manager_start(MsdTypingBreakManager *manager,
  ...
  g_signal_connect(manager->settings, "changed::enabled",
                   G_CALLBACK(typing_break_enabled_callback), manager);

  enabled = g_settings_get_boolean(manager->settings, "enabled");

  if (enabled) {
    manager->setup_id = g_timeout_add_seconds(
        3, (GSourceFunc)really_setup_typing_break, manager);
  }
...
static void msd_typing_break_manager_class_init(
...
static void msd_typing_break_manager_init(MsdTypingBreakManager *manager) {}
