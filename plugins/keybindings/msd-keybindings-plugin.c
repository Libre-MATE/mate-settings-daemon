...
MATE_SETTINGS_PLUGIN_REGISTER_WITH_PRIVATE(MsdKeybindingsPlugin,
                                           msd_keybindings_plugin)

static void msd_keybindings_plugin_init(MsdKeybindingsPlugin *plugin) {
...
static void msd_keybindings_plugin_finalize(GObject *object) {
...
static void impl_activate(MateSettingsPlugin *plugin) {
  ...
  error = NULL;
  res = msd_keybindings_manager_start(
      MSD_KEYBINDINGS_PLUGIN(plugin)->priv->manager, &error);
  if (!res) {
...
static void msd_keybindings_plugin_class_init(
...
static void msd_keybindings_plugin_class_finalize(
    MsdKeybindingsPluginClass *klass) {}
