...
G_DEFINE_TYPE_WITH_PRIVATE(MsdKeybindingsManager, msd_keybindings_manager,
                           G_TYPE_OBJECT)

static gpointer manager_object = NULL;

static GSList *get_screens_list(void) {
  GSList *list = NULL;
...
static gboolean parse_binding(Binding *binding) {
  gboolean success;
  ...
  if (binding->binding_str == NULL || binding->binding_str[0] == '\0' ||
      g_strcmp0(binding->binding_str, "Disabled") == 0 ||
      g_strcmp0(binding->binding_str, "disabled") == 0) {
    return FALSE;
  }

  success = egg_accelerator_parse_virtual(
      binding->binding_str, &binding->key.keysym, &binding->key.keycodes,
      &binding->key.state);
...
static gint compare_bindings(gconstpointer a, gconstpointer b) {
...
static gboolean bindings_get_entry(MsdKeybindingsManager *manager,
  ...
  g_debug("keybindings: get entries from '%s' (action: '%s', key: '%s')",
          settings_path, action, key);

  tmp_elem = g_slist_find_custom(manager->priv->binding_list, settings_path,
                                 compare_bindings);
  ...
  if (parse_binding(new_binding)) {
    if (!tmp_elem)
      manager->priv->binding_list =
          g_slist_prepend(manager->priv->binding_list, new_binding);
  } else {
    g_free(new_binding->binding_str);
    g_free(new_binding->action);
    g_free(new_binding->settings_path);
    ...
    if (tmp_elem)
      manager->priv->binding_list =
          g_slist_delete_link(manager->priv->binding_list, tmp_elem);
    return FALSE;
  }
...
static void bindings_clear(MsdKeybindingsManager *manager) {
...
static void bindings_get_entries(MsdKeybindingsManager *manager) {
  ...
  if (custom_list != NULL) {
    for (i = 0; custom_list[i] != NULL; i++) {
      gchar *settings_path;
      settings_path =
          g_strdup_printf("%s%s", GSETTINGS_KEYBINDINGS_DIR, custom_list[i]);
...
static gboolean same_keycode(const Key *key, const Key *other) {
  if (key->keycodes != NULL && other->keycodes != NULL) {
    guint *c;

    for (c = key->keycodes; *c; ++c) {
      if (key_uses_keycode(other, *c)) return TRUE;
    }
  }
  return FALSE;
}

static gboolean same_key(const Key *key, const Key *other) {
  if (key->state == other->state) {
    if (key->keycodes != NULL && other->keycodes != NULL) {
      guint *c1, *c2;

      for (c1 = key->keycodes, c2 = other->keycodes; *c1 || *c2; ++c1, ++c2) {
...
static gboolean key_already_used(MsdKeybindingsManager *manager,
                                 Binding *binding) {
  GSList *li;
...
static void binding_unregister_keys(MsdKeybindingsManager *manager) {
  GdkDisplay *dpy;
  GSList *li;
...
static void binding_register_keys(MsdKeybindingsManager *manager) {
  GSList *li;
  GdkDisplay *dpy;
  gboolean need_flush = FALSE;
    ...
    if (!same_key(&binding->previous_key, &binding->key)) {
      ...
      if (!key_already_used(manager, binding)) {
        ...
        if (binding->previous_key.keycodes) {
          grab_key_unsafe(&binding->previous_key, FALSE,
...
static char *screen_exec_display_string(GdkScreen *screen) {
  GString *str;
 ...
 **/
static char **get_exec_environment(XEvent *xevent) {
  char **retval = NULL;
  int i;
  int display_index = -1;
  GdkScreen *screen = NULL;
  GdkWindow *window = gdk_x11_window_lookup_for_display(
      gdk_display_get_default(), xevent->xkey.root);
...
static GdkFilterReturn keybindings_filter(GdkXEvent *gdk_xevent,
                                          GdkEvent *event,
  ...
  for (li = manager->priv->binding_list; li != NULL; li = li->next) {
    ...
    if (match_key(&binding->key, xevent)) {
      ...
      if (!g_shell_parse_argv(binding->action, NULL, &argv, &error)) {
        return GDK_FILTER_CONTINUE;
      }

      envp = get_exec_environment(xevent);

      retval = g_spawn_async(NULL, argv, envp, G_SPAWN_SEARCH_PATH, NULL, NULL,
                             NULL, &error);
      g_strfreev(argv);
      g_strfreev(envp);
...
gboolean msd_keybindings_manager_start(MsdKeybindingsManager *manager,
  ...
  gdk_window_add_filter(window, (GdkFilterFunc)keybindings_filter, manager);
  ...
  g_signal_connect(manager->priv->client, "changed",
                   G_CALLBACK(bindings_callback), manager);
...
void msd_keybindings_manager_stop(MsdKeybindingsManager *manager) {
  ...
  for (l = p->screens; l; l = l->next) {
    ...
    gdk_window_remove_filter(gdk_screen_get_root_window(screen),
                             (GdkFilterFunc)keybindings_filter, manager);
  }
...
static void msd_keybindings_manager_class_init(
...
static void msd_keybindings_manager_init(MsdKeybindingsManager *manager) {
