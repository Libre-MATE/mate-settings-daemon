...
G_DEFINE_TYPE_WITH_PRIVATE(MsdA11ySettingsManager, msd_a11y_settings_manager,
                           G_TYPE_OBJECT)
...
static void apps_settings_changed(GSettings *settings, const char *key,
  ...
  screen_reader = g_settings_get_boolean(manager->priv->a11y_apps_settings,
                                         "screen-reader-enabled");
  keyboard = g_settings_get_boolean(manager->priv->a11y_apps_settings,
                                    "screen-keyboard-enabled");

  if (screen_reader || keyboard) {
    g_debug("Enabling accessibility, screen reader or OSK enabled");
    g_settings_set_boolean(manager->priv->interface_settings, "accessibility",
                           TRUE);
...
gboolean msd_a11y_settings_manager_start(MsdA11ySettingsManager *manager,
  ...
  if (g_settings_get_boolean(manager->priv->a11y_apps_settings,
                             "screen-keyboard-enabled") ||
      g_settings_get_boolean(manager->priv->a11y_apps_settings,
                             "screen-reader-enabled"))
    g_settings_set_boolean(manager->priv->interface_settings, "accessibility",
                           TRUE);
...
static GObject *msd_a11y_settings_manager_constructor(
    GType type, guint n_construct_properties,
    GObjectConstructParam *construct_properties) {
  MsdA11ySettingsManager *a11y_settings_manager;

  a11y_settings_manager = MSD_A11Y_SETTINGS_MANAGER(
      G_OBJECT_CLASS(msd_a11y_settings_manager_parent_class)
          ->constructor(type, n_construct_properties, construct_properties));

  return G_OBJECT(a11y_settings_manager);
}

static void msd_a11y_settings_manager_dispose(GObject *object) {
  G_OBJECT_CLASS(msd_a11y_settings_manager_parent_class)->dispose(object);
}

static void msd_a11y_settings_manager_class_init(
    MsdA11ySettingsManagerClass *klass) {
...
static void msd_a11y_settings_manager_init(MsdA11ySettingsManager *manager) {
