...
static const gchar *indicator_on_icon_names[] = {
    "kbd-scrolllock-on", "kbd-numlock-on", "kbd-capslock-on"};

static const gchar *indicator_off_icon_names[] = {
    "kbd-scrolllock-off", "kbd-numlock-off", "kbd-capslock-off"};
...
static void activation_error(void) {
  char const *vendor =
      ServerVendor(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()));
  int release = VendorRelease(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()));
  GtkWidget *dialog;

  /* VNC viewers will not work, do not barrage them with warnings */
  if (NULL != vendor && NULL != strstr(vendor, "VNC")) return;
  ...
  dialog = gtk_message_dialog_new_with_markup(
      ...
      _("Error activating XKB configuration.\n"
        ...
        " • The result of <b>%s</b>\n"
        " • The result of <b>%s</b>"),
      vendor, release, "xprop -root | grep XKB",
...
static void apply_desktop_settings(void) {
  ...
  for (i = G_N_ELEMENTS(indicator_icons); --i >= 0;) {
    gtk_status_icon_set_visible(indicator_icons[i], show_leds);
  }
}
...
static void popup_menu_show_layout(void) {
  GtkWidget *dialog;
  XklEngine *engine =
  ...
  gpointer p =
      g_hash_table_lookup(preview_dialogs, GINT_TO_POINTER(xkl_state->group));
  gchar **group_names = matekbd_status_get_group_names();

  if (xkl_state->group < 0 || xkl_state->group >= g_strv_length(group_names)) {
    return;
  }
  ...
  g_signal_connect(dialog, "destroy", G_CALLBACK(show_layout_destroy),
                   GINT_TO_POINTER(xkl_state->group));
  g_hash_table_insert(preview_dialogs, GINT_TO_POINTER(xkl_state->group),
                      dialog);
}
...
static void popup_menu_set_group(GtkMenuItem *item, gpointer param) {
  ...
  if (cur != (Window)NULL) {
    ...
    xkl_engine_save_state(engine, xkl_engine_get_current_window(engine), &st);
    /*    XSetInputFocus(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()), cur,
     * RevertToNone, CurrentTime );*/
  } else {
    xkl_debug(150, "??? Enforcing the state %d for unknown window\n", st.group);
...
static void status_icon_popup_menu_cb(GtkStatusIcon *status_icon, guint button,
  ...
  gtk_menu_item_set_submenu(GTK_MENU_ITEM(item), GTK_WIDGET(groups_menu));
  ...
  for (i = 0; *current_name; i++, current_name++) {
    ...
    if (image_file == NULL) {
      ...
      GdkPixbuf *pixbuf =
          gdk_pixbuf_new_from_file_at_size(image_file, 24, 24, NULL);
      GtkWidget *img = gtk_image_new_from_pixbuf(pixbuf);
      item = gtk_image_menu_item_new_with_label(*current_name);
      gtk_widget_show(img);
    ...
    g_signal_connect(item, "activate", G_CALLBACK(popup_menu_set_group),
                     GINT_TO_POINTER(i));
  }

  gtk_menu_popup(popup_menu, NULL, NULL, gtk_status_icon_position_menu,
                 (gpointer)status_icon, button, time);
}

static void show_hide_icon(void) {
  if (g_strv_length(current_kbd_config.layouts_variants) > 1) {
    if (icon == NULL) {
      gboolean disable =
          g_settings_get_boolean(settings_desktop, DISABLE_INDICATOR_KEY);
      if (disable) return;
      ...
      g_signal_connect(icon, "popup-menu",
                       G_CALLBACK(status_icon_popup_menu_cb), NULL);
    }
  } else {
    if (icon != NULL) {
...
static gboolean try_activating_xkb_config_if_new(
    MatekbdKeyboardConfig *current_sys_kbd_config) {
  /* Activate - only if different! */
  if (!matekbd_keyboard_config_equals(&current_kbd_config,
...
static void apply_xkb_settings(void) {
  ...
  matekbd_keyboard_config_load_from_x_current(&current_sys_kbd_config, NULL);
...
static void msd_keyboard_xkb_analyze_sysconfig(void) {
  if (!inited_ok) return;
...
void msd_keyboard_xkb_set_post_activation_callback(PostActivationCallback fun,
                                                   void *user_data) {
  pa_callback = fun;
  pa_callback_user_data = user_data;
}

static GdkFilterReturn msd_keyboard_xkb_evt_filter(GdkXEvent *xev,
                                                   GdkEvent *event,
                                                   gpointer data) {
  (void)data;
...
static void msd_keyboard_update_indicator_icons(void) {
  Bool state;
  int new_state, i;
  Display *display = GDK_DISPLAY_XDISPLAY(gdk_display_get_default());
  XkbGetNamedIndicator(display, caps_lock, NULL, &state, NULL, NULL);
  new_state = state ? 1 : 0;
  XkbGetNamedIndicator(display, num_lock, NULL, &state, NULL, NULL);
  new_state <<= 1;
  new_state |= (state ? 1 : 0);
  XkbGetNamedIndicator(display, scroll_lock, NULL, &state, NULL, NULL);
  new_state <<= 1;
  new_state |= (state ? 1 : 0);
...
void msd_keyboard_xkb_init(MsdKeyboardManager *kbd_manager) {
  int i;
  Display *display = GDK_DISPLAY_XDISPLAY(gdk_display_get_default());
  ...
  if (xkl_engine) {
    ...
    matekbd_desktop_config_start_listen(
        &current_desktop_config, G_CALLBACK(apply_desktop_settings_cb), NULL);

    matekbd_keyboard_config_start_listen(
        &current_kbd_config, G_CALLBACK(apply_xkb_settings_cb), NULL);

    g_signal_connect(settings_desktop, "changed",
                     G_CALLBACK(apply_desktop_settings_cb), NULL);
    g_signal_connect(settings_kbd, "changed", G_CALLBACK(apply_xkb_settings_cb),
                     NULL);
    ...
    if (xkl_engine_get_features(xkl_engine) & XKLF_DEVICE_DISCOVERY)
      g_signal_connect(xkl_engine, "X-new-device",
                       G_CALLBACK(msd_keyboard_new_device), NULL);
    g_signal_connect(xkl_engine, "X-state-changed",
                     G_CALLBACK(msd_keyboard_state_changed), NULL);
