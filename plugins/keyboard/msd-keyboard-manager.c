...
G_DEFINE_TYPE_WITH_PRIVATE(MsdKeyboardManager, msd_keyboard_manager,
                           G_TYPE_OBJECT)
...
static void numlock_xkb_init(MsdKeyboardManager *manager) {
  Display *dpy = GDK_DISPLAY_XDISPLAY(gdk_display_get_default());
  gboolean have_xkb;
  int opcode, error_base, major, minor;

  have_xkb = XkbQueryExtension(dpy, &opcode, &manager->priv->xkb_event_base,
                               &error_base, &major, &minor) &&
             XkbUseExtension(dpy, &major, &minor);

  if (have_xkb) {
    XkbSelectEventDetails(dpy, XkbUseCoreKbd, XkbStateNotifyMask,
...
static void numlock_set_xkb_state(NumLockState new_state) {
  ...
  Display *dpy = GDK_DISPLAY_XDISPLAY(gdk_display_get_default());
  if (new_state != NUMLOCK_STATE_ON && new_state != NUMLOCK_STATE_OFF) return;
...
static void numlock_set_settings_state(GSettings *settings,
...
static GdkFilterReturn numlock_xkb_callback(GdkXEvent *xev_, GdkEvent *gdkev_,
...
static void numlock_install_xkb_callback(MsdKeyboardManager *manager) {
  if (!manager->priv->have_xkb) return;

  gdk_window_add_filter(NULL, numlock_xkb_callback,
                        GINT_TO_POINTER(manager->priv->xkb_event_base));
}

#endif /* HAVE_X11_EXTENSIONS_XKB_H */

static void apply_settings(GSettings *settings, gchar *key,
...
#ifdef HAVE_X11_EXTENSIONS_XKB_H
    rate_set = xkb_set_keyboard_autorepeat_rate(
        g_settings_get_int(settings, KEY_DELAY),
        g_settings_get_int(settings, KEY_RATE));
#endif /* HAVE_X11_EXTENSIONS_XKB_H */
    if (!rate_set)
      g_warning(
          "Neither XKeyboard not Xfree86's keyboard extensions are available,\n"
...
static gboolean start_keyboard_idle_cb(MsdKeyboardManager *manager) {
...
void msd_keyboard_manager_stop(MsdKeyboardManager *manager) {
  ...
  if (p->have_xkb) {
    gdk_window_remove_filter(NULL, numlock_xkb_callback,
...
static void msd_keyboard_manager_init(MsdKeyboardManager *manager) {
