...
gboolean egg_accelerator_parse_virtual(
    const gchar *accelerator, guint *accelerator_key, guint **accelerator_codes,
  ...
  if (accelerator_codes) *accelerator_codes = NULL;
    ...
    if (*accelerator == '<') {
      ...
      if (keyval == 0) {
        ...
        if (!gdk_keymap_get_entries_for_keyval(
                gdk_keymap_get_for_display(gdk_display_get_default()), keyval,
                &keys, &n_keys)) {
          bad_keyval = TRUE;
        } else {
          *accelerator_codes = g_new0(guint, n_keys + 1);
          ...
          for (i = 0, j = 0; i < n_keys; ++i) {
            if (keys[i].level == 0) (*accelerator_codes)[j++] = keys[i].keycode;
          }
