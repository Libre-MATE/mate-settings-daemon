...
static void setup_modifiers(void) {
  if (msd_used_mods == 0 || msd_ignored_mods == 0) {
    GdkModifierType dynmods;

    /* default modifiers */
    msd_ignored_mods = 0x2000 /*Xkb modifier*/ | GDK_LOCK_MASK | GDK_HYPER_MASK;
    msd_used_mods = GDK_SHIFT_MASK | GDK_CONTROL_MASK | GDK_MOD1_MASK |
    ...
    egg_keymap_resolve_virtual_modifiers(
        ...
        EGG_VIRTUAL_NUM_LOCK_MASK, &dynmods);
...
static void grab_key_real(guint keycode, GdkWindow *root, gboolean grab,
                          int mask) {
  if (grab) {
    XGrabKey(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()), keycode, mask,
             GDK_WINDOW_XID(root), True, GrabModeAsync, GrabModeAsync);
  } else {
    XUngrabKey(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()), keycode, mask,
               GDK_WINDOW_XID(root));
  }
}
  ...
  for (i = 0; i < uppervalue; ++i) {
    ...
    for (l = screens; l; l = l->next) {
      ...
      for (code = key->keycodes; *code; ++code) {
        grab_key_real(*code, gdk_screen_get_root_window(screen), grab,
                      result | key->state);
      }
    }
  }
...
static gboolean have_xkb(Display *dpy) {
  ...
  if (have_xkb == -1) {
    ...
    have_xkb = XkbQueryExtension(dpy, &opcode, &xkb_event_base, &error_base,
                                 &major, &minor) &&
...
gboolean match_key(Key *key, XEvent *event) {
  ...
  if (gdk_keymap_translate_keyboard_state(
          gdk_keymap_get_for_display(gdk_display_get_default()),
          event->xkey.keycode, event->xkey.state, group, &keyval, NULL, NULL,
          &consumed)) {
    guint lower, upper;
    ...
    if (lower == key->keysym) consumed &= ~GDK_SHIFT_MASK;
