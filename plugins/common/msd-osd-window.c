...
/* This is our draw-event handler when the window is *not* in a compositing
 * manager. We just draw a rectangular frame by hand.  We do this with hardcoded
 * drawing code, instead of GtkFrame, to avoid changing the window's internal
 * widget hierarchy:  in either case (composited or non-composited), callers can
 * assume that this works identically to a GtkWindow without any intermediate
 * widgetry.
 */
