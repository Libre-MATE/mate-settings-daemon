...
gboolean cc_rfkill_glib_send_event_finish(CcRfkillGlib *rfkill,
  ...
  g_return_val_if_fail(g_async_result_is_tagged(res, cc_rfkill_glib_send_event),
                       FALSE);

  return (g_task_propagate_int(G_TASK(res), error) >= 0);
}

static void write_done_cb(GObject *source_object, GAsyncResult *res,
                          gpointer user_data) {
...
void cc_rfkill_glib_send_event(CcRfkillGlib *rfkill, struct rfkill_event *event,
  ...
  g_output_stream_write_async(rfkill->priv->stream, event,
                              sizeof(struct rfkill_event), G_PRIORITY_DEFAULT,
                              cancellable, write_done_cb, g_object_ref(task));
}
...
gboolean cc_rfkill_glib_send_change_all_event_finish(CcRfkillGlib *rfkill,
  ...
  g_return_val_if_fail(
      g_async_result_is_tagged(res, cc_rfkill_glib_send_change_all_event),
      FALSE);

  return g_task_propagate_boolean(G_TASK(res), error);
}

static void write_change_all_again_done_cb(GObject *source_object,
                                           GAsyncResult *res,
                                           gpointer user_data) {
...
static gboolean write_change_all_timeout_cb(CcRfkillGlib *rfkill) {
  g_assert(rfkill->priv->event);

  g_debug("Sending second RFKILL_OP_CHANGE_ALL timed out");

  g_task_return_new_error(rfkill->priv->task, G_IO_ERROR, G_IO_ERROR_TIMED_OUT,
...
static void write_change_all_done_cb(GObject *source_object, GAsyncResult *res,
                                     gpointer user_data) {
  CcRfkillGlib *rfkill = user_data;
  g_autoptr(GError) error = NULL;
  gssize ret;
  ...
  rfkill->priv->change_all_timeout_id = g_timeout_add(
      CHANGE_ALL_TIMEOUT, (GSourceFunc)write_change_all_timeout_cb, rfkill);
...
static void print_event(struct rfkill_event *event) {
  g_debug("RFKILL event: idx %u type %u (%s) op %u (%s) soft %u hard %u",
          event->idx, event->type, type_to_string(event->type), event->op,
          op_to_string(event->op), event->soft, event->hard);
}

static gboolean got_change_event(GList *events) {
  GList *l;
  ...
  for (l = events; l != NULL; l = l->next) {
    ...
    if (event->op == RFKILL_OP_CHANGE) return TRUE;
  }

  return FALSE;
}

static void emit_changed_signal_and_free(CcRfkillGlib *rfkill, GList *events) {
  if (events == NULL) return;

  g_signal_emit(G_OBJECT(rfkill), signals[CHANGED], 0, events);

  if (rfkill->priv->change_all_timeout_id > 0 && got_change_event(events)) {
    g_debug(
        "Received a change event after a RFKILL_OP_CHANGE_ALL event, "
        "re-sending RFKILL_OP_CHANGE_ALL");

    g_output_stream_write_async(rfkill->priv->stream, rfkill->priv->event,
                                sizeof(struct rfkill_event), G_PRIORITY_DEFAULT,
                                rfkill->priv->cancellable,
...
static gboolean event_cb(GIOChannel *source, GIOCondition condition,
                         CcRfkillGlib *rfkill) {
  GList *events;
  ...
  if (condition & G_IO_IN) {
    ...
    status = g_io_channel_read_chars(source, (char *)&event, sizeof(event),
                                     &read, NULL);
...
static void cc_rfkill_glib_init(CcRfkillGlib *rfkill) {
...
int cc_rfkill_glib_open(CcRfkillGlib *rfkill) {
  ...
  if (fd < 0) {
    if (errno == EACCES)
      g_warning(
          "Could not open RFKILL control device, please verify your "
          "installation");
    return fd;
  }
  ...
  while (1) {
    ...
    if (event.op != RFKILL_OP_ADD) continue;

    g_debug("Read killswitch of type '%s' (idx=%d): soft %d hard %d",
            type_to_string(event.type), event.idx, event.soft, event.hard);
  ...
  priv->watch_id = g_io_add_watch(priv->channel, G_IO_IN | G_IO_HUP | G_IO_ERR,
                                  (GIOFunc)event_cb, rfkill);
...
static void cc_rfkill_glib_finalize(GObject *object) {
  CcRfkillGlib *rfkill;
  CcRfkillGlibPrivate *priv;
  ...
  if (priv->change_all_timeout_id > 0) write_change_all_timeout_cb(rfkill);
...
static void cc_rfkill_glib_class_init(CcRfkillGlibClass *klass) {
  ...
  signals[CHANGED] =
      g_signal_new("changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(CcRfkillGlibClass, changed), NULL, NULL,
