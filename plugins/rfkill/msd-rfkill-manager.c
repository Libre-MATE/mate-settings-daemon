...
static void msd_rfkill_manager_init(MsdRfkillManager *manager) {
  manager->priv = msd_rfkill_manager_get_instance_private(manager);
}

static gboolean engine_get_airplane_mode_helper(GHashTable *killswitches) {
  GHashTableIter iter;
  gpointer key, value;

  if (g_hash_table_size(killswitches) == 0) return FALSE;
  ...
  while (g_hash_table_iter_next(&iter, &key, &value)) {
    ...
    if (state == RFKILL_STATE_UNBLOCKED) return FALSE;
  }

  return TRUE;
}

static gboolean engine_get_bluetooth_airplane_mode(MsdRfkillManager *manager) {
  return engine_get_airplane_mode_helper(manager->priv->bt_killswitches);
}

static gboolean engine_get_bluetooth_hardware_airplane_mode(
    MsdRfkillManager *manager) {
  GHashTableIter iter;
  gpointer key, value;

  /* If we have no killswitches, hw airplane mode is off. */
  if (g_hash_table_size(manager->priv->bt_killswitches) == 0) return FALSE;
...
static gboolean engine_get_has_bluetooth_airplane_mode(
    MsdRfkillManager *manager) {
  return (g_hash_table_size(manager->priv->bt_killswitches) > 0);
}

static gboolean engine_get_airplane_mode(MsdRfkillManager *manager) {
  ...
  return engine_get_airplane_mode_helper(manager->priv->killswitches) &&
         !manager->priv->wwan_enabled;
}

static gboolean engine_get_hardware_airplane_mode(MsdRfkillManager *manager) {
  GHashTableIter iter;
  gpointer key, value;

  /* If we have no killswitches, hw airplane mode is off. */
  if (g_hash_table_size(manager->priv->killswitches) == 0) return FALSE;
...
static gboolean engine_get_has_airplane_mode(MsdRfkillManager *manager) {
  return (g_hash_table_size(manager->priv->killswitches) > 0) ||
         manager->priv->wwan_interesting;
}

static gboolean engine_get_should_show_airplane_mode(
    MsdRfkillManager *manager) {
...
static void engine_properties_changed(MsdRfkillManager *manager) {
  GVariantBuilder props_builder;
  GVariant *props_changed = NULL;

  /* not yet connected to the session bus */
  if (manager->priv->connection == NULL) return;

  g_variant_builder_init(&props_builder, G_VARIANT_TYPE("a{sv}"));

  g_variant_builder_add(
      &props_builder, "{sv}", "AirplaneMode",
      g_variant_new_boolean(engine_get_airplane_mode(manager)));
  g_variant_builder_add(
      &props_builder, "{sv}", "HardwareAirplaneMode",
      g_variant_new_boolean(engine_get_hardware_airplane_mode(manager)));
  g_variant_builder_add(
      &props_builder, "{sv}", "HasAirplaneMode",
      g_variant_new_boolean(engine_get_has_airplane_mode(manager)));
  g_variant_builder_add(
      &props_builder, "{sv}", "ShouldShowAirplaneMode",
      g_variant_new_boolean(engine_get_should_show_airplane_mode(manager)));
  g_variant_builder_add(
      &props_builder, "{sv}", "BluetoothAirplaneMode",
      g_variant_new_boolean(engine_get_bluetooth_airplane_mode(manager)));
  g_variant_builder_add(
      &props_builder, "{sv}", "BluetoothHardwareAirplaneMode",
      g_variant_new_boolean(
          engine_get_bluetooth_hardware_airplane_mode(manager)));
  g_variant_builder_add(
      &props_builder, "{sv}", "BluetoothHasAirplaneMode",
  ...
  g_dbus_connection_emit_signal(manager->priv->connection, NULL,
      ...
      case RFKILL_OP_DEL:
        ...
        if (event->type == RFKILL_TYPE_BLUETOOTH)
          g_hash_table_remove(manager->priv->bt_killswitches,
                              GINT_TO_POINTER(event->idx));
        g_debug("Removed %srfkill with ID %d",
                event->type == RFKILL_TYPE_BLUETOOTH ? "Bluetooth " : "",
                event->idx);
        break;
    }
...
static void rfkill_set_cb(GObject *source_object, GAsyncResult *res,
                          gpointer user_data) {
  gboolean ret;
  GError *error = NULL;

  ret = cc_rfkill_glib_send_change_all_event_finish(
      CC_RFKILL_GLIB(source_object), res, &error);
  if (!ret) {
...
static void set_wwan_complete(GObject *object, GAsyncResult *result,
                              gpointer user_data) {
  GError *error;
  GVariant *variant;
...
static gboolean engine_set_bluetooth_airplane_mode(MsdRfkillManager *manager,
                                                   gboolean enable) {
  cc_rfkill_glib_send_change_all_event(
      manager->priv->rfkill, RFKILL_TYPE_BLUETOOTH, enable,
      manager->priv->cancellable, rfkill_set_cb, manager);

  return TRUE;
}

static gboolean engine_set_airplane_mode(MsdRfkillManager *manager,
                                         gboolean enable) {
  cc_rfkill_glib_send_change_all_event(manager->priv->rfkill, RFKILL_TYPE_ALL,
                                       enable, manager->priv->cancellable,
                                       rfkill_set_cb, manager);
  ...
  if (manager->priv->nm_client) {
    g_dbus_proxy_call(
        manager->priv->nm_client, "org.freedesktop.DBus.Properties.Set",
        g_variant_new("(ssv)", "org.freedesktop.NetworkManager", "WwanEnabled",
                      g_variant_new_boolean(!enable)),
        G_DBUS_CALL_FLAGS_NONE, -1, /* timeout */
        manager->priv->cancellable, set_wwan_complete, NULL);
  }
...
static const GDBusInterfaceVTable interface_vtable = {
    NULL, handle_get_property, handle_set_property, {0}};

static void on_bus_gotten(GObject *source_object, GAsyncResult *res,
                          MsdRfkillManager *manager) {
  GDBusConnection *connection;
  GError *error = NULL;
  ...
  g_dbus_connection_register_object(
      connection, MSD_RFKILL_DBUS_PATH,
      manager->priv->introspection_data->interfaces[0], &interface_vtable,
      manager, NULL, NULL);

  manager->priv->name_id = g_bus_own_name_on_connection(
      connection, MSD_RFKILL_DBUS_NAME, G_BUS_NAME_OWNER_FLAGS_NONE, NULL, NULL,
      NULL, NULL);
}

static void sync_wwan_enabled(MsdRfkillManager *manager) {
  GVariant *property;

  property =
      g_dbus_proxy_get_cached_property(manager->priv->nm_client, "WwanEnabled");
...
static void on_nm_proxy_gotten(GObject *source, GAsyncResult *result,
                               gpointer user_data) {
  MsdRfkillManager *manager = user_data;
  GDBusProxy *proxy;
  GError *error;
  ...
  g_signal_connect(manager->priv->nm_client, "g-signal", G_CALLBACK(nm_signal),
                   manager);
  sync_wwan_enabled(manager);
...
static void on_mm_proxy_gotten(GObject *source, GAsyncResult *result,
                               gpointer user_data) {
  MsdRfkillManager *manager = user_data;
...
gboolean msd_rfkill_manager_start(MsdRfkillManager *manager, GError **error) {
  ...
  manager->priv->introspection_data =
  ...
  g_dbus_proxy_new_for_bus(
      G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_NONE, NULL, /* g-interface-info */
      "org.freedesktop.NetworkManager", "/org/freedesktop/NetworkManager",
      "org.freedesktop.NetworkManager", manager->priv->cancellable,
      on_nm_proxy_gotten, g_object_ref(manager));

  g_dbus_object_manager_client_new_for_bus(
      G_BUS_TYPE_SYSTEM, G_DBUS_OBJECT_MANAGER_CLIENT_FLAGS_DO_NOT_AUTO_START,
      "org.freedesktop.ModemManager1", "/org/freedesktop/ModemManager1", NULL,
      NULL, NULL, /* get_proxy_type and closure */
      manager->priv->cancellable, on_mm_proxy_gotten, g_object_ref(manager));

  /* Start process of owning a D-Bus name */
  g_bus_get(G_BUS_TYPE_SESSION, manager->priv->cancellable,
