...
static void locate_pointer_paint(MsdLocatePointerData *data, cairo_t *cr,
                                 gboolean composited) {
  GdkRGBA color;
  ...
  gtk_style_context_get_background_color(
      style, gtk_style_context_get_state(style), &color);
  if (color.alpha == 0.) {
  ...
  for (i = 0; i <= N_CIRCLES; i++) {
    ...
    if (circle_progress >= 1.) continue;

    if (composited) {
      cairo_set_source_rgba(cr, color.red, color.green, color.blue,
                            1 - circle_progress);
      cairo_arc(cr, width / 2, height / 2, circle_progress * width / 2, 0,
                2 * G_PI);
...
static void update_shape(MsdLocatePointerData *data) {
  cairo_t *cr;
  cairo_surface_t *mask;
  cairo_region_t *region;

  mask = gdk_window_create_similar_image_surface(data->window, CAIRO_FORMAT_A1,
...
static void timeline_frame_cb(MsdTimeline *timeline, gdouble progress,
  ...
  gtk_window_move(data->widget, cursor_x - WINDOW_SIZE / 2,
                  cursor_y - WINDOW_SIZE / 2);
}

static void set_transparent_shape(GdkWindow *window) {
  cairo_t *cr;
  cairo_surface_t *mask;
  cairo_region_t *region;

  mask = gdk_window_create_similar_image_surface(window, CAIRO_FORMAT_A1,
                                                 WINDOW_SIZE, WINDOW_SIZE, 0);
  cr = cairo_create(mask);
...
static void unset_transparent_shape(GdkWindow *window) {
  gdk_window_shape_combine_region(window, NULL, 0, 0);
}

static void composited_changed(GdkScreen *screen, MsdLocatePointerData *data) {
  if (gdk_screen_is_composited(screen)) {
...
static void locate_pointer_realize_cb(GtkWidget *widget,
  ...
  data->window = gdk_window_new(gdk_screen_get_root_window(screen), &attributes,
                                attributes_mask);

  gtk_widget_set_window(GTK_WIDGET(data->widget), data->window);
...
static MsdLocatePointerData *msd_locate_pointer_data_new(void) {
  MsdLocatePointerData *data;
  ...
  g_signal_connect(data->timeline, "frame", G_CALLBACK(timeline_frame_cb),
                   data);
  g_signal_connect(data->timeline, "finished", G_CALLBACK(timeline_finished_cb),
                   data);

  return data;
}

static void move_locate_pointer_window(MsdLocatePointerData *data,
                                       GdkDisplay *display) {
  GdkSeat *seat;
  GdkDevice *pointer;
  gint cursor_x, cursor_y;
  ...
  gtk_window_move(data->widget, cursor_x - WINDOW_SIZE / 2,
                  cursor_y - WINDOW_SIZE / 2);
  gtk_window_resize(data->widget, WINDOW_SIZE, WINDOW_SIZE);
...
void msd_locate_pointer(GdkDisplay *display) {
  ...
  g_signal_connect(screen, "composited-changed", G_CALLBACK(composited_changed),
                   data);
...
/* Owen magic */
static GdkFilterReturn event_filter(GdkXEvent *gdkxevent, GdkEvent *event,
                                    gpointer user_data) {
  ...
  if (xevent->xany.type == KeyPress || xevent->xany.type == KeyRelease) {
    ...
    gdk_keymap_translate_keyboard_state(
        gdk_keymap_get_for_display(display), xevent->xkey.keycode,
        xevent->xkey.state, group, &keyval, NULL, NULL, NULL);

    if (keyval == GDK_KEY_Control_L || keyval == GDK_KEY_Control_R) {
      if (xevent->xany.type == KeyRelease) {
        XAllowEvents(xevent->xany.display, AsyncKeyboard, xevent->xkey.time);
        msd_locate_pointer(display);
      } else {
        XAllowEvents(xevent->xany.display, SyncKeyboard, xevent->xkey.time);
      }
    } else {
      XAllowEvents(xevent->xany.display, ReplayKeyboard, xevent->xkey.time);
      XUngrabButton(xevent->xany.display, AnyButton, AnyModifier,
                    xevent->xany.window);
      XUngrabKeyboard(xevent->xany.display, xevent->xkey.time);
    }
  } else if (xevent->xany.type == ButtonPress) {
    XAllowEvents(xevent->xany.display, ReplayPointer, xevent->xbutton.time);
    XUngrabButton(xevent->xany.display, AnyButton, AnyModifier,
                  xevent->xany.window);
    XUngrabKeyboard(xevent->xany.display, xevent->xbutton.time);
  }

  return GDK_FILTER_CONTINUE;
}

static void set_locate_pointer(void) {
  GdkKeymapKey *keys;
  GdkDisplay *display;
  GdkScreen *screen;
  int n_keys;
  ...
  for (i = 0; i < G_N_ELEMENTS(keyvals); ++i) {
    if (gdk_keymap_get_entries_for_keyval(gdk_keymap_get_for_display(display),
      ...
      for (j = 0; j < n_keys; ++j) {
        ...
        XGrabKey(GDK_DISPLAY_XDISPLAY(display), keys[j].keycode, 0, xroot,
                 False, GrabModeAsync, GrabModeSync);
        XGrabKey(GDK_DISPLAY_XDISPLAY(display), keys[j].keycode, LockMask,
                 xroot, False, GrabModeAsync, GrabModeSync);
        XGrabKey(GDK_DISPLAY_XDISPLAY(display), keys[j].keycode, Mod2Mask,
                 xroot, False, GrabModeAsync, GrabModeSync);
        XGrabKey(GDK_DISPLAY_XDISPLAY(display), keys[j].keycode, Mod4Mask,
                 xroot, False, GrabModeAsync, GrabModeSync);
      }
      g_free(keys);
    }
  ...
  if (has_entries) {
    gdk_window_add_filter(gdk_screen_get_root_window(screen),
                          (GdkFilterFunc)event_filter, display);
  }
}
