...
static void msd_timeline_set_property(GObject *object, guint prop_id,
                                      const GValue *value, GParamSpec *pspec);
static void msd_timeline_get_property(GObject *object, guint prop_id,
                                      GValue *value, GParamSpec *pspec);
static void msd_timeline_finalize(GObject *object);

G_DEFINE_TYPE_WITH_PRIVATE(MsdTimeline, msd_timeline, G_TYPE_OBJECT)

GType msd_timeline_direction_get_type(void) {
  static GType type = 0;

  if (G_UNLIKELY(type == 0)) {
    static const GEnumValue values[] = {
        {MSD_TIMELINE_DIRECTION_FORWARD, "MSD_TIMELINE_DIRECTION_FORWARD",
         "forward"},
        {MSD_TIMELINE_DIRECTION_BACKWARD, "MSD_TIMELINE_DIRECTION_BACKWARD",
         "backward"},
        {0, NULL, NULL}};

    type = g_enum_register_static(
        g_intern_static_string("MsdTimelineDirection"), values);
  }

  return type;
}

GType msd_timeline_progress_type_get_type(void) {
  static GType type = 0;

  if (G_UNLIKELY(type == 0)) {
    static const GEnumValue values[] = {
        {MSD_TIMELINE_PROGRESS_LINEAR, "MSD_TIMELINE_PROGRESS_LINEAR",
         "linear"},
        {MSD_TIMELINE_PROGRESS_SINUSOIDAL, "MSD_TIMELINE_PROGRESS_SINUSOIDAL",
         "sinusoidal"},
        {MSD_TIMELINE_PROGRESS_EXPONENTIAL, "MSD_TIMELINE_PROGRESS_EXPONENTIAL",
         "exponential"},
        {0, NULL, NULL}};

    type = g_enum_register_static(
        g_intern_static_string("MsdTimelineProgressType"), values);
  }

  return type;
}

static void msd_timeline_class_init(MsdTimelineClass *class) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum(
          ...
          MSD_TYPE_TIMELINE_DIRECTION, MSD_TIMELINE_DIRECTION_FORWARD,
          G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_DIRECTION,
      g_param_spec_enum("progress-type", "Progress type",
                        "Type of progress through the timeline",
                        MSD_TYPE_TIMELINE_PROGRESS_TYPE,
                        MSD_TIMELINE_PROGRESS_LINEAR, G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_SCREEN,
      g_param_spec_object("screen", "Screen", "Screen to get the settings from",
                          GDK_TYPE_SCREEN, G_PARAM_READWRITE));

  signals[STARTED] = g_signal_new(
      "started", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(MsdTimelineClass, started), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[PAUSED] =
      g_signal_new("paused", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(MsdTimelineClass, paused), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[FINISHED] = g_signal_new(
      "finished", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(MsdTimelineClass, finished), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[FRAME] = g_signal_new(
      "frame", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(MsdTimelineClass, frame), NULL, NULL,
      g_cclosure_marshal_VOID__DOUBLE, G_TYPE_NONE, 1, G_TYPE_DOUBLE);
}

static void msd_timeline_init(MsdTimeline *timeline) {
...
static void msd_timeline_set_property(GObject *object, guint prop_id,
  ...
  switch (prop_id) {
    ...
    case PROP_DIRECTION:
      ...
      break;
    case PROP_SCREEN:
      msd_timeline_set_screen(timeline, GDK_SCREEN(g_value_get_object(value)));
      break;
...
/* Sinusoidal progress */
static gdouble sinusoidal_progress(gdouble progress) {
  return (sinf((progress * G_PI) / 2));
}

static gdouble exponential_progress(gdouble progress) {
  return progress * progress;
}

static MsdTimelineProgressFunc progress_type_to_func(
...
static gboolean msd_timeline_run_frame(MsdTimeline *timeline,
  ...
  g_signal_emit(timeline, signals[FRAME], 0, CLAMP(progress, 0.0, 1.0));
...
static gboolean msd_timeline_frame_idle_func(MsdTimeline *timeline) {
...
MsdTimeline *msd_timeline_new(guint duration) {
  return g_object_new(MSD_TYPE_TIMELINE, "duration", duration, NULL);
}

MsdTimeline *msd_timeline_new_for_screen(guint duration, GdkScreen *screen) {
  return g_object_new(MSD_TYPE_TIMELINE, "duration", duration, "screen", screen,
                      NULL);
}
...
void msd_timeline_start(MsdTimeline *timeline) {
  ...
  if (enable_animations) {
    if (!priv->source_id) {
      ...
      priv->source_id = gdk_threads_add_timeout(
          FRAME_INTERVAL(priv->fps), (GSourceFunc)msd_timeline_frame_idle_func,
          timeline);
    }
  } else {
 ...
 **/
void msd_timeline_set_fps(MsdTimeline *timeline, guint fps) {
  MsdTimelinePrivate *priv;
  ...
  if (msd_timeline_is_running(timeline)) {
    g_source_remove(priv->source_id);
    priv->source_id =
        gdk_threads_add_timeout(FRAME_INTERVAL(priv->fps),
                                (GSourceFunc)msd_timeline_run_frame, timeline);
  }
 ...
 **/
void msd_timeline_set_loop(MsdTimeline *timeline, gboolean loop) {
  MsdTimelinePrivate *priv;
...
void msd_timeline_set_duration(MsdTimeline *timeline, guint duration) {
  MsdTimelinePrivate *priv;
...
void msd_timeline_set_screen(MsdTimeline *timeline, GdkScreen *screen) {
  MsdTimelinePrivate *priv;
  ...
  if (priv->screen) g_object_unref(priv->screen);
...
MsdTimelineProgressType msd_timeline_get_progress_type(MsdTimeline *timeline) {
  ...
  if (priv->progress_func) return MSD_TIMELINE_PROGRESS_LINEAR;
