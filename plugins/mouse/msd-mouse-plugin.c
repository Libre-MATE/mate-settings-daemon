...
static void msd_mouse_plugin_init(MsdMousePlugin *plugin) {
...
static void msd_mouse_plugin_finalize(GObject *object) {
...
static void impl_activate(MateSettingsPlugin *plugin) {
  ...
  error = NULL;
  res =
      msd_mouse_manager_start(MSD_MOUSE_PLUGIN(plugin)->priv->manager, &error);
  if (!res) {
...
static void msd_mouse_plugin_class_init(MsdMousePluginClass *klass) {
...
static void msd_mouse_plugin_class_finalize(MsdMousePluginClass *klass) {}
