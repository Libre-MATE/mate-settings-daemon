...
G_DEFINE_TYPE_WITH_PRIVATE(MsdA11yPreferencesDialog,
                           msd_a11y_preferences_dialog, GTK_TYPE_DIALOG)

static void msd_a11y_preferences_dialog_class_init(
    MsdA11yPreferencesDialogClass *klass) {
...
static gboolean config_get_bool(GSettings *settings, const char *key,
...
static gboolean config_get_large_print(gboolean *is_writable) {
  GSettings *settings;
  gboolean ret;
  gdouble x_dpi;
  gdouble u_dpi;
...
static void config_set_large_print(gboolean enabled) {
  GSettings *settings;
...
static gboolean config_get_high_contrast(MsdA11yPreferencesDialog *dialog,
                                         gboolean *is_writable) {
  gboolean ret;
  char *gtk_theme;
  ...
  *is_writable =
      g_settings_is_writable(dialog->priv->settings_interface, KEY_GTK_THEME);
  gtk_theme =
      g_settings_get_string(dialog->priv->settings_interface, KEY_GTK_THEME);
...
static void config_set_high_contrast(MsdA11yPreferencesDialog *dialog,
                                     gboolean enabled) {
  if (enabled) {
    g_settings_set_string(dialog->priv->settings_interface, KEY_GTK_THEME,
                          HIGH_CONTRAST_THEME);
    g_settings_set_string(dialog->priv->settings_interface, KEY_ICON_THEME,
                          HIGH_CONTRAST_THEME);
...
static gboolean config_get_sticky_keys(MsdA11yPreferencesDialog *dialog,
                                       gboolean *is_writable) {
  return config_get_bool(dialog->priv->settings_a11y, KEY_STICKY_KEYS_ENABLED,
                         is_writable);
}

static void config_set_sticky_keys(MsdA11yPreferencesDialog *dialog,
                                   gboolean enabled) {
  g_settings_set_boolean(dialog->priv->settings_a11y, KEY_STICKY_KEYS_ENABLED,
                         enabled);
}

static gboolean config_get_bounce_keys(MsdA11yPreferencesDialog *dialog,
                                       gboolean *is_writable) {
  return config_get_bool(dialog->priv->settings_a11y, KEY_BOUNCE_KEYS_ENABLED,
                         is_writable);
}

static void config_set_bounce_keys(MsdA11yPreferencesDialog *dialog,
                                   gboolean enabled) {
  g_settings_set_boolean(dialog->priv->settings_a11y, KEY_BOUNCE_KEYS_ENABLED,
                         enabled);
}

static gboolean config_get_slow_keys(MsdA11yPreferencesDialog *dialog,
                                     gboolean *is_writable) {
  return config_get_bool(dialog->priv->settings_a11y, KEY_SLOW_KEYS_ENABLED,
                         is_writable);
}

static void config_set_slow_keys(MsdA11yPreferencesDialog *dialog,
                                 gboolean enabled) {
  g_settings_set_boolean(dialog->priv->settings_a11y, KEY_SLOW_KEYS_ENABLED,
                         enabled);
}

#ifdef HAVE_LIBATSPI
static gboolean config_get_capslock_beep(MsdA11yPreferencesDialog *dialog,
                                         gboolean *is_writable) {
  return config_get_bool(dialog->priv->settings_a11y, KEY_CAPSLOCK_BEEP_ENABLED,
                         is_writable);
}

static void config_set_capslock_beep(MsdA11yPreferencesDialog *dialog,
                                     gboolean enabled) {
  g_settings_set_boolean(dialog->priv->settings_a11y, KEY_CAPSLOCK_BEEP_ENABLED,
                         enabled);
}
#endif

static gboolean config_have_at_gsettings_condition(const char *condition) {
  DBusGProxy *sm_proxy;
  ...
  sm_proxy = dbus_g_proxy_new_for_name(connection, SM_DBUS_NAME, SM_DBUS_PATH,
                                       SM_DBUS_INTERFACE);
  if (sm_proxy == NULL) {
    return FALSE;
  }
  ...
  is_handled = FALSE;
  res = dbus_g_proxy_call(sm_proxy, "IsAutostartConditionHandled", &error,
                          G_TYPE_STRING, condition, G_TYPE_INVALID,
                          G_TYPE_BOOLEAN, &is_handled, G_TYPE_INVALID);
  if (!res) {
    g_warning("Unable to call IsAutostartConditionHandled (%s): %s", condition,
              error->message);
  }
...
static gboolean config_get_at_screen_reader(MsdA11yPreferencesDialog *dialog,
                                            gboolean *is_writable) {
  return config_get_bool(dialog->priv->settings_at,
                         KEY_AT_SCREEN_READER_ENABLED, is_writable);
}

static gboolean config_get_at_screen_keyboard(MsdA11yPreferencesDialog *dialog,
                                              gboolean *is_writable) {
  return config_get_bool(dialog->priv->settings_at,
                         KEY_AT_SCREEN_KEYBOARD_ENABLED, is_writable);
}

static gboolean config_get_at_screen_magnifier(MsdA11yPreferencesDialog *dialog,
                                               gboolean *is_writable) {
  return config_get_bool(dialog->priv->settings_at,
                         KEY_AT_SCREEN_MAGNIFIER_ENABLED, is_writable);
}

static void config_set_at_screen_reader(MsdA11yPreferencesDialog *dialog,
                                        gboolean enabled) {
  g_settings_set_boolean(dialog->priv->settings_at,
                         KEY_AT_SCREEN_READER_ENABLED, enabled);
}

static void config_set_at_screen_keyboard(MsdA11yPreferencesDialog *dialog,
                                          gboolean enabled) {
  g_settings_set_boolean(dialog->priv->settings_at,
                         KEY_AT_SCREEN_KEYBOARD_ENABLED, enabled);
}

static void config_set_at_screen_magnifier(MsdA11yPreferencesDialog *dialog,
                                           gboolean enabled) {
  g_settings_set_boolean(dialog->priv->settings_at,
                         KEY_AT_SCREEN_MAGNIFIER_ENABLED, enabled);
}

static void on_sticky_keys_checkbutton_toggled(
    GtkToggleButton *button, MsdA11yPreferencesDialog *dialog) {
  config_set_sticky_keys(dialog, gtk_toggle_button_get_active(button));
}

static void on_bounce_keys_checkbutton_toggled(
    GtkToggleButton *button, MsdA11yPreferencesDialog *dialog) {
  config_set_bounce_keys(dialog, gtk_toggle_button_get_active(button));
}

static void on_slow_keys_checkbutton_toggled(GtkToggleButton *button,
                                             MsdA11yPreferencesDialog *dialog) {
  config_set_slow_keys(dialog, gtk_toggle_button_get_active(button));
}

#ifdef HAVE_LIBATSPI
static void on_capslock_beep_checkbutton_toggled(
    GtkToggleButton *button, MsdA11yPreferencesDialog *dialog) {
  config_set_capslock_beep(dialog, gtk_toggle_button_get_active(button));
}
#endif

static void on_high_contrast_checkbutton_toggled(
    GtkToggleButton *button, MsdA11yPreferencesDialog *dialog) {
  config_set_high_contrast(dialog, gtk_toggle_button_get_active(button));
}

static void on_at_screen_reader_checkbutton_toggled(
    GtkToggleButton *button, MsdA11yPreferencesDialog *dialog) {
  config_set_at_screen_reader(dialog, gtk_toggle_button_get_active(button));
}

static void on_at_screen_keyboard_checkbutton_toggled(
    GtkToggleButton *button, MsdA11yPreferencesDialog *dialog) {
  config_set_at_screen_keyboard(dialog, gtk_toggle_button_get_active(button));
}

static void on_at_screen_magnifier_checkbutton_toggled(
    GtkToggleButton *button, MsdA11yPreferencesDialog *dialog) {
...
static void ui_set_sticky_keys(MsdA11yPreferencesDialog *dialog,
                               gboolean enabled) {
  gboolean active;

  active = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(dialog->priv->sticky_keys_checkbutton));
  if (active != enabled) {
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(dialog->priv->sticky_keys_checkbutton), enabled);
  }
}

static void ui_set_bounce_keys(MsdA11yPreferencesDialog *dialog,
                               gboolean enabled) {
  gboolean active;

  active = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(dialog->priv->bounce_keys_checkbutton));
  if (active != enabled) {
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(dialog->priv->bounce_keys_checkbutton), enabled);
  }
}

static void ui_set_slow_keys(MsdA11yPreferencesDialog *dialog,
                             gboolean enabled) {
  gboolean active;
  ...
  if (active != enabled) {
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(dialog->priv->slow_keys_checkbutton), enabled);
  }
}

#ifdef HAVE_LIBATSPI
static void ui_set_capslock_beep(MsdA11yPreferencesDialog *dialog,
                                 gboolean enabled) {
  gboolean active;

  active = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(dialog->priv->capslock_beep_checkbutton));
  if (active != enabled) {
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(dialog->priv->capslock_beep_checkbutton), enabled);
  }
}
#endif

static void ui_set_high_contrast(MsdA11yPreferencesDialog *dialog,
                                 gboolean enabled) {
  gboolean active;
  ...
  if (active != enabled) {
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(dialog->priv->high_contrast_checkbutton), enabled);
  }
}

static void ui_set_at_screen_reader(MsdA11yPreferencesDialog *dialog,
                                    gboolean enabled) {
  gboolean active;

  active = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(dialog->priv->screen_reader_checkbutton));
  if (active != enabled) {
    gtk_toggle_button_set_active(
...
static void ui_set_at_screen_keyboard(MsdA11yPreferencesDialog *dialog,
                                      gboolean enabled) {
  gboolean active;
  ...
  if (active != enabled) {
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(dialog->priv->screen_keyboard_checkbutton), enabled);
  }
}

static void ui_set_at_screen_magnifier(MsdA11yPreferencesDialog *dialog,
                                       gboolean enabled) {
  gboolean active;

  active = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(dialog->priv->screen_magnifier_checkbutton));
  if (active != enabled) {
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(dialog->priv->screen_magnifier_checkbutton), enabled);
  }
}

static void ui_set_large_print(MsdA11yPreferencesDialog *dialog,
                               gboolean enabled) {
  gboolean active;

  active = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(dialog->priv->large_print_checkbutton));
  if (active != enabled) {
    gtk_toggle_button_set_active(
...
static void key_changed_cb(GSettings *settings, gchar *key,
                           MsdA11yPreferencesDialog *dialog) {
...
static void setup_dialog(MsdA11yPreferencesDialog *dialog,
                         GtkBuilder *builder) {
  GtkWidget *widget;
  gboolean enabled;
  gboolean is_writable;
  ...
  widget =
      GTK_WIDGET(gtk_builder_get_object(builder, "sticky_keys_checkbutton"));
  dialog->priv->sticky_keys_checkbutton = widget;
  g_signal_connect(widget, "toggled",
                   G_CALLBACK(on_sticky_keys_checkbutton_toggled), dialog);
  ...
  widget =
      GTK_WIDGET(gtk_builder_get_object(builder, "bounce_keys_checkbutton"));
  dialog->priv->bounce_keys_checkbutton = widget;
  g_signal_connect(widget, "toggled",
                   G_CALLBACK(on_bounce_keys_checkbutton_toggled), dialog);
  ...
  g_signal_connect(widget, "toggled",
                   G_CALLBACK(on_slow_keys_checkbutton_toggled), dialog);
  ...
  widget =
      GTK_WIDGET(gtk_builder_get_object(builder, "capslock_beep_checkbutton"));
  ...
  g_signal_connect(widget, "toggled",
                   G_CALLBACK(on_capslock_beep_checkbutton_toggled), dialog);
  ...
  widget =
      GTK_WIDGET(gtk_builder_get_object(builder, "high_contrast_checkbutton"));
  dialog->priv->high_contrast_checkbutton = widget;
  g_signal_connect(widget, "toggled",
                   G_CALLBACK(on_high_contrast_checkbutton_toggled), dialog);
  ...
  widget = GTK_WIDGET(
      gtk_builder_get_object(builder, "at_screen_keyboard_checkbutton"));
  ...
  gtk_widget_set_no_show_all(widget, TRUE);
  if (config_have_at_gsettings_condition("MATE " KEY_AT_SCHEMA
                                         " " KEY_AT_SCREEN_KEYBOARD_ENABLED)) {
  ...
  widget = GTK_WIDGET(
      gtk_builder_get_object(builder, "at_screen_reader_checkbutton"));
  dialog->priv->screen_reader_checkbutton = widget;
  g_signal_connect(widget, "toggled",
                   G_CALLBACK(on_at_screen_reader_checkbutton_toggled), dialog);
  ...
  widget = GTK_WIDGET(
      gtk_builder_get_object(builder, "at_screen_magnifier_checkbutton"));
  ...
  gtk_widget_set_no_show_all(widget, TRUE);
  if (config_have_at_gsettings_condition("MATE " KEY_AT_SCHEMA
                                         " " KEY_AT_SCREEN_MAGNIFIER_ENABLED)) {
    gtk_widget_show_all(widget);
  } else {
    gtk_widget_hide(widget);
  }
  ...
  widget =
      GTK_WIDGET(gtk_builder_get_object(builder, "large_print_checkbutton"));
  dialog->priv->large_print_checkbutton = widget;
  g_signal_connect(widget, "toggled",
                   G_CALLBACK(on_large_print_checkbutton_toggled), dialog);
  ...
  g_signal_connect(dialog->priv->settings_a11y, "changed",
                   G_CALLBACK(key_changed_cb), dialog);
  g_signal_connect(dialog->priv->settings_at, "changed",
                   G_CALLBACK(key_changed_cb), dialog);
}

static void msd_a11y_preferences_dialog_init(MsdA11yPreferencesDialog *dialog) {
  ...
  if (gtk_builder_add_objects_from_file(builder, ui_file_path, objects,
    ...
    widget = GTK_WIDGET(gtk_builder_get_object(builder, "main_box"));
    gtk_container_add(
        GTK_CONTAINER(gtk_dialog_get_content_area(GTK_DIALOG(dialog))), widget);
...
GtkWidget *msd_a11y_preferences_dialog_new(void) {
  ...
  object = g_object_new(MSD_TYPE_A11Y_PREFERENCES_DIALOG, NULL);
