...
static void msd_a11y_keyboard_manager_ensure_status_icon(
    MsdA11yKeyboardManager *manager);
static void set_server_from_settings(MsdA11yKeyboardManager *manager);

G_DEFINE_TYPE_WITH_PRIVATE(MsdA11yKeyboardManager, msd_a11y_keyboard_manager,
                           G_TYPE_OBJECT)

static gpointer manager_object = NULL;

static gboolean togglekeys_backend_enabled(MsdA11yKeyboardManager *manager,
                                           TogglekeysBackend backend) {
  return (
      g_settings_get_boolean(manager->priv->settings, "togglekeys-enable") &&
      g_settings_get_enum(manager->priv->settings, "togglekeys-backend") ==
          (gint)backend);
}
...
static void set_devicepresence_handler(MsdA11yKeyboardManager *manager) {
  Display *display;
...
static gboolean xkb_enabled(MsdA11yKeyboardManager *manager) {
  gboolean have_xkb;
  int opcode, errorBase, major, minor;

  have_xkb = XkbQueryExtension(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()),
                               &opcode, &manager->priv->xkbEventBase,
                               &errorBase, &major, &minor) &&
             XkbUseExtension(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()),
                             &major, &minor);

  return have_xkb;
}

static XkbDescRec *get_xkb_desc_rec(MsdA11yKeyboardManager *manager) {
  GdkDisplay *display;
  XkbDescRec *desc;
  ...
  desc = XkbGetMap(GDK_DISPLAY_XDISPLAY(display), XkbAllMapComponentsMask,
                   XkbUseCoreKbd);
  if (desc != NULL) {
    desc->ctrls = NULL;
    status =
        XkbGetControls(GDK_DISPLAY_XDISPLAY(display), XkbAllControlsMask, desc);
  }
...
static int get_int(GSettings *settings, char const *key) {
...
static gboolean set_int(GSettings *settings, char const *key, int val) {
...
static gboolean set_bool(GSettings *settings, char const *key, int val) {
  gboolean bval = (val != 0);
...
static unsigned long set_clear(gboolean flag, unsigned long value,
                               unsigned long mask) {
  if (flag) {
    return value | mask;
  }
...
static gboolean set_ctrl_from_settings(XkbDescRec *desc, GSettings *settings,
...
static void set_server_from_settings(MsdA11yKeyboardManager *manager) {
  ...
  desc->ctrls->enabled_ctrls =
      set_clear(enable_accessX, desc->ctrls->enabled_ctrls, XkbAccessXKeysMask);
  ...
  /* bounce keys */
  if (set_ctrl_from_settings(desc, manager->priv->settings, "bouncekeys-enable",
                             XkbBounceKeysMask)) {
    desc->ctrls->debounce_delay =
        get_int(manager->priv->settings, "bouncekeys-delay");
    desc->ctrls->ax_options = set_clear(
        g_settings_get_boolean(manager->priv->settings,
  ...
  /* mouse keys */
  if (set_ctrl_from_settings(desc, manager->priv->settings, "mousekeys-enable",
    ...
    desc->ctrls->mk_max_speed =
        get_int(manager->priv->settings, "mousekeys-max-speed") /
        (1000 / desc->ctrls->mk_interval);
    if (desc->ctrls->mk_max_speed <= 0) desc->ctrls->mk_max_speed = 1;

    desc->ctrls->mk_time_to_max =
        get_int(manager->priv->settings, /* events before max */
                "mousekeys-accel-time") /
        desc->ctrls->mk_interval;
    if (desc->ctrls->mk_time_to_max <= 0) desc->ctrls->mk_time_to_max = 1;

    desc->ctrls->mk_delay =
        get_int(manager->priv->settings, /* ms before 1st event */
                "mousekeys-init-delay");
  }

  /* slow keys */
  if (set_ctrl_from_settings(desc, manager->priv->settings, "slowkeys-enable",
                             XkbSlowKeysMask)) {
    desc->ctrls->ax_options = set_clear(
        g_settings_get_boolean(manager->priv->settings, "slowkeys-beep-press"),
        desc->ctrls->ax_options, XkbAccessXFeedbackMask | XkbAX_SKPressFBMask);
    desc->ctrls->ax_options = set_clear(
        g_settings_get_boolean(manager->priv->settings, "slowkeys-beep-accept"),
        desc->ctrls->ax_options, XkbAccessXFeedbackMask | XkbAX_SKAcceptFBMask);
    desc->ctrls->ax_options = set_clear(
        g_settings_get_boolean(manager->priv->settings, "slowkeys-beep-reject"),
        desc->ctrls->ax_options, XkbAccessXFeedbackMask | XkbAX_SKRejectFBMask);
    desc->ctrls->slow_keys_delay =
        get_int(manager->priv->settings, "slowkeys-delay");
    /* anything larger than 500 seems to loose all keyboard input */
    if (desc->ctrls->slow_keys_delay > 500) desc->ctrls->slow_keys_delay = 500;
  }

  /* sticky keys */
  if (set_ctrl_from_settings(desc, manager->priv->settings, "stickykeys-enable",
                             XkbStickyKeysMask)) {
    desc->ctrls->ax_options =
        set_clear(g_settings_get_boolean(manager->priv->settings,
  ...
  /* toggle keys */
  desc->ctrls->ax_options = set_clear(
      togglekeys_backend_enabled(manager, TOGGLEKEYS_BACKEND_XKB),
  ...
  */
  ...
  XkbSetControls(GDK_DISPLAY_XDISPLAY(display),
                 XkbSlowKeysMask | XkbBounceKeysMask | XkbStickyKeysMask |
                     XkbMouseKeysMask | XkbMouseKeysAccelMask |
                     XkbAccessXKeysMask | XkbAccessXTimeoutMask |
                     XkbAccessXFeedbackMask | XkbControlsEnabledMask,
                 desc);
...
static gboolean ax_response_callback(MsdA11yKeyboardManager *manager,
                                     GtkWindow *parent, gint response_id,
  ...
  switch (response_id) {
    ...
    case GTK_RESPONSE_CANCEL:
      ...
      if (revert_controls_mask == XkbStickyKeysMask) {
        g_settings_set_boolean(manager->priv->settings, "stickykeys-enable",
                               !enabled);
      } else if (revert_controls_mask == XkbSlowKeysMask) {
        g_settings_set_boolean(manager->priv->settings, "slowkeys-enable",
                               !enabled);
      }
    ...
    case GTK_RESPONSE_HELP:
      ...
      if (!gtk_show_uri_on_window(parent,
        ...
        GtkWidget *error_dialog = gtk_message_dialog_new(
            parent, 0, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
...
static void ax_stickykeys_response(GtkDialog *dialog, gint response_id,
                                   MsdA11yKeyboardManager *manager) {
  if (ax_response_callback(manager, GTK_WINDOW(dialog), response_id,
                           XkbStickyKeysMask,
...
static void ax_slowkeys_response(GtkDialog *dialog, gint response_id,
                                 MsdA11yKeyboardManager *manager) {
  if (ax_response_callback(manager, GTK_WINDOW(dialog), response_id,
                           XkbSlowKeysMask,
...
static void maybe_show_status_icon(MsdA11yKeyboardManager *manager) {
  gboolean show;

  /* for now, show if accessx is enabled */
  show = g_settings_get_boolean(manager->priv->settings, "enable");

  if (!show && manager->priv->status_icon == NULL) return;
...
static void on_slow_keys_action(NotifyNotification *notification,
                                const char *action,
                                MsdA11yKeyboardManager *manager) {
  gboolean res;
  int response_id;
  ...
  res = ax_response_callback(manager, NULL, response_id, XkbSlowKeysMask,
                             manager->priv->slowkeys_shortcut_val);
  if (res) {
...
static void on_sticky_keys_action(NotifyNotification *notification,
                                  const char *action,
                                  MsdA11yKeyboardManager *manager) {
  gboolean res;
  int response_id;
  ...
  res = ax_response_callback(manager, NULL, response_id, XkbStickyKeysMask,
                             manager->priv->stickykeys_shortcut_val);
  if (res) {
    notify_notification_close(manager->priv->notification, NULL);
  }
...
static gboolean ax_slowkeys_warning_post_bubble(MsdA11yKeyboardManager *manager,
                                                gboolean enabled) {
#ifdef HAVE_LIBNOTIFY
  gboolean res;
  const char *title;
  ...
  title = enabled ? _("Do you want to activate Slow Keys?")
  ...
  msd_a11y_keyboard_manager_ensure_status_icon(manager);
  manager->priv->notification = notify_notification_new(
      title, message, "preferences-desktop-accessibility");
  notify_notification_set_timeout(manager->priv->notification,
                                  NOTIFICATION_TIMEOUT * 1000);

  notify_notification_add_action(
      manager->priv->notification, "reject",
      enabled ? _("Don't activate") : _("Don't deactivate"),
      (NotifyActionCallback)on_slow_keys_action, manager, NULL);
  notify_notification_add_action(manager->priv->notification, "accept",
                                 enabled ? _("Activate") : _("Deactivate"),
                                 (NotifyActionCallback)on_slow_keys_action,
                                 manager, NULL);

  g_signal_connect(manager->priv->notification, "closed",
                   G_CALLBACK(on_notification_closed), manager);

  error = NULL;
  res = notify_notification_show(manager->priv->notification, &error);
  if (!res) {
    g_warning("MsdA11yKeyboardManager: unable to show notification: %s",
              error->message);
    g_error_free(error);
    notify_notification_close(manager->priv->notification, NULL);
  }
...
static void ax_slowkeys_warning_post_dialog(MsdA11yKeyboardManager *manager,
                                            gboolean enabled) {
  const char *title;
  const char *message;
  ...
  manager->priv->slowkeys_alert = gtk_message_dialog_new(
      NULL, 0, GTK_MESSAGE_WARNING, GTK_BUTTONS_NONE, "%s", title);

  gtk_message_dialog_format_secondary_text(
      GTK_MESSAGE_DIALOG(manager->priv->slowkeys_alert), "%s", message);
  ...
  g_signal_connect(manager->priv->slowkeys_alert, "response",
                   G_CALLBACK(ax_slowkeys_response), manager);
  gtk_widget_show(manager->priv->slowkeys_alert);
...
static void ax_slowkeys_warning_post(MsdA11yKeyboardManager *manager,
                                     gboolean enabled) {
...
static gboolean ax_stickykeys_warning_post_bubble(
    MsdA11yKeyboardManager *manager, gboolean enabled) {
#ifdef HAVE_LIBNOTIFY
  gboolean res;
  const char *title;
  const char *message;
  ...
  title = enabled ? _("Do you want to activate Sticky Keys?")
                  : _("Do you want to deactivate Sticky Keys?");
  message = enabled ? _("You just pressed the Shift key 5 times in a row.  "
                        "This is the shortcut "
                        "for the Sticky Keys feature, which affects the way "
                        "your keyboard works.")
                    : _("You just pressed two keys at once, or pressed the "
                        "Shift key 5 times in a row.  "
                        "This turns off the Sticky Keys feature, which affects "
  ...
  notify_notification_add_action(
      manager->priv->notification, "reject",
      enabled ? _("Don't activate") : _("Don't deactivate"),
      (NotifyActionCallback)on_sticky_keys_action, manager, NULL);
  notify_notification_add_action(manager->priv->notification, "accept",
                                 enabled ? _("Activate") : _("Deactivate"),
...
static void ax_stickykeys_warning_post_dialog(MsdA11yKeyboardManager *manager,
                                              gboolean enabled) {
  const char *title;
  const char *message;
  ...
  manager->priv->stickykeys_alert = gtk_message_dialog_new(
      NULL, 0, GTK_MESSAGE_WARNING, GTK_BUTTONS_NONE, "%s", title);

  gtk_message_dialog_format_secondary_text(
      GTK_MESSAGE_DIALOG(manager->priv->stickykeys_alert), "%s", message);
  ...
  g_signal_connect(manager->priv->stickykeys_alert, "response",
                   G_CALLBACK(ax_stickykeys_response), manager);
  gtk_widget_show(manager->priv->stickykeys_alert);
...
static void ax_stickykeys_warning_post(MsdA11yKeyboardManager *manager,
                                       gboolean enabled) {
...
static void set_settings_from_server(MsdA11yKeyboardManager *manager) {
  GSettings *settings;
  XkbDescRec *desc;
  ...
  */
  ...
  changed |= set_bool(
      settings, "feature-state-change-beep",
  ...
  changed |= set_int(settings, "timeout", desc->ctrls->ax_timeout);

  changed |= set_bool(settings, "bouncekeys-enable",
                      desc->ctrls->enabled_ctrls & XkbBounceKeysMask);
  changed |= set_int(settings, "bouncekeys-delay", desc->ctrls->debounce_delay);
  ...
  changed |=
      set_int(settings, "mousekeys-max-speed",
  ...
  changed |= set_int(settings, "mousekeys-init-delay", desc->ctrls->mk_delay);
  ...
  changed |= set_int(settings, "slowkeys-delay", desc->ctrls->slow_keys_delay);

  stickykeys_changed = set_bool(settings, "stickykeys-enable",
                                desc->ctrls->enabled_ctrls & XkbStickyKeysMask);
  changed |= set_bool(settings, "stickykeys-latch-to-lock",
  ...
  if (g_settings_get_enum(manager->priv->settings, "togglekeys-backend") ==
      TOGGLEKEYS_BACKEND_XKB) {
  ...
  if (!changed && stickykeys_changed ^ slowkeys_changed) {
    ...
    if (desc->ctrls->enabled_ctrls & XkbAccessXKeysMask) {
      if (slowkeys_changed) {
        ...
        ax_stickykeys_warning_post(
            manager, desc->ctrls->enabled_ctrls & XkbStickyKeysMask);
      }
    }
  }
...
static gboolean on_beep_dequence_timeout(gpointer user_data) {
...
static void beep_sequence(MsdA11yKeyboardManager *manager, GdkDisplay *display,
                          gint count, gint delay) {
...
static GdkFilterReturn cb_xkb_event_filter(GdkXEvent *xevent,
                                           GdkEvent *ignored1,
                                           MsdA11yKeyboardManager *manager) {
  XEvent *xev = (XEvent *)xevent;
  ...
  if (xev->xany.type == (manager->priv->xkbEventBase + XkbEventCode) &&
    ...
    if (xkbEv->indicators.state & xkbEv->indicators.changed) {
      beep_count = g_settings_get_int(manager->priv->settings,
                                      "togglekeys-enable-beep-count");
      beep_delay = g_settings_get_int(manager->priv->settings,
                                      "togglekeys-enable-beep-delay");
    } else {
      beep_count = g_settings_get_int(manager->priv->settings,
                                      "togglekeys-disable-beep-count");
      beep_delay = g_settings_get_int(manager->priv->settings,
                                      "togglekeys-disable-beep-delay");
    }
...
static gboolean start_a11y_keyboard_idle_cb(MsdA11yKeyboardManager *manager) {
  guint event_mask;
  ...
  if (g_settings_get_boolean(manager->priv->settings,
                             KEY_CAPSLOCK_BEEP_ENABLED))
    msd_a11y_keyboard_atspi_start(manager->priv->capslock_beep);
  g_signal_connect(manager->priv->settings,
                   "changed::" KEY_CAPSLOCK_BEEP_ENABLED,
                   G_CALLBACK(capslock_beep_callback), manager);
#endif

  if (!xkb_enabled(manager)) goto out;
...
#ifdef MATE_ENABLE_DEBUG
  event_mask |=
      XkbAccessXNotifyMask; /* make default when AXN_AXKWarning works */
#endif                      /* MATE_ENABLE_DEBUG */
  ...
  XkbSelectEvents(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()),
                  XkbUseCoreKbd, event_mask, event_mask);

  gdk_window_add_filter(NULL, (GdkFilterFunc)cb_xkb_event_filter, manager);
...
static void restore_server_xkb_config(MsdA11yKeyboardManager *manager) {
  GdkDisplay *display;
  ...
  XkbFreeKeyboard(manager->priv->original_xkb_desc, XkbAllComponentsMask, True);
...
void msd_a11y_keyboard_manager_stop(MsdA11yKeyboardManager *manager) {
  ...
  if (p->status_icon) gtk_status_icon_set_visible(p->status_icon, FALSE);
  ...
  gdk_window_remove_filter(NULL, (GdkFilterFunc)cb_xkb_event_filter, manager);

  /* Disable all the AccessX bits
   */
  restore_server_xkb_config(manager);

  if (p->slowkeys_alert != NULL) gtk_widget_destroy(p->slowkeys_alert);

  if (p->stickykeys_alert != NULL) gtk_widget_destroy(p->stickykeys_alert);
...
static void msd_a11y_keyboard_manager_class_init(
...
static void on_status_icon_activate(GtkStatusIcon *status_icon,
  ...
  if (manager->priv->preferences_dialog == NULL) {
    ...
    g_signal_connect(manager->priv->preferences_dialog, "response",
                     G_CALLBACK(on_preferences_dialog_response), manager);
...
static void msd_a11y_keyboard_manager_ensure_status_icon(
  ...
  if (!manager->priv->status_icon) {
    ...
    g_signal_connect(manager->priv->status_icon, "activate",
                     G_CALLBACK(on_status_icon_activate), manager);
  }

  mate_settings_profile_end(NULL);
}

static void msd_a11y_keyboard_manager_init(MsdA11yKeyboardManager *manager) {
