...
static void msd_a11y_keyboard_atspi_finalize(GObject *obj) {
...
static void msd_a11y_keyboard_atspi_class_init(
...
static void msd_a11y_keyboard_atspi_init(MsdA11yKeyboardAtspi *self) {
  self->listener = NULL;
  self->listening = FALSE;
  ...
  self->listener = atspi_device_listener_new(on_key_press_event, self, NULL);
...
static void register_deregister_events(MsdA11yKeyboardAtspi *self,
  ...
  for (AtspiKeyMaskType mod_mask = 0; mod_mask < 256; mod_mask++) {
    if (!(mod_mask & (1 << ATSPI_MODIFIER_SHIFTLOCK))) continue;

    if (do_register)
      atspi_register_keystroke_listener(self->listener, NULL, mod_mask,
                                        1 << ATSPI_KEY_PRESSED_EVENT,
                                        ATSPI_KEYLISTENER_NOSYNC, NULL);
    else
      atspi_deregister_keystroke_listener(self->listener, NULL, mod_mask,
                                          1 << ATSPI_KEY_PRESSED_EVENT, NULL);
  }
}
...
MsdA11yKeyboardAtspi *msd_a11y_keyboard_atspi_new() {
