...
MATE_SETTINGS_PLUGIN_REGISTER_WITH_PRIVATE(MsdA11yKeyboardPlugin,
                                           msd_a11y_keyboard_plugin)

static void msd_a11y_keyboard_plugin_init(MsdA11yKeyboardPlugin *plugin) {
...
static void msd_a11y_keyboard_plugin_finalize(GObject *object) {
...
static void impl_activate(MateSettingsPlugin *plugin) {
  ...
  error = NULL;
  res = msd_a11y_keyboard_manager_start(
      MSD_A11Y_KEYBOARD_PLUGIN(plugin)->priv->manager, &error);
  if (!res) {
...
static void impl_deactivate(MateSettingsPlugin *plugin) {
  ...
  msd_a11y_keyboard_manager_stop(
      MSD_A11Y_KEYBOARD_PLUGIN(plugin)->priv->manager);
}

static void msd_a11y_keyboard_plugin_class_init(
    MsdA11yKeyboardPluginClass *klass) {
...
static void msd_a11y_keyboard_plugin_class_finalize(
    MsdA11yKeyboardPluginClass *klass) {}
