...
MATE_SETTINGS_PLUGIN_REGISTER_WITH_PRIVATE(MsdSmartcardPlugin,
                                           msd_smartcard_plugin);

static void simulate_user_activity(MsdSmartcardPlugin *plugin) {
  DBusGProxy *screensaver_proxy;

  g_debug("MsdSmartcardPlugin telling screensaver about smart card insertion");
  screensaver_proxy = dbus_g_proxy_new_for_name(
...
static void lock_screen(MsdSmartcardPlugin *plugin) {
  DBusGProxy *screensaver_proxy;
  ...
  dbus_g_proxy_call_no_reply(screensaver_proxy, "Lock", G_TYPE_INVALID,
                             G_TYPE_INVALID);

  g_object_unref(screensaver_proxy);
}

static void force_logout(MsdSmartcardPlugin *plugin) {
  DBusGProxy *sm_proxy;
  GError *error;
  gboolean res;
  ...
  sm_proxy =
      dbus_g_proxy_new_for_name(plugin->priv->bus_connection, SM_DBUS_NAME,
                                SM_DBUS_PATH, SM_DBUS_INTERFACE);

  error = NULL;
  res = dbus_g_proxy_call(sm_proxy, "Logout", &error, G_TYPE_UINT,
                          SM_LOGOUT_MODE_FORCE, G_TYPE_INVALID, G_TYPE_INVALID);
...
static void msd_smartcard_plugin_init(MsdSmartcardPlugin *plugin) {
...
static void msd_smartcard_plugin_finalize(GObject *object) {
...
static gboolean user_logged_in_with_smartcard(void) {
  return g_getenv("PKCS11_LOGIN_TOKEN_NAME") != NULL;
}

static MsdSmartcardRemoveAction get_configured_remove_action(
    MsdSmartcardPlugin *plugin) {
  GSettings *settings;
...
static void process_smartcard_removal(MsdSmartcardPlugin *plugin) {
  MsdSmartcardRemoveAction remove_action;
...
static void impl_activate(MateSettingsPlugin *plugin) {
  ...
  if (smartcard_plugin->priv->is_active) {
    g_debug(
        "MsdSmartcardPlugin Not activating smartcard plugin, because it's "
        "already active");
    return;
  }
  ...
  if (!user_logged_in_with_smartcard()) {
    g_debug(
        "MsdSmartcardPlugin Not activating smartcard plugin, because user "
        "didn't use "
        " smartcard to log in");
  ...
  error = NULL;
  smartcard_plugin->priv->bus_connection =
      dbus_g_bus_get(DBUS_BUS_SESSION, &error);

  if (smartcard_plugin->priv->bus_connection == NULL) {
    g_warning("MsdSmartcardPlugin Unable to connect to session bus: %s",
              error->message);
    return;
  }

  if (!msd_smartcard_manager_start(smartcard_plugin->priv->manager, &error)) {
    g_warning("MsdSmartcardPlugin Unable to start smartcard manager: %s",
              error->message);
    g_error_free(error);
  }

  g_signal_connect(smartcard_plugin->priv->manager, "smartcard-removed",
                   G_CALLBACK(smartcard_removed_cb), smartcard_plugin);

  g_signal_connect(smartcard_plugin->priv->manager, "smartcard-inserted",
                   G_CALLBACK(smartcard_inserted_cb), smartcard_plugin);

  if (!msd_smartcard_manager_login_card_is_inserted(
          smartcard_plugin->priv->manager)) {
    g_debug(
        "MsdSmartcardPlugin processing smartcard removal immediately user "
        "logged in with smartcard "
...
static void impl_deactivate(MateSettingsPlugin *plugin) {
  ...
  if (!smartcard_plugin->priv->is_active) {
    g_debug(
        "MsdSmartcardPlugin Not deactivating smartcard plugin, "
...
static void msd_smartcard_plugin_class_init(MsdSmartcardPluginClass *klass) {
...
static void msd_smartcard_plugin_class_finalize(
    MsdSmartcardPluginClass *klass) {}
