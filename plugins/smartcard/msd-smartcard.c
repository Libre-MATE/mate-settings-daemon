...
static void msd_smartcard_class_install_properties(
    MsdSmartcardClass *card_class);
static void msd_smartcard_set_property(GObject *object, guint prop_id,
                                       const GValue *value, GParamSpec *pspec);
static void msd_smartcard_get_property(GObject *object, guint prop_id,
                                       GValue *value, GParamSpec *pspec);
static void msd_smartcard_set_name(MsdSmartcard *card, const char *name);
static void msd_smartcard_set_slot_id(MsdSmartcard *card, int slot_id);
static void msd_smartcard_set_slot_series(MsdSmartcard *card, int slot_series);
static void msd_smartcard_set_module(MsdSmartcard *card, SECMODModule *module);

static PK11SlotInfo *msd_smartcard_find_slot_from_id(MsdSmartcard *card,
                                                     int slot_id);

static PK11SlotInfo *msd_smartcard_find_slot_from_card_name(
    MsdSmartcard *card, const char *card_name);
...
enum { INSERTED, REMOVED, NUMBER_OF_SIGNALS };
...
static void msd_smartcard_class_init(MsdSmartcardClass *card_class) {
...
static void msd_smartcard_class_install_signals(MsdSmartcardClass *card_class) {
  ...
  msd_smartcard_signals[INSERTED] = g_signal_new(
      "inserted", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(MsdSmartcardClass, inserted), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  msd_smartcard_signals[REMOVED] = g_signal_new(
      "removed", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(MsdSmartcardClass, removed), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}
...
static void msd_smartcard_class_install_properties(
  ...
  param_spec =
      g_param_spec_ulong("slot-id", _("Slot ID"), _("The slot the card is in"),
                         1, G_MAXULONG, MSD_SMARTCARD_DEFAULT_SLOT_ID,
  ...
  param_spec = g_param_spec_int("slot-series", _("Slot Series"),
                                _("per-slot card identifier"), -1, G_MAXINT,
                                MSD_SMARTCARD_DEFAULT_SLOT_SERIES,
                                G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);
  g_object_class_install_property(object_class, PROP_SLOT_SERIES, param_spec);

  param_spec = g_param_spec_string("name", _("name"), _("name"), NULL,
                                   G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);
  g_object_class_install_property(object_class, PROP_NAME, param_spec);

  param_spec =
      g_param_spec_pointer("module", _("Module"), _("smartcard driver"),
...
static void msd_smartcard_set_property(GObject *object, guint prop_id,
  ...
  switch (prop_id) {
    ...
    case PROP_SLOT_ID:
      msd_smartcard_set_slot_id(card, g_value_get_ulong(value));
      break;

    case PROP_SLOT_SERIES:
      msd_smartcard_set_slot_series(card, g_value_get_int(value));
      break;

    case PROP_MODULE:
      msd_smartcard_set_module(card,
                               (SECMODModule *)g_value_get_pointer(value));
      break;
...
CK_SLOT_ID
msd_smartcard_get_slot_id(MsdSmartcard *card) { return card->priv->slot_id; }
...
static void msd_smartcard_get_property(GObject *object, guint prop_id,
  ...
  switch (prop_id) {
    case PROP_NAME:
      g_value_take_string(value, msd_smartcard_get_name(card));
      break;

    case PROP_SLOT_ID:
      g_value_set_ulong(value, (gulong)msd_smartcard_get_slot_id(card));
      break;

    case PROP_SLOT_SERIES:
      g_value_set_int(value, msd_smartcard_get_slot_series(card));
      break;
...
static void msd_smartcard_set_name(MsdSmartcard *card, const char *name) {
  ...
  if ((card->priv->name == NULL) || (strcmp(card->priv->name, name) != 0)) {
    g_free(card->priv->name);
    card->priv->name = g_strdup(name);

    if (card->priv->slot == NULL) {
      card->priv->slot =
          msd_smartcard_find_slot_from_card_name(card, card->priv->name);
...
static void msd_smartcard_set_slot_id(MsdSmartcard *card, int slot_id) {
  if (card->priv->slot_id != slot_id) {
    ...
    if (card->priv->slot == NULL) {
      card->priv->slot =
          msd_smartcard_find_slot_from_id(card, card->priv->slot_id);
...
int msd_smartcard_get_slot_series(MsdSmartcard *card) {
  return card->priv->slot_series;
}

static void msd_smartcard_init(MsdSmartcard *card) {
...
MsdSmartcard *_msd_smartcard_new(SECMODModule *module, CK_SLOT_ID slot_id,
                                 int slot_series) {
  MsdSmartcard *card;
  ...
  card = MSD_SMARTCARD(g_object_new(MSD_TYPE_SMARTCARD, "module", module,
                                    "slot-id", (gulong)slot_id, "slot-series",
                                    slot_series, NULL));
  return card;
}
...
MsdSmartcard *_msd_smartcard_new_from_name(SECMODModule *module,
  ...
  card = MSD_SMARTCARD(
      g_object_new(MSD_TYPE_SMARTCARD, "module", module, "name", name, NULL));
  return card;
}
 ...
 */
static char *msd_smartcard_password_handler(PK11SlotInfo *slot,
                                            PRBool is_retrying,
                                            const char *password) {
  if (is_retrying) {
    return NULL;
...
gboolean msd_smartcard_unlock(MsdSmartcard *card, const char *password) {
  SECStatus status;
...
static PK11SlotInfo *msd_smartcard_find_slot_from_card_name(
  ...
  for (i = 0; i < card->priv->module->slotCount; i++) {
    ...
    if ((slot_card_name != NULL) && (strcmp(slot_card_name, card_name) == 0)) {
