...
static void msd_smartcard_manager_class_install_signals(
    MsdSmartcardManagerClass *service_class);
static void msd_smartcard_manager_class_install_properties(
    MsdSmartcardManagerClass *service_class);
static void msd_smartcard_manager_set_property(GObject *object, guint prop_id,
                                               const GValue *value,
                                               GParamSpec *pspec);
static void msd_smartcard_manager_get_property(GObject *object, guint prop_id,
                                               GValue *value,
                                               GParamSpec *pspec);
static void msd_smartcard_manager_set_module_path(MsdSmartcardManager *manager,
                                                  const char *module_path);
static void msd_smartcard_manager_card_removed_handler(
    MsdSmartcardManager *manager, MsdSmartcard *card);
static void msd_smartcard_manager_card_inserted_handler(
    MsdSmartcardManager *manager_class, MsdSmartcard *card);
...
static gboolean msd_smartcard_manager_create_worker(
    MsdSmartcardManager *manager, int *worker_fd, GThread **worker_thread);

static MsdSmartcardManagerWorker *msd_smartcard_manager_worker_new(
    int write_fd);
static void msd_smartcard_manager_worker_free(
...
enum { PROP_0 = 0, PROP_MODULE_PATH, NUMBER_OF_PROPERTIES };

enum { SMARTCARD_INSERTED = 0, SMARTCARD_REMOVED, ERROR, NUMBER_OF_SIGNALS };

static guint msd_smartcard_manager_signals[NUMBER_OF_SIGNALS] = {0};

G_DEFINE_TYPE_WITH_PRIVATE(MsdSmartcardManager, msd_smartcard_manager,
                           G_TYPE_OBJECT);

static void msd_smartcard_manager_class_init(
    MsdSmartcardManagerClass *manager_class) {
...
static void msd_smartcard_manager_class_install_properties(
    MsdSmartcardManagerClass *card_class) {
  ...
  param_spec = g_param_spec_string("module-path", _("Module Path"),
                                   _("path to smartcard PKCS #11 driver"), NULL,
...
static void msd_smartcard_manager_set_property(GObject *object, guint prop_id,
  ...
  switch (prop_id) {
    case PROP_MODULE_PATH:
      msd_smartcard_manager_set_module_path(manager, g_value_get_string(value));
      break;
...
static void msd_smartcard_manager_class_install_signals(
  ...
  msd_smartcard_manager_signals[SMARTCARD_INSERTED] = g_signal_new(
      "smartcard-inserted", G_OBJECT_CLASS_TYPE(object_class),
      G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(MsdSmartcardManagerClass, smartcard_inserted), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  manager_class->smartcard_inserted =
      msd_smartcard_manager_card_inserted_handler;

  msd_smartcard_manager_signals[SMARTCARD_REMOVED] = g_signal_new(
      "smartcard-removed", G_OBJECT_CLASS_TYPE(object_class),
      G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(MsdSmartcardManagerClass, smartcard_removed), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  manager_class->smartcard_removed = msd_smartcard_manager_card_removed_handler;

  msd_smartcard_manager_signals[ERROR] = g_signal_new(
      "error", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(MsdSmartcardManagerClass, error), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  manager_class->error = NULL;
}

static gboolean slot_id_equal(const CK_SLOT_ID *slot_id_1,
...
static void msd_smartcard_manager_init(MsdSmartcardManager *manager) {
...
static void msd_smartcard_manager_finalize(GObject *object) {
  ...
  gobject_class = G_OBJECT_CLASS(msd_smartcard_manager_parent_class);
...
GQuark msd_smartcard_manager_error_quark(void) {
  static GQuark error_quark = 0;

  if (error_quark == 0) {
    error_quark =
        g_quark_from_static_string("msd-smartcard-manager-error-quark");
  }

  return error_quark;
}

MsdSmartcardManager *msd_smartcard_manager_new(const char *module_path) {
  MsdSmartcardManager *instance;

  instance = MSD_SMARTCARD_MANAGER(g_object_new(
      MSD_TYPE_SMARTCARD_MANAGER, "module-path", module_path, NULL));

  return instance;
}

static void msd_smartcard_manager_emit_error(MsdSmartcardManager *manager,
                                             GError *error) {
  manager->priv->is_unstoppable = TRUE;
  g_signal_emit(manager, msd_smartcard_manager_signals[ERROR], 0, error);
  manager->priv->is_unstoppable = FALSE;
}

static void msd_smartcard_manager_emit_smartcard_inserted(
    MsdSmartcardManager *manager, MsdSmartcard *card) {
  manager->priv->is_unstoppable = TRUE;
...
static void msd_smartcard_manager_emit_smartcard_removed(
    MsdSmartcardManager *manager, MsdSmartcard *card) {
  manager->priv->is_unstoppable = TRUE;
...
static gboolean msd_smartcard_manager_check_for_and_process_events(
    GIOChannel *io_channel, GIOCondition condition,
    MsdSmartcardManager *manager) {
  MsdSmartcard *card;
  ...
  if (should_stop) {
    g_debug(
        "received %s on event socket, stopping "
        "manager...",
        (condition & G_IO_HUP) && (condition & G_IO_ERR) ? "error and hangup"
        : (condition & G_IO_HUP)                         ? "hangup"
                                                         : "error");
  }
  ...
  switch (event_type) {
    case 'I':
      g_hash_table_replace(manager->priv->smartcards, card_name, card);
      card_name = NULL;
...
out:
  if (should_stop) {
    ...
    error = g_error_new(MSD_SMARTCARD_MANAGER_ERROR,
                        MSD_SMARTCARD_MANAGER_ERROR_WATCHING_FOR_EVENTS, "%s",
                        (condition & G_IO_IN)
                            ? g_strerror(errno)
                            : _("received error or hang up from event source"));
...
static void msd_smartcard_manager_event_processing_stopped_handler(
    MsdSmartcardManager *manager) {
...
static void msd_smartcard_manager_stop_watching_for_events(
    MsdSmartcardManager *manager) {
...
static gboolean load_nss(GError **error) {
  ...
  static const guint32 flags = NSS_INIT_READONLY | NSS_INIT_NOCERTDB |
                               NSS_INIT_NOMODDB | NSS_INIT_FORCEOPEN |
                               NSS_INIT_NOROOTINIT | NSS_INIT_OPTIMIZESPACE |
                               NSS_INIT_PK11RELOAD;

  g_debug("attempting to load NSS database '%s'", MSD_SMARTCARD_MANAGER_NSS_DB);

  status =
      NSS_Initialize(MSD_SMARTCARD_MANAGER_NSS_DB, "", "", SECMOD_DB, flags);
  ...
  if (status != SECSuccess) {
    ...
    g_set_error(error, MSD_SMARTCARD_MANAGER_ERROR,
                MSD_SMARTCARD_MANAGER_ERROR_WITH_NSS, "%s", error_message);
    g_debug("NSS security system could not be initialized - %s", error_message);
...
static SECMODModule *load_driver(char *module_path, GError **error) {
  SECMODModule *module;
  char *module_spec;
  ...
  if (module_explicitly_specified) {
    ...
    g_debug("loading smartcard driver using spec '%s'", module_spec);

    module = SECMOD_LoadUserModule(module_spec, NULL /* parent */,
                                   FALSE /* recurse */);
    g_free(module_spec);
    module_spec = NULL;
    ...
    for (tmp = modules; tmp != NULL; tmp = tmp->next) {
      if (!SECMOD_HasRemovableSlots(tmp->module) || !tmp->module->loaded)
        continue;
  ...
  if (!module_explicitly_specified && module == NULL) {
    ...
    if (error_message_size == 0) {
      g_debug("smartcard driver '%s' could not be loaded", module_path);
      g_set_error(error, MSD_SMARTCARD_MANAGER_ERROR,
                  MSD_SMARTCARD_MANAGER_ERROR_LOADING_DRIVER,
    ...
    g_set_error(error, MSD_SMARTCARD_MANAGER_ERROR,
                MSD_SMARTCARD_MANAGER_ERROR_LOADING_DRIVER, "%s",
                error_message);

    g_debug("smartcard driver '%s' could not be loaded - %s", module_path,
            error_message);
    g_slice_free1(error_message_size, error_message);
  }
...
static void msd_smartcard_manager_get_all_cards(MsdSmartcardManager *manager) {
  int i;
  ...
  for (i = 0; i < manager->priv->module->slotCount; i++) {
    ...
    card = _msd_smartcard_new(manager->priv->module, slot_id, slot_series);
...
gboolean msd_smartcard_manager_start(MsdSmartcardManager *manager,
  ...
  if (!msd_smartcard_manager_create_worker(manager, &worker_fd,
                                           &manager->priv->worker_thread)) {
    g_set_error(error, MSD_SMARTCARD_MANAGER_ERROR,
  ...
  g_source_set_callback(
      ...
      (GSourceFunc)(GIOFunc)msd_smartcard_manager_check_for_and_process_events,
      manager,
      (GDestroyNotify)msd_smartcard_manager_event_processing_stopped_handler);
...
static void msd_smartcard_manager_check_for_login_card(CK_SLOT_ID slot_id,
                                                       MsdSmartcard *card,
...

{
  ...
  g_hash_table_foreach(manager->priv->smartcards,
                       (GHFunc)msd_smartcard_manager_check_for_login_card,
                       &is_inserted);
  return is_inserted;
}
...
static MsdSmartcardManagerWorker *msd_smartcard_manager_worker_new(
  ...
  worker->smartcards = g_hash_table_new_full(
      (GHashFunc)slot_id_hash, (GEqualFunc)slot_id_equal,
...
static gboolean msd_smartcard_manager_worker_emit_smartcard_removed(
    MsdSmartcardManagerWorker *worker, MsdSmartcard *card, GError **error) {
...
error_out:
  g_set_error(error, MSD_SMARTCARD_MANAGER_ERROR,
              MSD_SMARTCARD_MANAGER_ERROR_REPORTING_EVENTS, "%s",
              g_strerror(errno));
  return FALSE;
}

static gboolean msd_smartcard_manager_worker_emit_smartcard_inserted(
    MsdSmartcardManagerWorker *worker, MsdSmartcard *card, GError **error) {
...
static gboolean msd_smartcard_manager_worker_watch_for_and_process_event(
    MsdSmartcardManagerWorker *worker, GError **error) {
  PK11SlotInfo *slot;
  ...
  slot =
      SECMOD_WaitForAnyTokenEvent(worker->module, 0, PR_SecondsToInterval(1));
  ...
  *key = slot_id;
  ...
  if (PK11_IsPresent(slot)) {
     ...
     */
    if ((card != NULL) && card_slot_series != slot_series) {
    ...
    card = _msd_smartcard_new(worker->module, slot_id, slot_series);

    g_hash_table_replace(worker->smartcards, key, card);
    key = NULL;
...
static void msd_smartcard_manager_worker_run(
    MsdSmartcardManagerWorker *worker) {
  GError *error;
  ...
  while (
      msd_smartcard_manager_worker_watch_for_and_process_event(worker, &error))
    ;
...
static gboolean msd_smartcard_manager_create_worker(
  ...
  *worker_thread =
      g_thread_new("MsdSmartcardManagerWorker",
                   (GThreadFunc)msd_smartcard_manager_worker_run, worker);
...
static gboolean on_timeout(MsdSmartcardManager *manager) {
  GError *error;
  g_print("Re-enabling manager.\n");

  if (!msd_smartcard_manager_start(manager, &error)) {
    g_warning("could not start smartcard manager - %s", error->message);
    g_error_free(error);
    return TRUE;
  }
...
static void on_device_inserted(MsdSmartcardManager *manager,
                               MsdSmartcard *card) {
  g_print("smartcard inserted!\n");
  g_print("Please remove it.\n");
}

static void on_device_removed(MsdSmartcardManager *manager,
                              MsdSmartcard *card) {
...
int main(int argc, char *argv[]) {
  ...
  g_signal_connect(manager, "smartcard-removed", G_CALLBACK(on_device_removed),
                   NULL);
