...
enum { MEDIA_PLAYER_KEY_PRESSED, LAST_SIGNAL };

static guint signals[LAST_SIGNAL] = {0};

G_DEFINE_TYPE_WITH_PRIVATE(MsdMediaKeysManager, msd_media_keys_manager,
                           G_TYPE_OBJECT)

static gpointer manager_object = NULL;

static void init_screens(MsdMediaKeysManager *manager) {
  GdkDisplay *display;
...
static void acme_error(char *msg) {
  GtkWidget *error_dialog;

  error_dialog = gtk_message_dialog_new(
      NULL, GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, msg, NULL);
...
static char *get_term_command(MsdMediaKeysManager *manager) {
...
static void execute(MsdMediaKeysManager *manager, char *cmd, gboolean sync,
                    gboolean need_term) {
  gboolean retval;
  char **argv;
  int argc;
  ...
  if (need_term) {
    ...
    if (term == NULL) {
      acme_error(
          _("Could not get default terminal. Verify that your default "
  ...
  if (g_shell_parse_argv(exec, &argc, &argv, NULL)) {
    if (sync != FALSE) {
      retval = g_spawn_sync(g_get_home_dir(), argv, NULL, G_SPAWN_SEARCH_PATH,
                            NULL, NULL, NULL, NULL, NULL, NULL);
    } else {
      retval = g_spawn_async(g_get_home_dir(), argv, NULL, G_SPAWN_SEARCH_PATH,
                             NULL, NULL, NULL, NULL);
    }
    g_strfreev(argv);
  }
...
static void dialog_init(MsdMediaKeysManager *manager) {
...
static gboolean is_valid_shortcut(const char *string) {
...
static void update_kbd_cb(GSettings *settings, gchar *settings_key,
  ...
  for (i = 0; i < HANDLED_KEYS; i++) {
    if (g_strcmp0(settings_key, keys[i].settings_key) == 0) {
      ...
      tmp = g_settings_get_string(settings, keys[i].settings_key);
      ...
      key = g_new0(Key, 1);
      if (!egg_accelerator_parse_virtual(tmp, &key->keysym, &key->keycodes,
                                         &key->state)) {
        g_free(tmp);
        g_free(key);
        break;
  ...
  if (need_flush) gdk_display_flush(dpy);
  if (gdk_x11_display_error_trap_pop(dpy))
    g_warning(
        "Grab failed for some keys, another application may already have "
        "access the them.");
}
...
static void init_kbd(MsdMediaKeysManager *manager) {
  ...
  for (i = 0; i < HANDLED_KEYS; i++) {
    ...
    g_signal_connect(manager->priv->settings, signal_name,
                     G_CALLBACK(update_kbd_cb), manager);
    g_free(signal_name);

    if (keys[i].settings_key != NULL) {
      tmp =
          g_settings_get_string(manager->priv->settings, keys[i].settings_key);
    } else {
...
static void ensure_cancellable(GCancellable **cancellable) {
  if (*cancellable == NULL) {
...
static void dialog_show(MsdMediaKeysManager *manager) {
  int orig_w;
  int orig_h;
  int screen_w;
  int screen_h;
  ...
  if (!g_settings_get_boolean(manager->priv->settings, "enable-osd")) return;
   ...
   */
  gtk_window_get_default_size(GTK_WINDOW(manager->priv->dialog), &orig_w,
                              &orig_h);
...
static void do_url_action(MsdMediaKeysManager *manager, const gchar *scheme) {
  GError *error = NULL;
  GAppInfo *app_info;
  ...
  if (app_info != NULL) {
    if (!g_app_info_launch(app_info, NULL, NULL, &error)) {
      g_warning("Could not launch '%s': %s",
                g_app_info_get_commandline(app_info), error->message);
...
static void do_media_action(MsdMediaKeysManager *manager) {
  GError *error = NULL;
  GAppInfo *app_info;
  ...
  if (app_info != NULL) {
    ...
    g_warning("Could not find default application for '%s' mime-type",
              "audio/x-vorbis+ogg");
  }
}

static void do_calculator_action(MsdMediaKeysManager *manager) {
  GSettings *settings;
  char *calc;
  ...
  if (calc) execute(manager, calc, FALSE, FALSE);

  g_free(calc);
  g_object_unref(settings);
}

static void do_messenger_action(MsdMediaKeysManager *manager) {
  GSettings *settings;
  char *messenger;
  ...
  if (messenger) execute(manager, messenger, FALSE, FALSE);

  g_free(messenger);
  g_object_unref(settings);
}

static void do_shutdown_action(MsdMediaKeysManager *manager) {
  execute(manager, "mate-session-save --shutdown-dialog", FALSE, FALSE);
}

static void do_logout_action(MsdMediaKeysManager *manager) {
  execute(manager, "mate-session-save --logout-dialog", FALSE, FALSE);
}

static void do_eject_action_cb(GDrive *drive, GAsyncResult *res,
                               MsdMediaKeysManager *manager) {
...
static void do_eject_action(MsdMediaKeysManager *manager) {
  GList *drives, *l;
  GDrive *fav_drive;
  guint score;
  ...
  msd_media_keys_window_set_action_custom(
      MSD_MEDIA_KEYS_WINDOW(manager->priv->dialog), "media-eject", NULL);
  dialog_show(manager);

  /* Clean up the drive selection and exit if no suitable
   * drives are found */
  if (fav_drive != NULL) fav_drive = g_object_ref(fav_drive);
...
static void do_touchpad_osd_action(MsdMediaKeysManager *manager,
                                   gboolean state) {
  dialog_init(manager);
  msd_media_keys_window_set_action_custom(
      MSD_MEDIA_KEYS_WINDOW(manager->priv->dialog),
      state ? "input-touchpad" : "touchpad-disabled",
      state ? _("Touchpad enabled") : _("Touchpad disabled"));
  dialog_show(manager);
}

static void do_touchpad_action(MsdMediaKeysManager *manager) {
...
#ifdef HAVE_LIBMATEMIXER
static void update_dialog(MsdMediaKeysManager *manager, guint volume,
                          gboolean muted, gboolean sound_changed,
                          gboolean quiet, gboolean is_mic) {
  if (muted) volume = 0;

  dialog_init(manager);

  if (is_mic)
    msd_media_keys_window_set_mic_muted(
        MSD_MEDIA_KEYS_WINDOW(manager->priv->dialog), muted);
  else
    msd_media_keys_window_set_volume_muted(
        MSD_MEDIA_KEYS_WINDOW(manager->priv->dialog), muted);

  msd_media_keys_window_set_volume_level(
      MSD_MEDIA_KEYS_WINDOW(manager->priv->dialog), volume);
  ...
  if (quiet == FALSE && sound_changed != FALSE && muted == FALSE &&
    ...
    ca_gtk_play_for_widget(
        manager->priv->dialog, 0, CA_PROP_EVENT_ID, "audio-volume-change",
        CA_PROP_EVENT_DESCRIPTION, "Volume changed through key press",
        CA_PROP_APPLICATION_NAME, PACKAGE_NAME, CA_PROP_APPLICATION_VERSION,
        PACKAGE_VERSION, CA_PROP_APPLICATION_ID, "org.mate.SettingsDaemon",
        NULL);
#endif
}

static void do_sound_action(MsdMediaKeysManager *manager, int type,
                            gboolean quiet) {
  gboolean muted;
  gboolean muted_last;
  ...
  gboolean is_input_control = type == MIC_MUTE_KEY ? TRUE : FALSE;
  if (is_input_control)
  ...
  if (volume_step <= 0 || volume_step > 100) {
    GVariant *variant =
        g_settings_get_default_value(manager->priv->settings, "volume-step");
  ...
  volume = volume_last = mate_mixer_stream_control_get_volume(control);
  muted = muted_last = mate_mixer_stream_control_get_mute(control);
  ...
  switch (type) {
    ...
    case VOLUME_UP_KEY:
      if (muted) {
        muted = FALSE;
        if (volume <= volume_min) volume = volume_min + volume_step_scaled;
      } else
        volume = CLAMP(volume + volume_step_scaled, volume_min, volume_max);
      break;
  }
  ...
  update_dialog(manager, MIN(100 * volume / (volume_max - volume_min), 100),
                muted, sound_changed, quiet, is_input_control);
}

static void update_default_output(MsdMediaKeysManager *manager) {
  MateMixerStream *stream;
  MateMixerStreamControl *control = NULL;

  stream = mate_mixer_context_get_default_output_stream(manager->priv->context);
  if (stream != NULL) control = mate_mixer_stream_get_default_control(stream);

  if (stream == manager->priv->stream) return;
  ...
  if (control != NULL) {
    MateMixerStreamControlFlags flags =
        mate_mixer_stream_control_get_flags(control);
...
static void update_default_input(MsdMediaKeysManager *manager) {
  MateMixerStream *stream;
  MateMixerStreamControl *control = NULL;
  ...
  if (stream == manager->priv->source_stream) return;
  ...
  if (control != NULL) {
    ...
    /* Do not use the stream if it is not possible to mute it or
     * change the volume */
    if (!(flags & MATE_MIXER_STREAM_CONTROL_MUTE_WRITABLE)) return;
...
static gboolean get_rfkill_property(MsdMediaKeysManager *manager,
                                    const char *property) {
  GVariant *v;
  gboolean ret;
...
static void set_rfkill_complete(GObject *object, GAsyncResult *result,
                                gpointer user_data) {
  GError *error = NULL;
  GVariant *variant;
  ...
  if (variant == NULL) {
    if (!g_error_matches(error, G_IO_ERROR, G_IO_ERROR_CANCELLED))
      g_warning("Failed to set '%s' property: %s", data->property,
                error->message);
    g_error_free(error);
    goto out;
  }
  ...
  g_debug("Finished changing rfkill, property %s is now %s", data->property,
          data->target_state ? "true" : "false");

  if (data->bluetooth) {
    if (data->target_state)
      msd_media_keys_window_set_action_custom(
          MSD_MEDIA_KEYS_WINDOW(data->manager->priv->dialog),
...
static void do_rfkill_action(MsdMediaKeysManager *manager, gboolean bluetooth) {
  ...
  hw_mode =
      bluetooth ? "BluetoothHardwareAirplaneMode" : "HardwareAirplaneMode";
  mode = bluetooth ? "BluetoothAirplaneMode" : "AirplaneMode";

  if (manager->priv->rfkill_proxy == NULL) return;

  if (get_rfkill_property(manager, has_mode) == FALSE) return;

  if (get_rfkill_property(manager, hw_mode)) {
    msd_media_keys_window_set_action_custom(
        MSD_MEDIA_KEYS_WINDOW(manager->priv->dialog), "airplane-mode-symbolic",
  ...
  g_dbus_proxy_call(
      manager->priv->rfkill_proxy, "org.freedesktop.DBus.Properties.Set",
      g_variant_new("(ssv)", "org.mate.SettingsDaemon.Rfkill", data->property,
                    g_variant_new_boolean(new_state)),
      G_DBUS_CALL_FLAGS_NONE, -1, manager->priv->rfkill_cancellable,
      set_rfkill_complete, data);

  g_debug("Setting rfkill property %s to %s", data->property,
          new_state ? "true" : "false");
}

static void do_display_osd_action(MsdMediaKeysManager *manager) {
  GdkDisplay *display;
  int n_monitors;
  ...
  if (n_monitors > 1)
    msd_media_keys_window_set_action_custom(
        MSD_MEDIA_KEYS_WINDOW(manager->priv->dialog),
        "video-joined-displays-symbolic", _("Changing Screen Layout"));
  else
    msd_media_keys_window_set_action_custom(
        MSD_MEDIA_KEYS_WINDOW(manager->priv->dialog),
        "video-single-display-symbolic", _("No External Display"));
  dialog_show(manager);
}

static gint find_by_application(gconstpointer a, gconstpointer b) {
  return strcmp(((MediaPlayer *)a)->application, b);
}

static gint find_by_time(gconstpointer a, gconstpointer b) {
 ...
 */
gboolean msd_media_keys_manager_grab_media_player_keys(
    MsdMediaKeysManager *manager, const char *application, guint32 time,
    GError **error) {
  GList *iter;
  ...
  iter = g_list_find_custom(manager->priv->media_players, application,
                            find_by_application);
  ...
  if (iter != NULL) {
    if (((MediaPlayer *)iter->data)->time < time) {
      ...
      g_free(iter->data);
      manager->priv->media_players =
          g_list_delete_link(manager->priv->media_players, iter);
    } else {
      return TRUE;
    }
  }
  ...
  manager->priv->media_players = g_list_insert_sorted(
      manager->priv->media_players, media_player, find_by_time);

  return TRUE;
}

gboolean msd_media_keys_manager_release_media_player_keys(
    MsdMediaKeysManager *manager, const char *application, GError **error) {
  GList *iter;
...
static gboolean msd_media_player_key_pressed(MsdMediaKeysManager *manager,
                                             const char *key) {
  ...
  if (have_listeners) {
    application =
        ((MediaPlayer *)manager->priv->media_players->data)->application;
  }

  g_signal_emit(manager, signals[MEDIA_PLAYER_KEY_PRESSED], 0, application,
                key);

  return !have_listeners;
}

static gboolean do_multimedia_player_action(MsdMediaKeysManager *manager,
                                            const char *key) {
  return msd_media_player_key_pressed(manager, key);
}

static void do_toggle_accessibility_key(const char *key) {
  GSettings *settings;
  gboolean state;
...
static void do_magnifier_action(MsdMediaKeysManager *manager) {
  do_toggle_accessibility_key("screen-magnifier-enabled");
}

static void do_screenreader_action(MsdMediaKeysManager *manager) {
  do_toggle_accessibility_key("screen-reader-enabled");
}

static void do_on_screen_keyboard_action(MsdMediaKeysManager *manager) {
  do_toggle_accessibility_key("screen-keyboard-enabled");
}

static gboolean do_action(MsdMediaKeysManager *manager, int type) {
  char *cmd;
  char *path;
...
static GdkFilterReturn acme_filter_events(GdkXEvent *xevent, GdkEvent *event,
                                          MsdMediaKeysManager *manager) {
...
static void on_rfkill_proxy_ready(GObject *source, GAsyncResult *result,
                                  gpointer data) {
  MsdMediaKeysManager *manager = data;

  manager->priv->rfkill_proxy = g_dbus_proxy_new_for_bus_finish(result, NULL);
}

static void rfkill_appeared_cb(GDBusConnection *connection, const gchar *name,
                               const gchar *name_owner, gpointer user_data) {
  MsdMediaKeysManager *manager = user_data;

  g_dbus_proxy_new_for_bus(
      G_BUS_TYPE_SESSION, 0, NULL, "org.mate.SettingsDaemon.Rfkill",
      "/org/mate/SettingsDaemon/Rfkill", "org.mate.SettingsDaemon.Rfkill",
      manager->priv->rfkill_cancellable, on_rfkill_proxy_ready, manager);
}

static gboolean start_media_keys_idle_cb(MsdMediaKeysManager *manager) {
  GSList *l;
  GdkDisplay *dpy;
  Display *xdpy;
  ...
  for (l = manager->priv->screens; l != NULL; l = l->next) {
    ...
    gdk_window_add_filter(window, (GdkFilterFunc)acme_filter_events, manager);
  ...
  manager->priv->rfkill_watch_id = g_bus_watch_name(
      G_BUS_TYPE_SESSION, "org.mate.SettingsDaemon.Rfkill",
      G_BUS_NAME_WATCHER_FLAGS_NONE, rfkill_appeared_cb, NULL, manager, NULL);
...
gboolean msd_media_keys_manager_start(MsdMediaKeysManager *manager,
  ...
  if (G_LIKELY(mate_mixer_is_initialized())) {
    ...
    g_signal_connect(manager->priv->context, "notify::state",
                     G_CALLBACK(on_context_state_notify), manager);
    g_signal_connect(manager->priv->context, "notify::default-output-stream",
                     G_CALLBACK(on_context_default_output_notify), manager);
    g_signal_connect(manager->priv->context, "notify::default-input-stream",
                     G_CALLBACK(on_context_default_input_notify), manager);
    g_signal_connect(manager->priv->context, "stream-removed",
                     G_CALLBACK(on_context_stream_removed), manager);
...
void msd_media_keys_manager_stop(MsdMediaKeysManager *manager) {
  ...
  for (ls = priv->screens; ls != NULL; ls = ls->next) {
    gdk_window_remove_filter(gdk_screen_get_root_window(ls->data),
                             (GdkFilterFunc)acme_filter_events, manager);
  }
...
static void msd_media_keys_manager_class_init(MsdMediaKeysManagerClass *klass) {
  signals[MEDIA_PLAYER_KEY_PRESSED] = g_signal_new(
      "media-player-key-pressed", G_OBJECT_CLASS_TYPE(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(MsdMediaKeysManagerClass, media_player_key_pressed), NULL,
      NULL, msd_marshal_VOID__STRING_STRING, G_TYPE_NONE, 2, G_TYPE_STRING,
      G_TYPE_STRING);

  dbus_g_object_type_install_info(
      MSD_TYPE_MEDIA_KEYS_MANAGER,
      &dbus_glib_msd_media_keys_manager_object_info);
}

static void msd_media_keys_manager_init(MsdMediaKeysManager *manager) {
  manager->priv = msd_media_keys_manager_get_instance_private(manager);
}

static gboolean register_manager(MsdMediaKeysManager *manager) {
  GError *error = NULL;
  ...
  dbus_g_connection_register_g_object(
      manager->priv->connection, MSD_MEDIA_KEYS_DBUS_PATH, G_OBJECT(manager));
