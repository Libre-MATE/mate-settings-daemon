...
MATE_SETTINGS_PLUGIN_REGISTER_WITH_PRIVATE(MsdMediaKeysPlugin,
                                           msd_media_keys_plugin)

static void msd_media_keys_plugin_init(MsdMediaKeysPlugin *plugin) {
...
static void msd_media_keys_plugin_dispose(GObject *object) {
...
static void impl_activate(MateSettingsPlugin *plugin) {
  ...
  mate_mixer_init();
#endif
  res = msd_media_keys_manager_start(
      MSD_MEDIA_KEYS_PLUGIN(plugin)->priv->manager, &error);
  if (!res) {
...
static void msd_media_keys_plugin_class_init(MsdMediaKeysPluginClass *klass) {
...
static void msd_media_keys_plugin_class_finalize(
    MsdMediaKeysPluginClass *klass) {}
