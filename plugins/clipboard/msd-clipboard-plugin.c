...
MATE_SETTINGS_PLUGIN_REGISTER_WITH_PRIVATE(MsdClipboardPlugin,
                                           msd_clipboard_plugin)

static void msd_clipboard_plugin_init(MsdClipboardPlugin *plugin) {
...
static void msd_clipboard_plugin_finalize(GObject *object) {
...
static void impl_activate(MateSettingsPlugin *plugin) {
  ...
  error = NULL;
  res = msd_clipboard_manager_start(MSD_CLIPBOARD_PLUGIN(plugin)->priv->manager,
                                    &error);
  if (!res) {
...
static void msd_clipboard_plugin_class_init(MsdClipboardPluginClass *klass) {
...
static void msd_clipboard_plugin_class_finalize(
    MsdClipboardPluginClass *klass) {}
