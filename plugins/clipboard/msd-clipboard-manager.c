...
static void clipboard_manager_watch_cb(MsdClipboardManager *manager,
                                       Window window, Bool is_start, long mask,
                                       void *cb_data);

G_DEFINE_TYPE_WITH_PRIVATE(MsdClipboardManager, msd_clipboard_manager,
                           G_TYPE_OBJECT)
 ...
 */
static TargetData *target_data_ref(TargetData *data) {
  data->refcount++;
  return data;
}

static void target_data_unref(TargetData *data) {
  data->refcount--;
  if (data->refcount == 0) {
    free(data->data);
    free(data);
...
static void conversion_free(IncrConversion *rdata) {
  if (rdata->data) {
...
static void send_selection_notify(MsdClipboardManager *manager, Bool success) {
  ...
  XSendEvent(manager->priv->display, manager->priv->requestor, False,
...
static void finish_selection_request(MsdClipboardManager *manager, XEvent *xev,
                                     Bool success) {
  XSelectionEvent notify;
  GdkDisplay *display;
  ...
  XSendEvent(xev->xselectionrequest.display, xev->xselectionrequest.requestor,
...
static int clipboard_bytes_per_item(int format) {
  switch (format) {
    case 8:
      return sizeof(char);
    case 16:
...
static void save_targets(MsdClipboardManager *manager, Atom *save_targets,
                         int nitems) {
  int nout, i;
  Atom *multiple;
  TargetData *tdata;
  ...
  nout = 0;
  for (i = 0; i < nitems; i++) {
    if (save_targets[i] != XA_TARGETS && save_targets[i] != XA_MULTIPLE &&
        save_targets[i] != XA_DELETE && save_targets[i] != XA_INSERT_PROPERTY &&
  ...
  XChangeProperty(manager->priv->display, manager->priv->window, XA_MULTIPLE,
                  XA_ATOM_PAIR, 32, PropModeReplace,
                  (const unsigned char *)multiple, nout);
  free(multiple);

  XConvertSelection(manager->priv->display, XA_CLIPBOARD, XA_MULTIPLE,
                    XA_MULTIPLE, manager->priv->window, manager->priv->time);
}

static int find_content_target(TargetData *tdata, Atom target) {
  return tdata->target == target;
}

static int find_content_type(TargetData *tdata, Atom type) {
  return tdata->type == type;
}

static int find_conversion_requestor(IncrConversion *rdata, XEvent *xev) {
  return (rdata->requestor == xev->xproperty.window &&
          rdata->property == xev->xproperty.atom);
}

static void get_property(TargetData *tdata, MsdClipboardManager *manager) {
  Atom type;
  int format;
  ...
  XGetWindowProperty(manager->priv->display, manager->priv->window,
                     tdata->target, 0, 0x1FFFFFFF, True, AnyPropertyType, &type,
                     &format, &length, &remaining, &data);
...
static Bool receive_incrementally(MsdClipboardManager *manager, XEvent *xev) {
  List *list;
  TargetData *tdata;
  Atom type;
  int format;
  ...
  if (xev->xproperty.window != manager->priv->window) return False;

  list = list_find(manager->priv->contents, (ListFindFunc)find_content_target,
  ...
  XGetWindowProperty(xev->xproperty.display, xev->xproperty.window,
                     xev->xproperty.atom, 0, 0x1FFFFFFF, True, AnyPropertyType,
  ...
  if (length == 0) {
    ...
    if (!list_find(manager->priv->contents, (ListFindFunc)find_content_type,
                   (void *)XA_INCR)) {
...
static Bool send_incrementally(MsdClipboardManager *manager, XEvent *xev) {
  List *list;
  ...
  XChangeProperty(manager->priv->display, rdata->requestor, rdata->property,
                  rdata->data->type, rdata->data->format, PropModeAppend, data,
                  items);
...
static void convert_clipboard_manager(MsdClipboardManager *manager,
                                      XEvent *xev) {
  GdkDisplay *display;
  Atom type = None;
  int format;
  ...
  if (xev->xselectionrequest.target == XA_SAVE_TARGETS) {
    if (manager->priv->requestor != None || manager->priv->contents != NULL) {
      ...
      clipboard_manager_watch_cb(manager, xev->xselectionrequest.requestor,
                                 True, StructureNotifyMask, NULL);
      XSelectInput(manager->priv->display, xev->xselectionrequest.requestor,
                   StructureNotifyMask);
      XSync(manager->priv->display, False);

      if (gdk_x11_display_error_trap_pop(display) != Success) return;

      gdk_x11_display_error_trap_push(display);

      if (xev->xselectionrequest.property != None) {
        XGetWindowProperty(
            manager->priv->display, xev->xselectionrequest.requestor,
            xev->xselectionrequest.property, 0, 0x1FFFFFFF, False, XA_ATOM,
            &type, &format, &nitems, &remaining, (unsigned char **)&targets);
      ...
      if (type == None)
        XConvertSelection(manager->priv->display, XA_CLIPBOARD, XA_TARGETS,
                          XA_TARGETS, manager->priv->window,
                          manager->priv->time);
      else
        save_targets(manager, targets, nitems);
    }
  } else if (xev->xselectionrequest.target == XA_TIMESTAMP) {
    XChangeProperty(manager->priv->display, xev->xselectionrequest.requestor,
                    xev->xselectionrequest.property, XA_INTEGER, 32,
                    PropModeReplace, (unsigned char *)&manager->priv->timestamp,
                    1);
    ...
    XChangeProperty(manager->priv->display, xev->xselectionrequest.requestor,
                    xev->xselectionrequest.property, XA_ATOM, 32,
                    PropModeReplace, (unsigned char *)targets, n_targets);
...
static void convert_clipboard_target(IncrConversion *rdata,
  ...
  if (rdata->target == XA_TARGETS) {
    ...
    if (tdata->length <= SELECTION_MAX_SIZE)
      XChangeProperty(manager->priv->display, rdata->requestor, rdata->property,
                      tdata->type, tdata->format, PropModeReplace, tdata->data,
                      items);
    else {
...
static void collect_incremental(IncrConversion *rdata,
                                MsdClipboardManager *manager) {
  if (rdata->offset >= 0)
    manager->priv->conversions =
        list_prepend(manager->priv->conversions, rdata);
  else {
    if (rdata->data) {
      target_data_unref(rdata->data);
      rdata->data = NULL;
...
static void convert_clipboard(MsdClipboardManager *manager, XEvent *xev) {
  List *list;
  List *conversions;
  IncrConversion *rdata;
  Atom type;
  ...
  if (xev->xselectionrequest.target == XA_MULTIPLE) {
    XGetWindowProperty(
        xev->xselectionrequest.display, xev->xselectionrequest.requestor,
        xev->xselectionrequest.property, 0, 0x1FFFFFFF, False, XA_ATOM_PAIR,
  ...
  if (conversions->next == NULL &&
    ...
    if (multiple) {
      ...
      XChangeProperty(xev->xselectionrequest.display,
                      ...
                      xev->xselectionrequest.property, XA_ATOM_PAIR, 32,
                      PropModeReplace, (unsigned char *)multiple, nitems);
    }
    finish_selection_request(manager, xev, True);
  }
...
static Bool clipboard_manager_process_event(MsdClipboardManager *manager,
                                            XEvent *xev) {
  Atom type;
  int format;
  unsigned long nitems;
  ...
  switch (xev->xany.type) {
    case DestroyNotify:
      if (xev->xdestroywindow.window == manager->priv->requestor) {
        list_foreach(manager->priv->contents, (Callback)target_data_unref,
                     NULL);
        list_free(manager->priv->contents);
        manager->priv->contents = NULL;

        clipboard_manager_watch_cb(manager, manager->priv->requestor, False, 0,
                                   NULL);
        manager->priv->requestor = None;
      }
      break;
    ...
    case SelectionClear:
      if (xev->xany.window != manager->priv->window) return False;
      ...
      if (xev->xselectionclear.selection == XA_CLIPBOARD_MANAGER) {
        ...
        if (manager->priv->contents) {
          ...
          XSetSelectionOwner(manager->priv->display, XA_CLIPBOARD, None,
                             manager->priv->time);
        }
      ...
      if (xev->xselection.selection == XA_CLIPBOARD) {
        ...
        if (xev->xselection.property == XA_TARGETS) {
          XGetWindowProperty(xev->xselection.display, xev->xselection.requestor,
                             xev->xselection.property, 0, 0x1FFFFFFF, True,
                             XA_ATOM, &type, &format, &nitems, &remaining,
          ...
          if (manager->priv->property != None)
            XChangeProperty(manager->priv->display, manager->priv->requestor,
                            manager->priv->property, XA_ATOM, 32,
                            PropModeReplace, (unsigned char *)&XA_NULL, 1);
...
static GdkFilterReturn clipboard_manager_event_filter(
    GdkXEvent *xevent, GdkEvent *event, MsdClipboardManager *manager) {
...
static void clipboard_manager_watch_cb(MsdClipboardManager *manager,
  ...
  if (is_start) {
    ...
    gdk_window_add_filter(gdkwin, (GdkFilterFunc)clipboard_manager_event_filter,
                          manager);
  } else {
    if (gdkwin == NULL) {
      return;
    ...
    gdk_window_remove_filter(
        gdkwin, (GdkFilterFunc)clipboard_manager_event_filter, manager);
    g_object_unref(gdkwin);
  }
}

static gboolean start_clipboard_idle_cb(MsdClipboardManager *manager) {
  ...
  manager->priv->window = XCreateSimpleWindow(
      manager->priv->display, DefaultRootWindow(manager->priv->display), 0, 0,
      10, 10, 0,
      WhitePixel(manager->priv->display, DefaultScreen(manager->priv->display)),
  ...
  XSelectInput(manager->priv->display, manager->priv->window,
               PropertyChangeMask);
  manager->priv->timestamp =
      get_server_time(manager->priv->display, manager->priv->window);

  XSetSelectionOwner(manager->priv->display, XA_CLIPBOARD_MANAGER,
                     manager->priv->window, manager->priv->timestamp);
  ...
  if (XGetSelectionOwner(manager->priv->display, XA_CLIPBOARD_MANAGER) ==
    ...
    XSendEvent(manager->priv->display,
               DefaultRootWindow(manager->priv->display), False,
               StructureNotifyMask, (XEvent *)&xev);
  } else {
    clipboard_manager_watch_cb(manager, manager->priv->window, False, 0, NULL);
...
static void msd_clipboard_manager_init(MsdClipboardManager *manager) {
