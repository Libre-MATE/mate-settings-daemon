...
static void msd_dummy_plugin_init(MsdDummyPlugin *plugin) {
...
static void msd_dummy_plugin_finalize(GObject *object) {
...
static void impl_activate(MateSettingsPlugin *plugin) {
  ...
  error = NULL;
  res =
      msd_dummy_manager_start(MSD_DUMMY_PLUGIN(plugin)->priv->manager, &error);
  if (!res) {
...
static void msd_dummy_plugin_class_init(MsdDummyPluginClass *klass) {
...
static void msd_dummy_plugin_class_finalize(MsdDummyPluginClass *klass) {}
