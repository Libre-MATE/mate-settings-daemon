...
static void system_timezone_finalize(GObject *obj) {
  int i;
  SystemTimezonePrivate *priv =
      system_timezone_get_instance_private(SYSTEM_TIMEZONE(obj));
  ...
  for (i = 0; i < CHECK_NB; i++) {
    if (priv->monitors[i]) g_object_unref(priv->monitors[i]);
...
static void system_timezone_monitor_changed(GFileMonitor *handle, GFile *file,
                                            ...
                                            gpointer user_data) {
  SystemTimezonePrivate *priv =
      system_timezone_get_instance_private(SYSTEM_TIMEZONE(user_data));
  char *new_tz;
