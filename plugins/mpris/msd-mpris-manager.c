 ...
 * User must free. */
static gchar *get_player_name(const gchar *name) {
  gchar **tokens;
  gchar *player_name;
...
static void mp_name_appeared(GDBusConnection *connection, const gchar *name,
  ...
  g_queue_push_head(manager->priv->media_player_queue, player_name);
}

/* A media player quit running and should be
 * removed from media_player_queue. */
static void mp_name_vanished(GDBusConnection *connection, const gchar *name,
                             gpointer user_data) {
  MsdMprisManager *manager = user_data;
  gchar *player_name;
  GList *player_list;

  if (g_queue_is_empty(manager->priv->media_player_queue)) return;
...
static void on_media_player_key_pressed(MsdMprisManager *manager,
                                        const gchar *key) {
  ...
  if (mpris_key != NULL) {
    ...
    proxy = g_dbus_proxy_new_for_bus_sync(G_BUS_TYPE_SESSION, flags, NULL,
                                          mpris_name, MPRIS_OBJECT_PATH,
                                          MPRIS_INTERFACE, NULL, &error);
    g_free(mpris_name);
    if (proxy == NULL) {
    ...
    g_dbus_proxy_call(proxy, mpris_key, NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL,
                      NULL, NULL);
...
static void grab_media_player_keys(MsdMprisManager *manager) {
  if (manager->priv->media_keys_proxy == NULL) return;

  g_dbus_proxy_call(manager->priv->media_keys_proxy, "GrabMediaPlayerKeys",
...
static void msd_name_appeared(GDBusConnection *connection, const gchar *name,
                              const gchar *name_owner,
                              MsdMprisManager *manager) {
  g_dbus_proxy_new_for_bus(G_BUS_TYPE_SESSION,
                           G_DBUS_PROXY_FLAGS_DO_NOT_LOAD_PROPERTIES |
                               G_DBUS_PROXY_FLAGS_DO_NOT_AUTO_START,
                           NULL, "org.mate.SettingsDaemon",
                           "/org/mate/SettingsDaemon/MediaKeys",
                           "org.mate.SettingsDaemon.MediaKeys", NULL,
                           (GAsyncReadyCallback)got_proxy_cb, manager);
}

static void msd_name_vanished(GDBusConnection *connection, const gchar *name,
                              MsdMprisManager *manager) {
  if (manager->priv->media_keys_proxy != NULL) {
...
gboolean msd_mpris_manager_start(MsdMprisManager *manager, GError **error) {
  ...
  manager->priv->namespace_watcher_id =
      bus_watch_namespace(G_BUS_TYPE_SESSION, "org.mpris.MediaPlayer2",
                          mp_name_appeared, mp_name_vanished, manager, NULL);

  manager->priv->watch_id = g_bus_watch_name(
      G_BUS_TYPE_SESSION, "org.mate.SettingsDaemon",
      G_BUS_NAME_WATCHER_FLAGS_NONE,
      (GBusNameAppearedCallback)msd_name_appeared,
      (GBusNameVanishedCallback)msd_name_vanished, manager, NULL);
...
static void msd_mpris_manager_init(MsdMprisManager *manager) {
