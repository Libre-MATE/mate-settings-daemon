...
static void namespace_watcher_stop(gpointer data) {
  ...
  if (watcher->subscription_id)
    g_dbus_connection_signal_unsubscribe(watcher->connection,
  ...
  if (watcher->connection) {
    g_signal_handlers_disconnect_by_func(watcher->connection,
  ...
  g_hash_table_remove(namespace_watcher_watchers,
                      GUINT_TO_POINTER(watcher->id));
...
static void namespace_watcher_name_appeared(NamespaceWatcher *watcher,
                                            const gchar *name,
                                            const gchar *owner) {
   ...
   */
  if (g_hash_table_contains(watcher->names, name)) return;

  g_hash_table_add(watcher->names, g_strdup(name));

  if (watcher->appeared_handler)
    watcher->appeared_handler(watcher->connection, name, owner,
                              watcher->user_data);
}

static void namespace_watcher_name_vanished(NamespaceWatcher *watcher,
                                            const gchar *name) {
  if (g_hash_table_remove(watcher->names, name) && watcher->vanished_handler)
    watcher->vanished_handler(watcher->connection, name, watcher->user_data);
}

static gboolean dbus_name_has_namespace(const gchar *name,
  ...
  if (len_name < len_namespace) return FALSE;

  if (memcmp(name_space, name, len_namespace) != 0) return FALSE;
...
static void name_owner_changed(GDBusConnection *connection,
  ...
  if (old_owner[0] != '\0') namespace_watcher_name_vanished(watcher, name);

  if (new_owner[0] != '\0')
    namespace_watcher_name_appeared(watcher, name, new_owner);
}

static void got_name_owner(GObject *object, GAsyncResult *result,
                           gpointer user_data) {
  ...
  if (reply == NULL) {
    if (!g_error_matches(error, G_DBUS_ERROR, G_DBUS_ERROR_NAME_HAS_NO_OWNER))
      g_warning(
          "bus_watch_namespace: error calling "
...
static void names_listed(GObject *object, GAsyncResult *result,
                         gpointer user_data) {
  NamespaceWatcher *watcher;
  GError *error = NULL;
  GVariant *reply;
  ...
  if (reply == NULL) {
    g_warning(
        "bus_watch_namespace: error calling org.freedesktop.DBus.ListNames: %s",
        error->message);
    g_error_free(error);
    return;
  }
...
static void got_bus(GObject *object, GAsyncResult *result, gpointer user_data) {
  GDBusConnection *connection;
  NamespaceWatcher *watcher;
  GError *error = NULL;
  ...
  g_signal_connect(watcher->connection, "closed", G_CALLBACK(connection_closed),
                   watcher);

  watcher->subscription_id = g_dbus_connection_signal_subscribe(
      watcher->connection, "org.freedesktop.DBus", "org.freedesktop.DBus",
      "NameOwnerChanged", "/org/freedesktop/DBus", watcher->name_space,
      G_DBUS_SIGNAL_FLAGS_MATCH_ARG0_NAMESPACE, name_owner_changed, watcher,
      NULL);

  g_dbus_connection_call(watcher->connection, "org.freedesktop.DBus", "/",
                         "org.freedesktop.DBus", "ListNames", NULL,
                         G_VARIANT_TYPE("(as)"), G_DBUS_CALL_FLAGS_NONE, -1,
                         watcher->cancellable, names_listed, watcher);
}
...
guint bus_watch_namespace(GBusType bus_type, const gchar *name_space,
  ...
  g_return_val_if_fail(
      name_space != NULL && g_dbus_is_interface_name(name_space), 0);
  ...
  if (namespace_watcher_watchers == NULL)
    namespace_watcher_watchers =
        g_hash_table_new(g_direct_hash, g_direct_equal);
  g_hash_table_insert(namespace_watcher_watchers, GUINT_TO_POINTER(watcher->id),
                      watcher);
...
void bus_unwatch_namespace(guint id) {
  ...
  if (namespace_watcher_watchers) {
    ...
    watcher =
        g_hash_table_lookup(namespace_watcher_watchers, GUINT_TO_POINTER(id));
    if (watcher) {
