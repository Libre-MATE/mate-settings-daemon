...
append_color_define(GString *string, const char *name, const GdkColor *color) {
...
static GdkColor *color_shade(GdkColor *a, gdouble shade, GdkColor *b) {
...
static void append_theme_colors(GtkStyle *style, GString *string) {
  GdkColor tmp;

  g_return_if_fail(style != NULL);
  g_return_if_fail(string != NULL);

  append_color_define(string, "BACKGROUND", &style->bg[GTK_STATE_NORMAL]);
  append_color_define(string, "FOREGROUND", &style->fg[GTK_STATE_NORMAL]);
  append_color_define(string, "SELECT_BACKGROUND",
                      &style->bg[GTK_STATE_SELECTED]);
  append_color_define(string, "SELECT_FOREGROUND",
                      &style->text[GTK_STATE_SELECTED]);
  append_color_define(string, "WINDOW_BACKGROUND",
                      &style->base[GTK_STATE_NORMAL]);
  append_color_define(string, "WINDOW_FOREGROUND",
                      &style->text[GTK_STATE_NORMAL]);
  append_color_define(string, "INACTIVE_BACKGROUND",
                      &style->bg[GTK_STATE_INSENSITIVE]);
  append_color_define(string, "INACTIVE_FOREGROUND",
                      &style->text[GTK_STATE_INSENSITIVE]);
  append_color_define(string, "ACTIVE_BACKGROUND",
                      &style->bg[GTK_STATE_SELECTED]);
  append_color_define(string, "ACTIVE_FOREGROUND",
                      &style->text[GTK_STATE_SELECTED]);

  append_color_define(string, "HIGHLIGHT",
 ...
 * GSList*
 */
static GSList *scan_ad_directory(const char *path, GError **error) {
  GSList *list;
  GDir *dir;
  const char *entry;
  GError *local_error;
 ...
 */
static GSList *scan_for_files(MsdXrdbManager *manager, GError **error) {
  const char *home_dir;
  GSList *user_list;
  GSList *system_list;
  GSList *list;
 ...
 */
static void append_file(const char *file, GString *string, GError **error) {
  char *contents;
 ...
 */
static void append_xresource_file(const char *filename, GString *string,
                                  GError **error) {
...
static void child_watch_cb(GPid pid, int status, gpointer user_data) {
...
static void spawn_with_input(const char *command, const char *input) {
  char **argv;
  int child_pid;
  int inpipe;
  GError *error;
  ...
  res = g_spawn_async_with_pipes(
      ...
      NULL, &child_pid, &inpipe, NULL, NULL, &error);
  g_strfreev(argv);
  ...
  g_child_watch_add(child_pid, (GChildWatchFunc)child_watch_cb,
                    (gpointer)command);
}

static void apply_settings(MsdXrdbManager *manager, GtkStyle *style) {
  const char *command;
  GString *string;
  GSList *list;
  GSList *p;
...
gboolean msd_xrdb_manager_start(MsdXrdbManager *manager, GError **error) {
  ...
  g_signal_connect(gtk_settings_get_default(), "notify::gtk-theme-name",
                   G_CALLBACK(theme_changed), manager);
...
static void msd_xrdb_manager_init(MsdXrdbManager *manager) {
