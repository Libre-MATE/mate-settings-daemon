...
static void stuff_changed(GFileMonitor *monitor, GFile *file, GFile *other_file,
                          GFileMonitorEvent event_type, gpointer handle);
...
static void monitor_files(GPtrArray *monitors, FcStrList *list, gpointer data) {
  const char *str;
...
static GPtrArray *monitors_create(gpointer data) {
...
static void monitors_free(GPtrArray *monitors) {
  if (!monitors) return;
...
static void stuff_changed(GFileMonitor *monitor G_GNUC_UNUSED,
  ...
  /* wait for quiescence */
  if (handle->timeout) g_source_remove(handle->timeout);
...
fontconfig_monitor_handle_t *fontconfig_monitor_start(GFunc notify_callback,
                                                      gpointer notify_data) {
  fontconfig_monitor_handle_t *handle =
      g_slice_new0(fontconfig_monitor_handle_t);
...
static void yay(void) { g_message("yay"); }
