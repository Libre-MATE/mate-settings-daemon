...
typedef void (*TranslationFunc)(MateXSettingsManager *manager,
                                TranslationEntry *trans, GVariant *value);
...
enum { MSD_XSETTINGS_ERROR_INIT };

static void mate_xsettings_manager_finalize(GObject *object);

G_DEFINE_TYPE_WITH_PRIVATE(MateXSettingsManager, mate_xsettings_manager,
                           G_TYPE_OBJECT)

static gpointer manager_object = NULL;

static GQuark msd_xsettings_error_quark(void) {
  return g_quark_from_static_string("msd-xsettings-error-quark");
}

static void translate_bool_int(MateXSettingsManager *manager,
                               TranslationEntry *trans, GVariant *value) {
  int i;
...
static void translate_int_int(MateXSettingsManager *manager,
                              TranslationEntry *trans, GVariant *value) {
  int i;
...
static void translate_string_string(MateXSettingsManager *manager,
                                    TranslationEntry *trans, GVariant *value) {
  int i;
...
static void translate_string_string_toolbar(MateXSettingsManager *manager,
...
static TranslationEntry translations[] = {
    ...
    {MOUSE_SCHEMA, "cursor-theme", "Gtk/CursorThemeName",
    ...
    {INTERFACE_SCHEMA, "gtk-key-theme", "Gtk/KeyThemeName",
     translate_string_string},
    {INTERFACE_SCHEMA, "toolbar-style", "Gtk/ToolbarStyle",
     translate_string_string_toolbar},
    {INTERFACE_SCHEMA, "toolbar-icons-size", "Gtk/ToolbarIconSize",
     translate_string_string},
    {INTERFACE_SCHEMA, "cursor-blink", "Net/CursorBlink", translate_bool_int},
    {INTERFACE_SCHEMA, "cursor-blink-time", "Net/CursorBlinkTime",
     translate_int_int},
    {INTERFACE_SCHEMA, "gtk-theme", "Net/ThemeName", translate_string_string},
    {INTERFACE_SCHEMA, "gtk-color-scheme", "Gtk/ColorScheme",
     translate_string_string},
    {INTERFACE_SCHEMA, "gtk-im-preedit-style", "Gtk/IMPreeditStyle",
     translate_string_string},
    {INTERFACE_SCHEMA, "gtk-im-status-style", "Gtk/IMStatusStyle",
     translate_string_string},
    {INTERFACE_SCHEMA, "gtk-im-module", "Gtk/IMModule",
     translate_string_string},
    {INTERFACE_SCHEMA, "icon-theme", "Net/IconThemeName",
     translate_string_string},
    {INTERFACE_SCHEMA, "file-chooser-backend", "Gtk/FileChooserBackend",
     translate_string_string},
    {INTERFACE_SCHEMA, "gtk-decoration-layout", "Gtk/DecorationLayout",
     translate_string_string},
    {INTERFACE_SCHEMA, "gtk-shell-shows-app-menu", "Gtk/ShellShowsAppMenu",
     translate_bool_int},
    {INTERFACE_SCHEMA, "gtk-shell-shows-menubar", "Gtk/ShellShowsMenubar",
     translate_bool_int},
    {INTERFACE_SCHEMA, "menus-have-icons", "Gtk/MenuImages",
     translate_bool_int},
    {INTERFACE_SCHEMA, "buttons-have-icons", "Gtk/ButtonImages",
     translate_bool_int},
    {INTERFACE_SCHEMA, "menubar-accel", "Gtk/MenuBarAccel",
     translate_string_string},
    {INTERFACE_SCHEMA, "show-input-method-menu", "Gtk/ShowInputMethodMenu",
     translate_bool_int},
    {INTERFACE_SCHEMA, "show-unicode-menu", "Gtk/ShowUnicodeMenu",
     translate_bool_int},
    {INTERFACE_SCHEMA, "automatic-mnemonics", "Gtk/AutoMnemonics",
     translate_bool_int},
    {INTERFACE_SCHEMA, "gtk-enable-primary-paste", "Gtk/EnablePrimaryPaste",
     translate_bool_int},
    {INTERFACE_SCHEMA, "gtk-enable-animations", "Gtk/EnableAnimations",
     translate_bool_int},
    {INTERFACE_SCHEMA, "gtk-dialogs-use-header", "Gtk/DialogsUseHeader",
     translate_bool_int},
    ...
    {SOUND_SCHEMA, "input-feedback-sounds", "Net/EnableInputFeedbackSounds",
     translate_bool_int}};

/* Auto-detect the most appropriate scale factor for the primary monitor.
 * A lot of this code is shamelessly copied and adapted from Linux
 * Mint/Cinnamon.
 */
static int get_window_scale_auto(void) {
  GdkDisplay *display;
  GdkMonitor *monitor;
  GdkRectangle rect;
  ...
  if (rect.height * monitor_scale < HIDPI_MIN_HEIGHT) return 1;

  /* Some monitors/TV encode the aspect ratio (16/9 or 16/10) instead of the
   * physical size */
  if ((width_mm == 160 && height_mm == 90) ||
      (width_mm == 160 && height_mm == 100) ||
      (width_mm == 16 && height_mm == 9) || (width_mm == 16 && height_mm == 10))
    return 1;
  ...
  if (width_mm > 0 && height_mm > 0) {
    ...
    if (dpi_x > HIDPI_LIMIT && dpi_y > HIDPI_LIMIT) window_scale = 2;
  }

  return window_scale;
}

static int get_window_scale(MateXSettingsManager *manager) {
  GSettings *gsettings;
  gint scale;
  ...
  /* Auto-detect */
  if (scale == 0) scale = get_window_scale_auto();
...
/* Read GSettings values and determine the appropriate Xft settings based on
 * them This probably could be done a bit more cleanly with g_settings_get_enum
 */
static void xft_settings_get(MateXSettingsManager *manager,
  ...
  antialiasing = g_settings_get_string(manager->priv->gsettings_font,
                                       FONT_ANTIALIASING_KEY);
  hinting =
      g_settings_get_string(manager->priv->gsettings_font, FONT_HINTING_KEY);
  rgba_order =
      g_settings_get_string(manager->priv->gsettings_font, FONT_RGBA_ORDER_KEY);
  scale = get_window_scale(manager);
  dpi =
      get_dpi_from_gsettings_or_x_server(manager->priv->gsettings_font, scale);
  ...
  settings->cursor_theme =
      g_settings_get_string(mouse_gsettings, CURSOR_THEME_KEY);
  settings->cursor_size =
      scale * g_settings_get_int(mouse_gsettings, CURSOR_SIZE_KEY);
  settings->rgba = "rgb";
  ...
  if (rgba_order) {
    ...
    if (!found) {
      g_warning("Invalid value for " FONT_RGBA_ORDER_KEY ": '%s'", rgba_order);
    }
  }
  ...
  if (hinting) {
    if (strcmp(hinting, "none") == 0) {
      ...
      g_warning("Invalid value for " FONT_HINTING_KEY ": '%s'", hinting);
    }
  }
...
static gboolean update_user_env_variable(const gchar *variable,
  ...
  reply = g_dbus_connection_call_sync(
      connection, "org.gnome.SessionManager", "/org/gnome/SessionManager",
      "org.gnome.SessionManager", "Setenv",
      g_variant_new("(ss)", variable, value), NULL, G_DBUS_CALL_FLAGS_NONE, -1,
      NULL, &bus_error);
...
static gboolean delayed_toggle_bg_draw(gpointer value) {
  GSettings *settings;

  settings = g_settings_new("org.mate.background");
  g_settings_set_boolean(settings, "show-desktop-icons",
                         GPOINTER_TO_BOOLEAN(value));
  g_object_unref(settings);

  return G_SOURCE_REMOVE;
}

static void scale_change_workarounds(MateXSettingsManager *manager,
                                     int new_scale, int unscaled_dpi) {
  if (manager->priv->window_scale == new_scale) return;
  ...
  if (!manager->priv->window_scale) {
    ...
    if (g_settings_get_boolean(gsettings, SCALING_FACTOR_QT_KEY)) {
      ...
      g_snprintf(dpibuf, sizeof(dpibuf), "%d",
                 (int)(unscaled_dpi / 1024.0 + 0.5));

      if (!update_user_env_variable("QT_FONT_DPI", dpibuf, &error)) {
        g_warning("There was a problem when setting QT_FONT_DPI=%s: %s", dpibuf,
                  error->message);
        g_clear_error(&error);
      }
      if (!update_user_env_variable("QT_SCALE_FACTOR",
                                    new_scale == 2 ? "2" : "1", &error)) {
        g_warning("There was a problem when setting QT_SCALE_FACTOR=%d: %s",
                  new_scale, error->message);
        g_clear_error(&error);
      }
    }
  } else {
    /* Restart marco */
    /* FIXME: The ideal scenario would be for marco to respect window scaling
     * and thus resize itself. Currently this is not happening, so msd restarts
     * it when the window scaling factor changes so that it's visually correct.
     */
    ...
    if (g_strcmp0(wm, WM_COMMON_MARCO) == 0) {
      ...
      if (!g_spawn_async(NULL, marco, NULL, G_SPAWN_SEARCH_PATH, NULL, NULL,
                         NULL, &error)) {
    ...
    /* Restart mate-panel */
    /* FIXME: The ideal scenario would be for mate-panel to respect window
     * scaling and thus resize itself. Currently this is not happening, so msd
     * restarts it when the window scaling factor changes so that it's visually
     * correct. */
    gchar *mate_panel[3] = {"killall", "mate-panel", NULL};
    if (!g_spawn_async(NULL, mate_panel, NULL, G_SPAWN_SEARCH_PATH, NULL, NULL,
                       NULL, &error)) {
      g_warning("There was a problem restarting mate-panel: %s",
                error->message);
      g_clear_error(&error);
    }

    /* Toggle icons on desktop to fix size */
    /* FIXME: The ideal scenario would be for caja to respect window scaling and
     * thus resize itself. Currently this is not happening, so msd restarts it
     * when the window scaling factor changes so that it's visually correct. */
    ...
    if (g_settings_get_boolean(desktop_settings, "show-desktop-icons")) {
      ...
      g_timeout_add_seconds(1, delayed_toggle_bg_draw,
                            GBOOLEAN_TO_POINTER(FALSE));
      g_timeout_add_seconds(2, delayed_toggle_bg_draw,
...
static void xft_settings_set_xsettings(MateXSettingsManager *manager,
                                       MateXftSettings *settings) {
  int i;
  ...
  for (i = 0; manager->priv->managers[i]; i++) {
    xsettings_manager_set_int(manager->priv->managers[i], "Xft/Antialias",
                              settings->antialias);
    xsettings_manager_set_int(manager->priv->managers[i], "Xft/Hinting",
                              settings->hinting);
    ...
    xsettings_manager_set_int(manager->priv->managers[i], "Gdk/UnscaledDPI",
                              settings->dpi);
    xsettings_manager_set_int(manager->priv->managers[i], "Xft/DPI",
                              settings->scaled_dpi);
    xsettings_manager_set_string(manager->priv->managers[i], "Xft/RGBA",
                                 settings->rgba);
    xsettings_manager_set_string(
        manager->priv->managers[i], "Xft/lcdfilter",
        g_str_equal(settings->rgba, "rgb") ? "lcddefault" : "none");
    xsettings_manager_set_int(manager->priv->managers[i], "Gtk/CursorThemeSize",
                              settings->cursor_size);
    xsettings_manager_set_string(manager->priv->managers[i],
                                 "Gtk/CursorThemeName", settings->cursor_theme);
  }
...
static void update_property(GString *props, const gchar *key,
                            const gchar *value) {
  gchar *needle;
  size_t needle_len;
  gchar *found = NULL;
...
static void xft_settings_set_xresources(MateXftSettings *settings) {
  GString *add_string;
  char dpibuf[G_ASCII_DTOSTR_BUF_SIZE];
  Display *dpy;
  ...
  g_snprintf(dpibuf, sizeof(dpibuf), "%d",
             (int)(settings->scaled_dpi / 1024.0 + 0.5));
  update_property(add_string, "Xft.dpi", dpibuf);
  update_property(add_string, "Xft.antialias", settings->antialias ? "1" : "0");
  update_property(add_string, "Xft.hinting", settings->hinting ? "1" : "0");
  update_property(add_string, "Xft.hintstyle", settings->hintstyle);
  update_property(add_string, "Xft.rgba", settings->rgba);
  update_property(add_string, "Xft.lcdfilter",
                  g_str_equal(settings->rgba, "rgb") ? "lcddefault" : "none");
  update_property(add_string, "Xcursor.theme", settings->cursor_theme);
  update_property(
      add_string, "Xcursor.size",
  ...
  XChangeProperty(dpy, RootWindow(dpy, 0), XA_RESOURCE_MANAGER, XA_STRING, 8,
                  PropModeReplace, (unsigned char *)add_string->str,
                  add_string->len);
  XCloseDisplay(dpy);
 ...
 * X resources
 */
static void update_xft_settings(MateXSettingsManager *manager) {
  MateXftSettings settings;
...
static void fontconfig_callback(fontconfig_monitor_handle_t *handle,
  ...
  for (i = 0; manager->priv->managers[i]; i++) {
    xsettings_manager_set_int(manager->priv->managers[i],
                              "Fontconfig/Timestamp", timestamp);
    xsettings_manager_notify(manager->priv->managers[i]);
  }
  mate_settings_profile_end(NULL);
}

static gboolean start_fontconfig_monitor_idle_cb(
    MateXSettingsManager *manager) {
  mate_settings_profile_start(NULL);

  manager->priv->fontconfig_handle =
      fontconfig_monitor_start((GFunc)fontconfig_callback, manager);
...
static void start_fontconfig_monitor(MateXSettingsManager *manager) {
  mate_settings_profile_start(NULL);
...
static void stop_fontconfig_monitor(MateXSettingsManager *manager) {
...
static void process_value(MateXSettingsManager *manager,
                          TranslationEntry *trans, GVariant *value) {
...
static gboolean setup_xsettings_managers(MateXSettingsManager *manager) {
  GdkDisplay *display;
  gboolean res;
  gboolean terminated;
  ...
  res = xsettings_manager_check_running(
  ...
  manager->priv->managers[0] = xsettings_manager_new(
      gdk_x11_display_get_xdisplay(display),
      gdk_x11_screen_get_screen_number(screen), terminate_cb, &terminated);
...
gboolean mate_xsettings_manager_start(MateXSettingsManager *manager,
  ...
  if (!setup_xsettings_managers(manager)) {
    g_set_error(error, MSD_XSETTINGS_ERROR, MSD_XSETTINGS_ERROR_INIT,
                "Could not initialize xsettings manager.");
    return FALSE;
  }

  manager->priv->gsettings = g_hash_table_new_full(
      g_str_hash, g_str_equal, NULL, (GDestroyNotify)g_object_unref);

  g_hash_table_insert(manager->priv->gsettings, MOUSE_SCHEMA,
                      g_settings_new(MOUSE_SCHEMA));
  g_hash_table_insert(manager->priv->gsettings, INTERFACE_SCHEMA,
                      g_settings_new(INTERFACE_SCHEMA));
  g_hash_table_insert(manager->priv->gsettings, SOUND_SCHEMA,
  ...
  for (i = 0; i < G_N_ELEMENTS(translations); i++) {
    ...
    if (gsettings == NULL) {
      g_warning("Schemas '%s' has not been setup",
                translations[i].gsettings_schema);
      continue;
    }
  ...
  g_signal_connect(screen, "size-changed",
                   G_CALLBACK(recalculate_scale_callback), manager);
  g_signal_connect(screen, "monitors-changed",
                   G_CALLBACK(recalculate_scale_callback), manager);

  manager->priv->gsettings_font = g_settings_new(FONT_RENDER_SCHEMA);
  g_signal_connect(manager->priv->gsettings_font, "changed",
...
static void mate_xsettings_manager_class_init(
...
static void mate_xsettings_manager_init(MateXSettingsManager *manager) {
