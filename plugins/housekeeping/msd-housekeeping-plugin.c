...
MATE_SETTINGS_PLUGIN_REGISTER_WITH_PRIVATE(MsdHousekeepingPlugin,
                                           msd_housekeeping_plugin)

static void msd_housekeeping_plugin_init(MsdHousekeepingPlugin *plugin) {
...
static void msd_housekeeping_plugin_finalize(GObject *object) {
...
static void impl_activate(MateSettingsPlugin *plugin) {
  ...
  error = NULL;
  res = msd_housekeeping_manager_start(
      MSD_HOUSEKEEPING_PLUGIN(plugin)->priv->manager, &error);
  if (!res) {
...
static void msd_housekeeping_plugin_class_init(
    MsdHousekeepingPluginClass *klass) {
...
static void msd_housekeeping_plugin_class_finalize(
    MsdHousekeepingPluginClass *klass) {}
