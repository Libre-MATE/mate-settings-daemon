 ...
 *
 * msd-ldsm-trash-empty.c is free software: you can redistribute it and/or
...
/* Some of this code has been borrowed from the trash-applet, courtesy of Ryan
 * Lortie */
...
static gboolean trash_empty_done(gpointer data) {
...
static gboolean trash_empty_update_dialog(gpointer user_data) {
  ...
  if (!trash_empty_dialog) goto out;

  if (!actually_deleting) {
    /* If we havent finished counting yet, then pulse the progressbar every
     * 100ms. This stops the user from thinking the dialog has frozen if there
     * are a lot of files to delete. We don't pulse it every time we are called
     * from the worker thread, otherwise it moves to fast and looks hideous
     */
    if (timer) {
    ...
    text = g_strdup_printf(_("Removing item %lu of %lu"), deleted, total);
...
static void trash_empty_delete_contents(GIOSchedulerJob *job,
  ...
  if (enumerator) {
    while ((info = g_file_enumerator_next_file(enumerator, cancellable,
      ...
      if (actually_delete) g_file_delete(child, cancellable, NULL);
...
static void trash_empty_start(void) {
  ...
  gtk_window_set_title(GTK_WINDOW(trash_empty_dialog), _("Emptying the trash"));
  ...
  gtk_progress_bar_set_text(GTK_PROGRESS_BAR(progressbar),
                            _("Preparing to empty trash…"));

  gtk_box_pack_start(
      GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(trash_empty_dialog))),
      vbox1, TRUE, TRUE, 0);
  ...
  gtk_container_set_border_width(GTK_CONTAINER(gtk_dialog_get_content_area(
                                     GTK_DIALOG(trash_empty_dialog))),
                                 6);
  gtk_container_set_border_width(GTK_CONTAINER(vbox1), 6);

  gtk_dialog_add_button(GTK_DIALOG(trash_empty_dialog), GTK_STOCK_CANCEL,
                        GTK_RESPONSE_CANCEL);

  markup =
      g_markup_printf_escaped("<big><b>%s</b></big>", _("Emptying the trash"));
...
static gboolean trash_empty_require_confirmation(void) {
  ...
  require_confirmation =
      g_settings_get_boolean(settings, CAJA_CONFIRM_TRASH_KEY);
  g_object_unref(settings);

  return require_confirmation;
}

static void trash_empty_show_confirmation_dialog(void) {
  GtkWidget *button;
  ...
  gtk_dialog_add_button(GTK_DIALOG(trash_empty_confirm_dialog),
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(trash_empty_confirm_dialog), button,
                               GTK_RESPONSE_YES);
