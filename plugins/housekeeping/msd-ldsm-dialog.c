...
static const gchar *msd_ldsm_dialog_get_checkbutton_text(
...
static gchar *msd_ldsm_dialog_get_primary_text(MsdLdsmDialog *dialog) {
  ...
  if (dialog->priv->other_partitions) {
    primary_text = g_strdup_printf(
        _("The volume \"%s\" has only %s disk space remaining."),
        dialog->priv->partition_name, free_space);
  } else {
    primary_text = g_strdup_printf(
        _("This computer has only %s disk space remaining."), free_space);
  }
...
static const gchar *msd_ldsm_dialog_get_secondary_text(MsdLdsmDialog *dialog) {
  g_return_val_if_fail(MSD_IS_LDSM_DIALOG(dialog), NULL);

  if (dialog->priv->other_usable_partitions) {
    if (dialog->priv->has_trash) {
      return _(
          "You can free up disk space by emptying the Trash, removing "
          "unused programs or files, or moving files to another disk or "
          "partition.");
    } else {
      return _(
          "You can free up disk space by removing unused programs or files, "
...
static gint ignore_path_compare(gconstpointer a, gconstpointer b) {
...
static void ignore_check_button_toggled_cb(GtkToggleButton *button,
                                           gpointer user_data) {
  ...
  if (i > 0) ignore_paths = g_slist_reverse(ignore_paths);

  ignore = gtk_toggle_button_get_active(button);
  updated =
      update_ignore_paths(&ignore_paths, dialog->priv->mount_path, ignore);
  ...
  if (updated) {
    ...
    if (!g_settings_set_strv(settings, SETTINGS_IGNORE_PATHS,
...
static void msd_ldsm_dialog_init(MsdLdsmDialog *dialog) {
  ...
  gtk_label_set_single_line_mode(GTK_LABEL(dialog->priv->secondary_label),
                                 FALSE);
   ...
   * I suppose it could be possible for the user to manually edit the GSettings
   * key between the mount being checked and the dialog appearing, but I don't
   * think it matters too much */
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(dialog->priv->ignore_check_button), FALSE);
  ...
  gtk_box_pack_start(GTK_BOX(text_vbox), dialog->priv->primary_label, FALSE,
                     FALSE, 0);
  gtk_box_pack_start(GTK_BOX(text_vbox), dialog->priv->secondary_label, TRUE,
                     TRUE, 0);
  gtk_box_pack_start(GTK_BOX(text_vbox), dialog->priv->ignore_check_button,
                     FALSE, FALSE, 0);
...
static void msd_ldsm_dialog_finalize(GObject *object) {
  MsdLdsmDialog *self;
  ...
  if (self->priv->partition_name) g_free(self->priv->partition_name);

  if (self->priv->mount_path) g_free(self->priv->mount_path);

  G_OBJECT_CLASS(msd_ldsm_dialog_parent_class)->finalize(object);
}

static void msd_ldsm_dialog_set_property(GObject *object, guint prop_id,
                                         const GValue *value,
                                         GParamSpec *pspec) {
  MsdLdsmDialog *self;
...
static void msd_ldsm_dialog_get_property(GObject *object, guint prop_id,
...
static void msd_ldsm_dialog_class_init(MsdLdsmDialogClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_OTHER_USABLE_PARTITIONS,
      g_param_spec_boolean(
          "other-usable-partitions", "other-usable-partitions",
          "Set to TRUE if there are other usable partitions on the system",
          FALSE, G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_OTHER_PARTITIONS,
      g_param_spec_boolean(
          "other-partitions", "other-partitions",
          "Set to TRUE if there are other partitions on the system", FALSE,
          G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_HAS_TRASH,
      g_param_spec_boolean("has-trash", "has-trash",
                           "Set to TRUE if the partition has files in it's "
                           "trash folder that can be deleted",
                           FALSE, G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_SPACE_REMAINING,
      g_param_spec_int64("space-remaining", "space-remaining",
                         "Specify how much space is remaining in bytes",
                         G_MININT64, G_MAXINT64, 0,
                         G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_PARTITION_NAME,
      g_param_spec_string("partition-name", "partition-name",
                          "Specify the name of the partition", "Unknown",
                          G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_MOUNT_PATH,
      g_param_spec_string("mount-path", "mount-path",
                          "Specify the mount path for the partition", "Unknown",
                          G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
}
...
MsdLdsmDialog *msd_ldsm_dialog_new(gboolean other_usable_partitions,
  ...
  dialog = MSD_LDSM_DIALOG(g_object_new(
      MSD_TYPE_LDSM_DIALOG, "other-usable-partitions", other_usable_partitions,
      "other-partitions", other_partitions, "has-trash", display_empty_trash,
      "space-remaining", space_remaining, "partition-name", partition_name,
      "mount-path", mount_path, NULL));
  ...
  button_ignore = gtk_dialog_add_button(GTK_DIALOG(dialog), _("Ignore"),
                                        GTK_RESPONSE_CANCEL);
  ignore_image =
  ...
  primary_text_markup =
      g_markup_printf_escaped("<big><b>%s</b></big>", primary_text);
  gtk_label_set_markup(GTK_LABEL(dialog->priv->primary_label),
                       primary_text_markup);
  ...
  gtk_button_set_label(GTK_BUTTON(dialog->priv->ignore_check_button),
                       checkbutton_text);
