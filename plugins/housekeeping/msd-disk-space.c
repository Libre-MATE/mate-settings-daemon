...
/* gcc -DHAVE_LIBNOTIFY -DTEST -Wall `pkg-config --cflags --libs gobject-2.0
...
#define SETTINGS_HOUSEKEEPING_SCHEMA \
...
static gchar *ldsm_get_fs_id_for_path(const gchar *path) {
  ...
  if (fileinfo) {
    attr_id_fs = g_strdup(g_file_info_get_attribute_string(
        fileinfo, G_FILE_ATTRIBUTE_ID_FILESYSTEM));
...
static gboolean ldsm_mount_has_trash(LdsmMountInfo *mount) {
  ...
  /* I can't think of a better way to find out if a volume has any trash. Any
   * suggestions? */
  if (mount_uses_user_trash) {
    trash_files_dir =
        g_build_filename(g_get_user_data_dir(), "Trash", "files", NULL);
  } else {
    gchar *uid;
  ...
  if (dir) {
    if (g_dir_read_name(dir)) has_trash = TRUE;
    g_dir_close(dir);
  }
...
static gboolean ldsm_notify_for_mount(LdsmMountInfo *mount,
  ...
  dialog =
      msd_ldsm_dialog_new(other_usable_volumes, multiple_volumes,
                          has_disk_analyzer, has_trash, free_space, name, path);
...
static gboolean ldsm_mount_has_space(LdsmMountInfo *mount) {
  gdouble free_space;

  free_space = (double)mount->buf.f_bavail / (double)mount->buf.f_blocks;
  /* enough free space, nothing to do */
  if (free_space > free_percent_notify) return TRUE;

  if (((gint64)mount->buf.f_frsize * (gint64)mount->buf.f_bavail) >
      ((gint64)free_size_gb_no_notify * GIGABYTE))
    return TRUE;
...
static gboolean ldsm_mount_is_virtual(LdsmMountInfo *mount) {
...
static gint ldsm_ignore_path_compare(gconstpointer a, gconstpointer b) {
  return g_strcmp0((const gchar *)a, (const gchar *)b);
}

static gboolean ldsm_mount_is_user_ignore(const gchar *path) {
  if (g_slist_find_custom(ignore_paths, path,
                          (GCompareFunc)ldsm_ignore_path_compare) != NULL)
    return TRUE;
  else
    return FALSE;
}

static gboolean is_in(const gchar *value, const gchar *set[]) {
  int i;
...
static gboolean ldsm_mount_should_ignore(GUnixMountEntry *mount) {
  const gchar *fs, *device, *path;

  path = g_unix_mount_get_mount_path(mount);
  if (ldsm_mount_is_user_ignore(path)) return TRUE;
  ...
  const gchar *ignore_fs[] = {
      "adfs",   "afs",    "auto",       "autofs",    "autofs4", "cifs",
      "cxfs",   "devfs",  "devpts",     "ecryptfs",  "fdescfs", "gfs",
      "gfs2",   "kernfs", "linprocfs",  "linsysfs",  "lustre",  "lustre_lite",
      "ncpfs",  "nfs",    "nfs4",       "nfsd",      "ocfs2",   "proc",
      "procfs", "ptyfs",  "rpc_pipefs", "selinuxfs", "smbfs",   "sysfs",
      "tmpfs",  "usbfs",  "zfs",        NULL};
  const gchar *ignore_devices[] = {"none",      "sunrpc",  "devpts", "nfsd",
...
static void ldsm_free_mount_info(gpointer data) {
...
static void ldsm_maybe_warn_mounts(GList *mounts, gboolean multiple_volumes,
                                   gboolean other_usable_volumes) {
  GList *l;
  gboolean done = FALSE;
  ...
  for (l = mounts; l != NULL; l = l->next) {
    ...
    if (done) {
      /* Don't show any more dialogs if the user took action with the last one.
       * The user action might free up space on multiple volumes, making the
       * next dialog redundant.
       */
    ...
    if (previous_mount_info != NULL)
      previous_free_space = (gdouble)previous_mount_info->buf.f_bavail /
                            (gdouble)previous_mount_info->buf.f_blocks;

    free_space =
        (gdouble)mount_info->buf.f_bavail / (gdouble)mount_info->buf.f_blocks;
    ...
    if (previous_mount_info == NULL) {
      ...
      /* We've notified for this mount before and free space has decreased
       * sufficiently since last time to notify again */
      curr_time = time(NULL);
      if (difftime(curr_time, previous_mount_info->notify_time) >
          (gdouble)(min_notify_period * 60)) {
        show_notify = TRUE;
        mount_info->notify_time = curr_time;
      } else {
        /* It's too soon to show the dialog again. However, we still replace the
         * LdsmMountInfo struct in the hash table, but give it the notfiy time
         * from the previous dialog. This will stop the notification from
         * reappearing unnecessarily as soon as the timeout expires.
         */
        show_notify = FALSE;
      ...
      /* We've notified for this mount before, but the free space hasn't
       * decreased sufficiently to notify again */
      ldsm_free_mount_info(mount_info);
      show_notify = FALSE;
    }

    if (show_notify) {
      if (ldsm_notify_for_mount(mount_info, multiple_volumes,
...
static gboolean ldsm_check_all_mounts(gpointer data) {
  GList *mounts;
  GList *l;
   ...
   * they're mounted by checking if the GUnixMountPoint has a corresponding
   * GUnixMountEntry. Iterating through the static mounts means we automatically
   * ignore dynamically mounted media.
   */
  ...
  if (number_of_mounts > 1) multiple_volumes = TRUE;
  ...
  for (l = check_mounts; l != NULL; l = l->next) {
    ...
    if (!ldsm_mount_has_space(mount_info)) {
      ...
      g_hash_table_remove(ldsm_notified_hash,
                          g_unix_mount_get_mount_path(mount_info->mount));
      ldsm_free_mount_info(mount_info);
    }
  }

  number_of_full_mounts = g_list_length(full_mounts);
  if (number_of_mounts > number_of_full_mounts) other_usable_volumes = TRUE;

  ldsm_maybe_warn_mounts(full_mounts, multiple_volumes, other_usable_volumes);
...
static gboolean ldsm_is_hash_item_not_in_mounts(gpointer key, gpointer value,
                                                gpointer user_data) {
  GList *l;
  ...
  for (l = (GList *)user_data; l != NULL; l = l->next) {
    ...
    if (strcmp(path, key) == 0) return FALSE;
  }
...
static void ldsm_mounts_changed(GObject *monitor G_GNUC_UNUSED,
  ...
  if (ldsm_timeout_id) g_source_remove(ldsm_timeout_id);
  ldsm_timeout_id =
      g_timeout_add_seconds(CHECK_EVERY_X_SECONDS, ldsm_check_all_mounts, NULL);
}

static gboolean ldsm_is_hash_item_in_ignore_paths(gpointer key, gpointer value,
                                                  gpointer user_data) {
  return ldsm_mount_is_user_ignore(key);
}

static void msd_ldsm_get_config(void) {
  gchar **settings_list;

  free_percent_notify =
      g_settings_get_double(settings, SETTINGS_FREE_PC_NOTIFY_KEY);
  if (free_percent_notify >= 1 || free_percent_notify < 0) {
    /* FIXME define min and max in gschema! */
    g_warning(
        "Invalid configuration of free_percent_notify: %f\n"
  ...
  free_percent_notify_again =
      g_settings_get_double(settings, SETTINGS_FREE_PC_NOTIFY_AGAIN_KEY);
  if (free_percent_notify_again >= 1 || free_percent_notify_again < 0) {
    /* FIXME define min and max in gschema! */
    g_warning(
        "Invalid configuration of free_percent_notify_again: %f\n"
        "Using sensible default\n",
        free_percent_notify_again);
  ...
  free_size_gb_no_notify =
      g_settings_get_int(settings, SETTINGS_FREE_SIZE_NO_NOTIFY);
  ...
  if (settings_list != NULL) {
    ...
    for (i = 0; settings_list[i] != NULL; i++) {
      if (settings_list[i] != NULL)
        ignore_paths =
            g_slist_prepend(ignore_paths, g_strdup(settings_list[i]));
    }
...
void msd_ldsm_setup(gboolean check_now) {
  ...
  ldsm_notified_hash = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
                                             ldsm_free_mount_info);

  settings = g_settings_new(SETTINGS_HOUSEKEEPING_SCHEMA);
  msd_ldsm_get_config();
  g_signal_connect(settings, "changed", G_CALLBACK(msd_ldsm_update_config),
                   NULL);
  ...
  if (check_now) ldsm_check_all_mounts(NULL);
...
void msd_ldsm_clean(void) {
  ...
  if (ldsm_notified_hash) g_hash_table_destroy(ldsm_notified_hash);
  ldsm_notified_hash = NULL;

  if (ldsm_monitor) g_object_unref(ldsm_monitor);
  ldsm_monitor = NULL;
