...
#define CONF_KEY_TURN_ON_EXTERNAL_MONITORS_AT_STARTUP \
  "turn-on-external-monitors-at-startup"
#define CONF_KEY_TURN_ON_LAPTOP_MONITOR_AT_STARTUP \
  "turn-on-laptop-monitor-at-startup"
...
struct MsdXrandrManagerPrivate {
  ...
  /* Key code of the XF86Display key (Fn-F7 on Thinkpads, Fn-F4 on HP machines,
   * etc.) */
  ...
  MateRRConfig *
      *fn_f7_configs; /* NULL terminated, NULL if there are no configs */

  /* Last time at which we got a "screen got reconfigured" event; see
   * on_randr_event() */
  guint32 last_config_timestamp;
};

static const MateRRRotation possible_rotations[] = {
    MATE_RR_ROTATION_0, MATE_RR_ROTATION_90, MATE_RR_ROTATION_180,
    MATE_RR_ROTATION_270
    /* We don't allow REFLECT_X or REFLECT_Y for now, as mate-display-properties
       doesn't allow them, either */
};

static void msd_xrandr_manager_finalize(GObject *object);

static void error_message(MsdXrandrManager *mgr, const char *primary_text,
                          GError *error_to_display, const char *secondary_text);

static void status_icon_popup_menu(MsdXrandrManager *manager, guint button,
                                   guint32 timestamp);
...
static void log_open(void) {
  ...
  log_filename =
      g_build_filename(g_get_home_dir(), "msd-debug-randr.log", NULL);

  if (stat(toggle_filename, &st) != 0) goto out;
  ...
  if (log_file && ftell(log_file) == 0)
    fprintf(
        log_file,
        "To keep this log from being created, please rm ~/msd-debug-randr\n");
...
static void log_output(MateRROutputInfo *output) {
  ...
  if (mate_rr_output_info_is_connected(output)) {
    if (mate_rr_output_info_is_active(output)) {
      ...
      log_msg("%dx%d@%d +%d+%d", width, height,
              mate_rr_output_info_get_refresh_rate(output), x, y);
    } else
      log_msg("off");
  } else
    log_msg("disconnected");

  if (display_name) log_msg(" (%s)", display_name);

  if (mate_rr_output_info_get_primary(output)) log_msg(" (primary output)");
...
static void log_configuration(MateRRConfig *config) {
  ...
  for (i = 0; outputs[i] != NULL; i++) log_output(outputs[i]);

  if (i == 0) log_msg("        no outputs!\n");
}

static char timestamp_relationship(guint32 a, guint32 b) {
  if (a < b)
    return '<';
  else if (a > b)
    return '>';
...
static void log_screen(MateRRScreen *screen) {
  ...
  log_msg("        Screen min(%d, %d), max(%d, %d), change=%u %c config=%u\n",
          min_w, min_h, max_w, max_h, change_timestamp,
...
static void show_timestamps_dialog(MsdXrandrManager *manager, const char *msg) {
#if 1
  return;
#else
  ...
  mate_rr_screen_get_timestamps(priv->rw_screen, &change_timestamp,
                                &config_timestamp);

  dialog = gtk_message_dialog_new(
      NULL, 0, GTK_MESSAGE_INFO, GTK_BUTTONS_CLOSE,
      "RANDR timestamps (%d):\n%s\nchange: %u\nconfig: %u", serial++, msg,
      change_timestamp, config_timestamp);
  g_signal_connect(dialog, "response", G_CALLBACK(gtk_widget_destroy), NULL);
  gtk_widget_show(dialog);
#endif
}

/* This function centralizes the use of
 * mate_rr_config_apply_from_filename_with_time().
 *
 * Optionally filters out MATE_RR_ERROR_NO_MATCHING_CONFIG from
 * mate_rr_config_apply_from_filename_with_time(), since that is not usually an
 * error.
 */
static gboolean apply_configuration_from_filename(
    MsdXrandrManager *manager, const char *filename,
    gboolean no_matching_config_is_an_error, guint32 timestamp,
    GError **error) {
  struct MsdXrandrManagerPrivate *priv = manager->priv;
  GError *my_error;
  gboolean success;
  ...
  my_error = NULL;
  success = mate_rr_config_apply_from_filename_with_time(
      priv->rw_screen, filename, timestamp, &my_error);
  if (success) return TRUE;

  if (g_error_matches(my_error, MATE_RR_ERROR,
                      MATE_RR_ERROR_NO_MATCHING_CONFIG)) {
    if (no_matching_config_is_an_error) goto fail;
 ...
 */
static gboolean apply_configuration_and_display_error(MsdXrandrManager *manager,
                                                      MateRRConfig *config,
                                                      guint32 timestamp) {
  ...
  error = NULL;
  success = mate_rr_config_apply_with_time(config, priv->rw_screen, timestamp,
                                           &error);
  if (!success) {
    log_msg(
        "Could not switch to the following configuration (timestamp %u): %s\n",
        timestamp, error->message);
    log_configuration(config);
    error_message(manager, _("Could not switch the monitor configuration"),
                  error, NULL);
    g_error_free(error);
  }
...
static void restore_backup_configuration_without_messages(
    const char *backup_filename, const char *intended_filename) {
  backup_filename = mate_rr_config_get_backup_filename();
  rename(backup_filename, intended_filename);
}

static void restore_backup_configuration(MsdXrandrManager *manager,
  ...
  if (rename(backup_filename, intended_filename) == 0) {
    ...
    error = NULL;
    if (!apply_configuration_from_filename(manager, intended_filename, FALSE,
                                           timestamp, &error)) {
      error_message(manager, _("Could not restore the display's configuration"),
                    error, NULL);
  ...
  if (saved_errno == ENOENT)
    ...
    msg = g_strdup_printf("Could not rename %s to %s: %s", backup_filename,
                          intended_filename, g_strerror(saved_errno));
    error_message(
        manager,
        _("Could not restore the display's configuration from a backup"), NULL,
        msg);
    g_free(msg);
  }
...
static void print_countdown_text(TimeoutDialog *timeout) {
  gtk_message_dialog_format_secondary_text(
      GTK_MESSAGE_DIALOG(timeout->dialog),
      ngettext("The display will be reset to its previous configuration in %d "
               "second",
...
static gboolean user_says_things_are_ok(MsdXrandrManager *manager,
  ...
  timeout.dialog =
      gtk_message_dialog_new(NULL, GTK_DIALOG_MODAL, GTK_MESSAGE_QUESTION,
                             GTK_BUTTONS_NONE, _("Does the display look OK?"));

  timeout.countdown = CONFIRMATION_DIALOG_SECONDS;

  print_countdown_text(&timeout);

  gtk_window_set_icon_name(GTK_WINDOW(timeout.dialog),
                           "preferences-desktop-display");
  gtk_dialog_add_button(GTK_DIALOG(timeout.dialog),
                        _("_Restore Previous Configuration"),
                        GTK_RESPONSE_CANCEL);
  gtk_dialog_add_button(GTK_DIALOG(timeout.dialog),
                        _("_Keep This Configuration"), GTK_RESPONSE_ACCEPT);
  gtk_dialog_set_default_response(GTK_DIALOG(timeout.dialog),
                                  GTK_RESPONSE_ACCEPT); /* ah, the optimism */

  g_signal_connect(timeout.dialog, "response", G_CALLBACK(timeout_response_cb),
                   &timeout);

  gtk_widget_realize(timeout.dialog);

  if (parent_window)
    gdk_window_set_transient_for(gtk_widget_get_window(timeout.dialog),
                                 parent_window);

  gtk_widget_show_all(timeout.dialog);
  /* We don't use g_timeout_add_seconds() since we actually care that the user
   * sees "real" second ticks in the dialog */
  timeout_id = g_timeout_add(1000, timeout_cb, &timeout);
  gtk_main();
...
static gboolean confirm_with_user_idle_cb(gpointer data) {
  ...
  if (user_says_things_are_ok(confirmation->manager,
                              confirmation->parent_window))
    unlink(backup_filename);
  else
    restore_backup_configuration(confirmation->manager, backup_filename,
                                 intended_filename, confirmation->timestamp);
...
static void queue_confirmation_by_user(MsdXrandrManager *manager,
                                       GdkWindow *parent_window,
                                       guint32 timestamp) {
...
static gboolean try_to_apply_intended_configuration(MsdXrandrManager *manager,
                                                    GdkWindow *parent_window,
                                                    guint32 timestamp,
                                                    GError **error) {
  char *backup_filename;
  ...
  result = apply_configuration_from_filename(manager, intended_filename, FALSE,
                                             timestamp, error);
  if (!result) {
    error_message(
        manager,
        _("The selected configuration for displays could not be applied"),
        error ? *error : NULL, NULL);
    restore_backup_configuration_without_messages(backup_filename,
                                                  intended_filename);
    goto out;
  } else {
...
/* DBus method for org.mate.SettingsDaemon.XRANDR ApplyConfiguration; see
 * msd-xrandr-manager.xml for the interface definition */
static gboolean msd_xrandr_manager_apply_configuration(
    MsdXrandrManager *manager, GError **error) {
  return try_to_apply_intended_configuration(manager, NULL, GDK_CURRENT_TIME,
                                             error);
}

/* DBus method for org.mate.SettingsDaemon.XRANDR_2 ApplyConfiguration; see
 * msd-xrandr-manager.xml for the interface definition */
static gboolean msd_xrandr_manager_2_apply_configuration(
    MsdXrandrManager *manager, gint64 parent_window_id, gint64 timestamp,
    GError **error) {
  GdkWindow *parent_window;
  gboolean result;
  ...
  if (parent_window_id != 0)
    parent_window = gdk_x11_window_foreign_new_for_display(
        gdk_display_get_default(), (Window)parent_window_id);
  else
    parent_window = NULL;

  result = try_to_apply_intended_configuration(manager, parent_window,
                                               (guint32)timestamp, error);

  if (parent_window) g_object_unref(parent_window);

  return result;
}

/* We include this after the definition of
 * msd_xrandr_manager_apply_configuration() so the prototype will already exist
 */
...
static gboolean is_laptop(MateRRScreen *screen, MateRROutputInfo *output) {
  ...
  rr_output = mate_rr_screen_get_output_by_name(
      screen, mate_rr_output_info_get_name(output));
...
static void print_output(MateRROutputInfo *info) {
  ...
  g_print("  Output: %s attached to %s\n",
          mate_rr_output_info_get_display_name(info),
          mate_rr_output_info_get_name(info));
  g_print("     status: %s\n",
...
static void print_configuration(MateRRConfig *config, const char *header) {
  ...
  for (i = 0; outputs[i] != NULL; ++i) print_output(outputs[i]);
}
...
static MateRRConfig *make_clone_setup(MateRRScreen *screen) {
  ...
  if (!get_clone_size(screen, &width, &height)) return NULL;
  ...
  for (i = 0; outputs[i] != NULL; ++i) {
    ...
    if (mate_rr_output_info_is_connected(info)) {
      MateRROutput *output = mate_rr_screen_get_output_by_name(
          screen, mate_rr_output_info_get_name(info));
...
static gboolean turn_on(MateRRScreen *screen, MateRROutputInfo *info, int x,
                        int y) {
  MateRROutput *output = mate_rr_screen_get_output_by_name(
      screen, mate_rr_output_info_get_name(info));
  ...
  if (mode) {
    mate_rr_output_info_set_active(info, TRUE);
    mate_rr_output_info_set_geometry(info, x, y, mate_rr_mode_get_width(mode),
...
static MateRRConfig *make_primary_only_setup(MateRRScreen *screen) {
  /*Leave all of the monitors turned on, just change from mirror to xinerama
   * layout*/
...
static int turn_on_and_get_rightmost_offset(MateRRScreen *screen,
                                            MateRROutputInfo *info, int x) {
...
static MateRRConfig *make_other_setup(MateRRScreen *screen) {
  ...
  for (i = 0; outputs[i] != NULL; ++i) {
    ...
    if (is_laptop(screen, info)) {
      ...
      if (mate_rr_output_info_is_connected(info)) turn_on(screen, info, 0, 0);
    }
  }
...
static GPtrArray *sanitize(MsdXrandrManager *manager, GPtrArray *array) {
  int i;
  GPtrArray *new;
  ...
  for (i = 0; i < array->len; i++) {
    ...
    if (config) {
      ...
      error = NULL;
      if (!mate_rr_config_applicable(config, manager->priv->rw_screen,
                                     &error)) { /* NULL-GError */
        g_debug("removing configuration which is not applicable because %s",
                error->message);
        g_error_free(error);
...
static void generate_fn_f7_configs(MsdXrandrManager *mgr) {
...
static void handle_fn_f7(MsdXrandrManager *mgr, guint32 timestamp) {
  MsdXrandrManagerPrivate *priv = mgr->priv;
  ...
  if (!mate_rr_screen_refresh(screen, &error) && error) {
    ...
    str = g_strdup_printf(_("Could not refresh the screen information: %s"),
                          error->message);
    g_error_free(error);

    log_msg("%s\n", str);
    error_message(mgr, str, NULL,
                  _("Trying to switch the monitor configuration anyway."));
    g_free(str);
  }
  ...
  if (priv->fn_f7_configs &&
      (!mate_rr_config_match(current, priv->fn_f7_configs[0]) ||
       !mate_rr_config_equal(
           current, priv->fn_f7_configs[mgr->priv->current_fn_f7_config]))) {
  ...
  if (priv->fn_f7_configs) {
    ...
    g_debug("cycling to next configuration (%d)",
            mgr->priv->current_fn_f7_config);

    print_configuration(priv->fn_f7_configs[mgr->priv->current_fn_f7_config],
                        "new config");
    ...
    if (timestamp < server_timestamp) timestamp = server_timestamp;

    success = apply_configuration_and_display_error(
        mgr, priv->fn_f7_configs[mgr->priv->current_fn_f7_config], timestamp);
...
static MateRROutputInfo *get_laptop_output_info(MateRRScreen *screen,
                                                MateRRConfig *config) {
  int i;
  MateRROutputInfo **outputs = mate_rr_config_get_outputs(config);

  for (i = 0; outputs[i] != NULL; i++) {
    if (is_laptop(screen, outputs[i])) return outputs[i];
  }

  return NULL;
}

static MateRRRotation get_next_rotation(MateRRRotation allowed_rotations,
  ...
  if (current_index == -1) {
    /* Huh, the current_rotation was not one of the supported rotations.  Bail
     * out. */
 ...
 * easily.
 */
static void handle_rotate_windows(MsdXrandrManager *mgr, guint32 timestamp) {
  MsdXrandrManagerPrivate *priv = mgr->priv;
  MateRRScreen *screen = priv->rw_screen;
  MateRRConfig *current;
  ...
  if (rotatable_output_info == NULL) {
    g_debug(
        "No laptop outputs found to rotate; XF86RotateWindows key will do "
        "nothing");
    goto out;
  }
  ...
  get_allowed_rotations_for_output(current, priv->rw_screen,
                                   rotatable_output_info,
                                   &num_allowed_rotations, &allowed_rotations);
  next_rotation = get_next_rotation(
      allowed_rotations,
      mate_rr_output_info_get_rotation(rotatable_output_info));

  if (next_rotation ==
      mate_rr_output_info_get_rotation(rotatable_output_info)) {
    g_debug(
        "No rotations are supported other than the current one; "
        "XF86RotateWindows key will do nothing");
    goto out;
  }
...
static GdkFilterReturn event_filter(GdkXEvent *xevent, GdkEvent *event,
  ...
  if (!manager->priv->running) return GDK_FILTER_CONTINUE;
...
static void refresh_tray_icon_menu_if_active(MsdXrandrManager *manager,
                                             guint32 timestamp) {
  MsdXrandrManagerPrivate *priv = manager->priv;
...
static void auto_configure_outputs(MsdXrandrManager *manager,
                                   guint32 timestamp) {
  MsdXrandrManagerPrivate *priv = manager->priv;
  MateRRConfig *config;
  MateRROutputInfo **outputs;
  ...
  for (i = 0; outputs[i] != NULL; i++) {
    ...
    if (mate_rr_output_info_is_connected(output) &&
        !mate_rr_output_info_is_active(output)) {
    ...
    } else if (!mate_rr_output_info_is_connected(output) &&
               mate_rr_output_info_is_active(output))
  ...
  for (i = 0; outputs[i] != NULL; i++) {
    ...
    if (g_list_find(just_turned_on, GINT_TO_POINTER(i))) continue;
  ...
  for (l = just_turned_on; l; l = l->next) {
    ...
    g_assert(mate_rr_output_info_is_active(output) &&
             mate_rr_output_info_is_connected(output));

    /* since the output was off, use its preferred width/height (it doesn't have
     * a real width/height yet) */
    width = mate_rr_output_info_get_preferred_width(output);
    mate_rr_output_info_set_geometry(
        output, x, 0, width, mate_rr_output_info_get_preferred_height(output));
  ...
  just_turned_on = g_list_reverse(
      just_turned_on); /* now the outputs here are from right to left */
  ...
  while (1) {
    ...
    is_bounds_error =
        g_error_matches(error, MATE_RR_ERROR, MATE_RR_ERROR_BOUNDS_ERROR);
    g_error_free(error);
...
static void on_randr_event(MateRRScreen *screen, gpointer data) {
  ...
  if (change_timestamp >= config_timestamp) {
    ...
    show_timestamps_dialog(
        manager, "need to deal with reconfiguration, as config > change");

    intended_filename = mate_rr_config_get_intended_filename();

    error = NULL;
    success = apply_configuration_from_filename(manager, intended_filename,
                                                TRUE, config_timestamp, &error);
    g_free(intended_filename);
  ...
  refresh_tray_icon_menu_if_active(manager,
                                   MAX(change_timestamp, config_timestamp));
...
static void run_display_capplet(GtkWidget *widget) {
  ...
  error = NULL;
  if (!mate_gdk_spawn_command_line_on_screen(screen, MSD_XRANDR_DISPLAY_CAPPLET,
                                             &error)) {
    GtkWidget *dialog;
...
static void title_item_size_allocate_cb(GtkWidget *widget,
   ...
   * GtkMenu calls gtk_menu_item_toggle_size_allocate() with that value, to tell
   * the menu item that it should later paint its child a bit to the right of
   * its edge.
   *
   * However, we want the "title" menu items for each RANDR output to span the
   * *whole* allocation of the menu item, not just the "allocation minus toggle"
   * area.
   *
...
static GtkWidget *make_menu_item_for_output_title(MsdXrandrManager *manager,
                                                  MateRROutputInfo *output) {
  GtkWidget *item;
  ...
  /* Add padding around the label to fit the box that we'll draw for
   * color-coding */
  gtk_label_set_xalign(GTK_LABEL(label), 0.0);
  gtk_label_set_yalign(GTK_LABEL(label), 0.5);
  gtk_widget_set_margin_start(
      label, OUTPUT_TITLE_ITEM_BORDER + OUTPUT_TITLE_ITEM_PADDING);
  ...
  /*This can be overriden by themes, check all label:insensitive entries if it
   * does not show up*/
  string = g_string_new(NULL);
  g_string_append(
      string,
      ".mate-panel-menu-bar menuitem.xrandr-applet:disabled>box>label{\n");
  /*g_string_append (string, "color: black;"); Does not work-overridden in all
   * themes*/
  ...
  gtk_css_provider_load_from_data(
      ...
      "opacity: 1.0; \n"
      "-gtk-icon-style:regular; \n" /* symbolic icons would get the disabled
                                       color*/
  ...
  if (g_strcmp0(theme_name, "Adwaita") == 0 ||
    ...
    gtk_css_provider_load_from_data(
        ...
        "}"
        ".mate-panel-menu-bar "
        "menuitem.xrandr-applet.monitor-off:disabled>box>label{\n"
...
static void get_allowed_rotations_for_output(MateRRConfig *config,
  ...
  *out_rotations = 0;
  ...
  if (*out_num_rotations == 0 || *out_rotations == 0) {
    g_warning(
        "Huh, output %p says it doesn't support any rotations, and yet it has "
        "a current rotation?",
        output);
    *out_num_rotations = 1;
...
static void add_unsupported_rotation_item(MsdXrandrManager *manager) {
  struct MsdXrandrManagerPrivate *priv = manager->priv;
  GtkWidget *item;
  GtkWidget *label;
  gchar *markup;
...
static void monitor_activate_cb(GtkCheckMenuItem *item, gpointer data) {
  MsdXrandrManager *manager = MSD_XRANDR_MANAGER(data);
  struct MsdXrandrManagerPrivate *priv = manager->priv;
  MateRROutputInfo *output;
  GError *error;
  ...
  if (!mate_rr_config_save(priv->configuration, &error)) {
    error_message(manager, _("Could not save monitor configuration"), error,
                  NULL);
    if (error) g_error_free(error);
  ...
  try_to_apply_intended_configuration(manager, NULL,
...
static void output_rotation_item_activate_cb(GtkCheckMenuItem *item,
  ...
  try_to_apply_intended_configuration(
      manager, NULL, gtk_get_current_event_time(), NULL); /* NULL-GError */
}

static void mirror_outputs_cb(GtkCheckMenuItem *item, gpointer data) {
  MsdXrandrManager *manager = MSD_XRANDR_MANAGER(data);
  struct MsdXrandrManagerPrivate *priv = manager->priv;
  MateRRScreen *screen = priv->rw_screen;
  ...
  if (gtk_check_menu_item_get_active(item)) {
    ...
    /*If nothing worked, bring up the display capplet so the user can
     * reconfigure*/
    if (config == NULL) run_display_capplet(GTK_WIDGET(item));
    mate_rr_config_save(config, NULL);
    try_to_apply_intended_configuration(manager, NULL,
                                        gtk_get_current_event_time(), NULL);
...
static void add_items_for_rotations(MsdXrandrManager *manager,
                                    MateRROutputInfo *output,
                                    MateRRRotation allowed_rotations) {
  typedef struct {
    MateRRRotation rotation;
  ...
  static const RotationInfo rotations[] = {
      ...
      /* We don't allow REFLECT_X or REFLECT_Y for now, as
         mate-display-properties doesn't allow them, either */
  };
  ...
  for (i = 0; i < G_N_ELEMENTS(rotations); i++) {
    ...
    if (!(mate_rr_output_info_is_active(output))) {
      gtk_widget_set_sensitive(
          item, FALSE); /*Rotation can't be set from the OFF state*/
    }
    gtk_widget_show_all(item);
    gtk_menu_shell_append(GTK_MENU_SHELL(priv->popup_menu), item);
...
static void add_rotation_items_for_output(MsdXrandrManager *manager,
                                          MateRROutputInfo *output) {
  struct MsdXrandrManagerPrivate *priv = manager->priv;
  int num_rotations;
  MateRRRotation rotations;

  get_allowed_rotations_for_output(priv->configuration, priv->rw_screen, output,
...
static void add_enable_option_for_output(MsdXrandrManager *manager,
                                         MateRROutputInfo *output) {
  struct MsdXrandrManagerPrivate *priv = manager->priv;
  GtkWidget *item;
  gulong activate_id;
...
static void add_menu_items_for_output(MsdXrandrManager *manager,
                                      MateRROutputInfo *output) {
  struct MsdXrandrManagerPrivate *priv = manager->priv;
  GtkWidget *item;
...
static void add_menu_items_for_outputs(MsdXrandrManager *manager) {
  struct MsdXrandrManagerPrivate *priv = manager->priv;
  int i;
  MateRROutputInfo **outputs;
...
static void add_menu_items_for_clone(MsdXrandrManager *manager) {
  struct MsdXrandrManagerPrivate *priv = manager->priv;
  GtkWidget *item;
  gulong activate_id;
  ...
  gtk_widget_set_tooltip_text(
      item, _("Mirror same output to all monitors and turn them on"));
  gtk_widget_show_all(item);
  gtk_menu_shell_append(GTK_MENU_SHELL(priv->popup_menu), item);
  activate_id = g_signal_connect(item, "activate",
...
static void status_icon_popup_menu(MsdXrandrManager *manager, guint button,
  ...
  gtk_container_add(GTK_CONTAINER(item), box);
  gtk_widget_set_tooltip_text(
      item, _("Open the display configuration dialog (all settings)"));
  g_signal_connect(item, "activate",
  ...
  g_signal_connect(priv->popup_menu, "selection-done",
                   G_CALLBACK(status_icon_popup_menu_selection_done_cb),
                   manager);
  ...
  gtk_menu_popup(GTK_MENU(priv->popup_menu), NULL, NULL,
                 gtk_status_icon_position_menu, priv->status_icon, button,
                 timestamp);
}
...
static void status_icon_start(MsdXrandrManager *manager) {
  struct MsdXrandrManagerPrivate *priv = manager->priv;
  ...
  if (!priv->status_icon) {
    priv->status_icon =
        gtk_status_icon_new_from_icon_name(MSD_XRANDR_ICON_NAME);
    gtk_status_icon_set_tooltip_text(priv->status_icon,
                                     _("Configure display settings"));
...
static void status_icon_stop(MsdXrandrManager *manager) {
  struct MsdXrandrManagerPrivate *priv = manager->priv;
...
static void start_or_stop_icon(MsdXrandrManager *manager) {
  if (g_settings_get_boolean(manager->priv->settings,
                             CONF_KEY_SHOW_NOTIFICATION_ICON)) {
...
static gboolean apply_intended_configuration(MsdXrandrManager *manager,
                                             const char *intended_filename,
                                             guint32 timestamp) {
  GError *my_error;
  gboolean result;
  ...
  my_error = NULL;
  result = apply_configuration_from_filename(manager, intended_filename, TRUE,
                                             timestamp, &my_error);
  if (!result) {
    if (my_error) {
      if (!g_error_matches(my_error, G_FILE_ERROR, G_FILE_ERROR_NOENT) &&
          !g_error_matches(my_error, MATE_RR_ERROR,
                           MATE_RR_ERROR_NO_MATCHING_CONFIG))
        error_message(
            manager, _("Could not apply the stored configuration for monitors"),
            my_error, NULL);
...
static void apply_default_boot_configuration(MsdXrandrManager *mgr,
                                             guint32 timestamp) {
  MsdXrandrManagerPrivate *priv = mgr->priv;
  MateRRScreen *screen = priv->rw_screen;
  MateRRConfig *config;
  ...
  turn_on_external = g_settings_get_boolean(
      mgr->priv->settings, CONF_KEY_TURN_ON_EXTERNAL_MONITORS_AT_STARTUP);
  turn_on_laptop = g_settings_get_boolean(
...
static gboolean apply_stored_configuration_at_startup(MsdXrandrManager *manager,
                                                      guint32 timestamp) {
  GError *my_error;
  gboolean success;
  char *backup_filename;
   ...
   * machine crashed.  In that case, we'll apply *that* configuration and save
   * it on top of the "intended" one.
   */

  my_error = NULL;

  success = apply_configuration_from_filename(manager, backup_filename, FALSE,
                                              timestamp, &my_error);
  if (success) {
...
out:
  if (my_error) g_error_free(my_error);
...
static gboolean apply_default_configuration_from_file(MsdXrandrManager *manager,
                                                      guint32 timestamp) {
  MsdXrandrManagerPrivate *priv = manager->priv;
  char *default_config_filename;
  gboolean result;

  default_config_filename = g_settings_get_string(
      priv->settings, CONF_KEY_DEFAULT_CONFIGURATION_FILE);
  if (!default_config_filename) return FALSE;

  result = apply_configuration_from_filename(manager, default_config_filename,
                                             TRUE, timestamp, NULL);
...
gboolean msd_xrandr_manager_start(MsdXrandrManager *manager, GError **error) {
  ...
  manager->priv->rw_screen =
      mate_rr_screen_new(gdk_screen_get_default(), error);
  ...
  g_signal_connect(manager->priv->rw_screen, "changed",
                   G_CALLBACK(on_randr_event), manager);
  ...
  if (manager->priv->switch_video_mode_keycode) {
    ...
    XGrabKey(gdk_x11_get_default_xdisplay(),
             ...
             gdk_x11_get_default_root_xwindow(), True, GrabModeAsync,
             GrabModeAsync);
  ...
  if (!apply_stored_configuration_at_startup(
          manager, GDK_CURRENT_TIME)) /* we don't have a real timestamp at
                                         startup anyway */
    if (!apply_default_configuration_from_file(manager, GDK_CURRENT_TIME))
      if (!g_settings_get_boolean(manager->priv->settings,
                                  CONF_KEY_USE_XORG_MONITOR_SETTINGS))
...
static void msd_xrandr_manager_class_init(MsdXrandrManagerClass *klass) {
  ...
  dbus_g_object_type_install_info(MSD_TYPE_XRANDR_MANAGER,
                                  &dbus_glib_msd_xrandr_manager_object_info);
}

static guint get_keycode_for_keysym_name(const char *name) {
  Display *dpy;
  guint keyval;
...
static void msd_xrandr_manager_init(MsdXrandrManager *manager) {
  manager->priv = msd_xrandr_manager_get_instance_private(manager);

  manager->priv->switch_video_mode_keycode =
      get_keycode_for_keysym_name(VIDEO_KEYSYM);
  manager->priv->rotate_windows_keycode =
      get_keycode_for_keysym_name(ROTATE_KEYSYM);
...
static gboolean register_manager_dbus(MsdXrandrManager *manager) {
  GError *error = NULL;
  ...
  dbus_g_connection_register_g_object(manager->priv->dbus_connection,
                                      MSD_XRANDR_DBUS_PATH, G_OBJECT(manager));
